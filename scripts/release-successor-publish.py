...
def main() -> int:
    ...
    try:
        ...
        require(manifest["descriptor"]["version"] == "0.91.3", "publisher version differs")
        ...
        try:
            ...
            try:
                api.get("repos/FS-GG/.github/git/ref/tags/coherent-set/v0.91.3")
            except NotFound:
                pass
            else:
            ...
            subprocess.run(
                [sys.executable, str(pathlib.Path(__file__).with_name("check-release-candidate-uniqueness.py")),
                 "--version", "0.91.3", "--predecessor", "0.91.2"],
