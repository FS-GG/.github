...
class AtomicRecoveryStorePort(Protocol):
    ...
    high-water, signed head generation, and pinned floor resource in the same
    transaction, then commits all jobs together or none. `claim` repeats that
    comparison at its own linearization point. `withdraw` and `claim`
    ...
    def append_schedule_batch_once(self, batch: RecoveryBatch, seal_id: str,
                                   high_water: int, expected_generation: int,
                                   floor_resource_id: str) -> StoreOutcome: ...
