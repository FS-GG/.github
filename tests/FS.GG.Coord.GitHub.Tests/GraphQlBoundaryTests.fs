...
let ``rate limit carries retry and rate metadata`` () =
    let body =
        """{"data":{"answer":42},"errors":[{"message":"API rate limit exceeded"}]}"""
...
let ``repeated cursor refuses instead of looping or succeeding short`` () =
    ...
    let fetch _ =
        let result = page "repeat" bodies.[index] in
        index <- index + 1
        result
...
let ``duplicate identity across pages exposes page-boundary mutation`` () =
    ...
    let fetch _ =
        let result = page "mutation" bodies.[index] in
        index <- index + 1
        result
...
let ``changing total count across pages is a typed incomplete read`` () =
    ...
    let fetch _ =
        let result = page "count" bodies.[index] in
        index <- index + 1
        result
...
let ``empty continuing page and explicit item limit both fail closed`` () =
    let empty =
        """{"data":{"connection":{"totalCount":1,"nodes":[],"pageInfo":{"hasNextPage":true,"endCursor":"one"}}}}"""
    ...
    let full =
        """{"data":{"connection":{"totalCount":2,"nodes":[{"id":"a"},{"id":"b"}],"pageInfo":{"hasNextPage":false,"endCursor":null}}}}"""
...
let ``operational project lookup drains every page and returns one typed visibility`` () =
    let replies =
        System.Collections.Generic.Queue<_>(
            [
                response
                    """{"data":{"organization":{"projectsV2":{"totalCount":2,"nodes":[{"id":"one","title":"Other","public":false,"number":2}],"pageInfo":{"hasNextPage":true,"endCursor":"next"}}},"rateLimit":{"cost":1,"remaining":99}}}"""
                response
                    """{"data":{"organization":{"projectsV2":{"totalCount":2,"nodes":[{"id":"target","title":"Coordination","public":true,"number":1}],"pageInfo":{"hasNextPage":false,"endCursor":null}}},"rateLimit":{"cost":1,"remaining":98}}}"""
            ]
        )
...
let ``operational project lookup refuses duplicate title rather than choosing`` () =
    let body =
        """{"data":{"organization":{"projectsV2":{"totalCount":2,"nodes":[{"id":"one","title":"Coordination","public":false,"number":1},{"id":"two","title":"Coordination","public":true,"number":2}],"pageInfo":{"hasNextPage":false,"endCursor":null}}},"rateLimit":{"cost":1,"remaining":99}}}"""
...
let ``operational repository policy never inverts mixed data and errors`` () =
    let body =
        """{"data":{"repository":{"issueCreationPolicy":"COLLABORATORS_ONLY","hasIssuesEnabled":true}},"errors":[{"message":"field denied"}]}"""
...
let ``#3091 incomplete merge policy fails closed`` () =
    let body =
        """{"data":{"repository":{"issueCreationPolicy":"COLLABORATORS_ONLY","hasIssuesEnabled":true,"mergeCommitAllowed":false,"squashMergeAllowed":true},"rateLimit":{"cost":1,"remaining":99}}}"""
...
let ``board intake shares repository policy and author permission while rebinding edited issues`` () =
    ...
    let isPolicy (request: Request) =
        match request.Body with
        | Query(document, _) -> document.Contains "issueCreationPolicy"
        | _ -> false
    ...
    let transport =
        Fake.Recorder(fun request ->
            if isPolicy request then
                response
                    """{"data":{"repository":{"id":"R_repo","issueCreationPolicy":"COLLABORATORS_ONLY","hasIssuesEnabled":true,"mergeCommitAllowed":true,"squashMergeAllowed":true,"rebaseMergeAllowed":true},"rateLimit":{"cost":1,"remaining":99}}}"""
            elif request.Budget = GraphQl then
                match request.Body with
                | Query(document, _) -> Assert.Contains("... on Node{id}", document)
                | _ -> failwith "expected typed issue identity query"

                revision <- revision + 1

                response (
                    $"{{\"data\":{{\"repository\":{{\"issue\":{{\"id\":\"I_issue\",\"updatedAt\":\"2026-09-10T00:00:0%d{revision}Z\",\"author\":{{\"id\":\"U_author\",\"login\":\"maintainer\"}}}}}},\"rateLimit\":{{\"cost\":1,\"remaining\":98}}}}}}"
                )
            elif request.Path.Contains("/collaborators/") then
                response """{"permission":"write","user":{"node_id":"U_author"}}"""
            else
    ...
    let require =
        function
        | Ok value -> value
        | Error error -> failwith $"unexpected refusal: %A{error}"

    let first = gate.Authorize("FS-GG", "demo", 1) |> require
    let edited = gate.Authorize("FS-GG", "demo", 1) |> require

    let laterBatch =
        BoardIntake.Gate(transport, cacheRoot = cache).Authorize("FS-GG", "demo", 1)
        |> require
    ...
    let transport =
        Fake.Recorder(fun request ->
            if isPolicy request then
                ...
                response (
                    $"{{\"data\":{{\"repository\":{{\"id\":\"R_repo\",\"issueCreationPolicy\":\"%s{policy}\",\"hasIssuesEnabled\":true,\"mergeCommitAllowed\":true,\"squashMergeAllowed\":true,\"rebaseMergeAllowed\":true}},\"rateLimit\":{{\"cost\":1,\"remaining\":99}}}}}}"
                )
            elif request.Budget = GraphQl then
                response
                    """{"data":{"repository":{"issue":{"id":"I_issue","updatedAt":"2026-09-10T00:00:00Z","author":{"id":"U_author","login":"reader"}}},"rateLimit":{"cost":1,"remaining":98}}}"""
            else
                response """{"permission":"read","user":{"node_id":"U_author"}}""")

    let cache = Path.Combine(Path.GetTempPath(), System.Guid.NewGuid().ToString("N"))

    match
        BoardIntake
            .Gate(transport, cacheRoot = Path.Combine(Path.GetTempPath(), System.Guid.NewGuid().ToString("N")))
            .Authorize("FS-GG", "demo", 1)
    with
...
let ``operational archive mutation preserves partial alias accounting`` () =
    let body =
        """{"data":{"a0":{"item":{"id":"one"}},"a1":null},"errors":[{"message":"denied","path":["a1"]}]}"""
...
let ``operational roster board refuses a partially resolved issue node`` () =
    let projects =
        """{"data":{"organization":{"projectsV2":{"totalCount":1,"nodes":[{"id":"board","title":"Coordination","public":false,"number":1}],"pageInfo":{"hasNextPage":false,"endCursor":null}}},"rateLimit":{"cost":1,"remaining":99}}}"""

    let items =
        """{"data":{"organization":{"projectV2":{"items":{"totalCount":1,"nodes":[{"id":"row","status":{"name":"Ready"},"content":{"__typename":"Issue","number":1}}],"pageInfo":{"hasNextPage":false,"endCursor":null}}}},"rateLimit":{"cost":1,"remaining":98}}}"""

    let replies =
        System.Collections.Generic.Queue<_>([ response projects; response items ])
...
let ``board intake persistent observations retain their original expiry`` () =
    ...
    let transport =
        Fake.Recorder(fun request ->
            if isPolicy request then
                ...
                response
                    """{"data":{"repository":{"issue":{"id":"I_issue","updatedAt":"2026-09-10T00:00:00Z","author":{"id":"U_author","login":"maintainer"}}},"rateLimit":{"cost":1,"remaining":98}}}"""
            else
                permissionReads <- permissionReads + 1

                response (
                    if permissionReads = 1 then
                        """{"permission":"write","user":{"node_id":"U_author"}}"""
                    else
                        """{"permission":"admin","user":{"node_id":"U_author"}}"""
                ))

    let cache = Path.Combine(Path.GetTempPath(), System.Guid.NewGuid().ToString("N"))

    let first =
        BoardIntake.Gate(transport, System.TimeSpan.FromSeconds 30., cache, fun () -> now)
    ...
    let refreshed =
        loaded.Authorize("FS-GG", "CaseRepo", 1)
    ...
    BoardIntake.Gate(transport, System.TimeSpan.FromSeconds 30., cache, fun () -> now).Authorize("FS-GG", "CaseRepo", 1)
    |> ignore
...
let ``board intake never authorizes from corrupt persistent evidence when refresh fails`` () =
    ...
    BoardIntake.Gate(good, cacheRoot = cache).Authorize("FS-GG", "demo", 1)
    |> ignore
