...
module QualificationEvidenceTests =
            ...
            QualificationEvidence.observeHosted
                {
                    ...
                    Items =
                        [
                            {
                                Scope = QualificationEvidence.WorkflowRun
                                Id = "10"
                                HeadSha = head
                                State = QualificationEvidence.Completed "success"
                            }
                            {
                                Scope = QualificationEvidence.Job
                                Id = "20"
                                HeadSha = head
                                State = QualificationEvidence.InProgress
                            }
                            {
                                Scope = QualificationEvidence.CheckRun
                                Id = "30"
                                HeadSha = System.String('b', 40)
                                State = QualificationEvidence.Queued
                            }
                        ]
                }
    ...
    let ``#3209 renders the existing delivery obligation grammar exactly`` () =
        let none =
            QualificationEvidence.renderObligationComment head Qualification.NoObligations
    ...
    let ``#3209 wrong typed hosted snapshot returns typed error`` () =
        let bytes =
            System.Text.Encoding.UTF8.GetBytes(
                "{\"schema\":\"fsgg.qualification.hosted-observation/1\",\"complete\":1,\"items\":[]}"
            )
    ...
    let ``#3209 authoritative readback preserves the existing delivery marker body`` () =
        ...
        let observed =
            QualificationEvidence.parseObligationReadback bytes
