...
module BrowserTests =
    let private directory () =
        let path =
            Path.Combine(Path.GetTempPath(), "browser-tests-" + Guid.NewGuid().ToString("N"))
    ...
    let private principal root id workspaces revoked =
        ...
        File.WriteAllText(
            path,
            $"{{\"schema\":\"fsgg.telemetry.browser-key/1\",\"algorithm\":\"sha256\",\"keyHash\":\"{digest}\"}}"
        )
    ...
    let private options origin sessions rate queries =
        {
            PublicOrigin = Uri origin
            IdleLifetime = TimeSpan.FromMinutes 15.
            AbsoluteLifetime = TimeSpan.FromHours 8.
            MaximumSessions = sessions
            LoginAttemptsPerMinute = rate
            LoginAdmission = 4
            QueryAdmission = queries
    ...
    let ``sessions rotate logout expire and reject producer material`` () =
        ...
        try
            ...
            use service =
                new BrowserSecurity.Service(options "https://localhost:9443" 4 8 2, [| config |])

            Assert.Equal(LoginDenied, service.Login("reader", "producer-bearer-token", DateTimeOffset.UnixEpoch))

            let session =
                match service.Login("reader", access, DateTimeOffset.UnixEpoch) with
                | LoginAccepted(value, _) -> value
                | result -> failwithf "%A" result

            let replacement, _ =
                service.Rotate(session, DateTimeOffset.UnixEpoch.AddMinutes 1.) |> Option.get
            ...
            match service.Login("reader", access, DateTimeOffset.UnixEpoch.AddMinutes 16.) with
            | LoginAccepted _ -> ()
            | result -> failwithf "expired slot was not reusable: %A" result

            let absolute =
                match service.Login("reader", access, DateTimeOffset.UnixEpoch.AddMinutes 17.) with
                | LoginAccepted(value, _) -> value
                | result -> failwithf "%A" result

            for minute in 27..10..477 do
                Assert.True(service.Validate(absolute, DateTimeOffset.UnixEpoch.AddMinutes(float minute)).IsSome)
            ...
            for cycle in 1..100 do
                ...
                let value =
                    match service.Login("reader", access, created) with
                    | LoginAccepted(id, _) -> id
                    | result -> failwithf "%A" result
    ...
    let ``session rotation is single winner under concurrency`` () =
        ...
        try
            ...
            use service =
                new BrowserSecurity.Service(options "https://localhost:9443" 8 16 1, [| config |])
            ...
            let results =
                Array.init 16 (fun _ ->
                    Task.Run(fun () -> service.Rotate(session, DateTimeOffset.UnixEpoch.AddMinutes 1.)))
                |> Task.WhenAll
    ...
    let ``capacity rate revoked and unsafe key files fail closed`` () =
        ...
        try
            ...
            use capped =
                new BrowserSecurity.Service(options "https://localhost:9443" 1 8 1, [| config |])

            match capped.Login("reader", access, DateTimeOffset.UnixEpoch) with
            | LoginAccepted _ -> ()
            | result -> failwithf "%A" result
            ...
            use concurrent =
                new BrowserSecurity.Service(options "https://localhost:9443" 4 32 1, [| config |])

            let admitted =
                Array.init 32 (fun _ ->
                    Task.Run(fun () -> concurrent.Login("reader", access, DateTimeOffset.UnixEpoch)))
                |> Task.WhenAll
                |> fun task -> task.Result
            ...
            Assert.Equal(
                ...
                |> Array.filter (function
                    | LoginAccepted _ -> true
                    | _ -> false)
                |> Array.length
            )

            use rated =
                new BrowserSecurity.Service(options "https://localhost:9443" 8 1 1, [| config |])
            ...
            use revokedService =
                new BrowserSecurity.Service(options "https://localhost:9443" 2 4 1, [| revoked |])
            ...
            File.WriteAllText(
                zeroPath,
                $"{{\"schema\":\"fsgg.telemetry.browser-key/1\",\"algorithm\":\"sha256\",\"keyHash\":\"{String('0', 64)}\"}}"
            )

            File.SetUnixFileMode(zeroPath, UnixFileMode.UserRead ||| UnixFileMode.UserWrite)

            use zeroService =
                new BrowserSecurity.Service(
                    options "https://localhost:9443" 2 4 1,
                    [|
                        {
                            PrincipalId = "zero"
                            KeyHashFile = zeroPath
            ...
            Assert.True(
                BrowserSecurity.validatePrincipals
                    [|
                        {
                            PrincipalId = "zero"
                            KeyHashFile = zeroPath
    ...
    let private request
        ...
        body
        |> Option.iter (fun content -> value.Content <- new StringContent(content, Encoding.UTF8, "application/json"))

        origin |> Option.iter (fun item -> value.Headers.Add("Origin", item))

        cookie
        |> Option.iter (fun item -> value.Headers.Add("Cookie", $"__Host-fsgg_session={item}"))

        value

    let private session (response: HttpResponseMessage) =
        response.Headers.GetValues("Set-Cookie")
        |> Seq.head
        |> fun value -> (value.Split(';')[0]).Split('=')[1]

    let private wrongHostStatus port (expected: X509Certificate2) =
        task {
            ...
            use tls =
                new SslStream(
                    ...
                    false,
                    fun _ seen _ errors ->
                        seen.GetCertHashString() = expected.GetCertHashString()
    ...
    let private runRealBrowser origin access (certificate: X509Certificate2) =
        if Environment.GetEnvironmentVariable("FSGG_RUN_REAL_BROWSER") = "1" then
            ...
            [
                "FSGG_BROWSER_BASE_URL", origin
                "FSGG_BROWSER_PRINCIPAL_ID", "reader"
                "FSGG_BROWSER_ACCESS_KEY", access
                "FSGG_BROWSER_KNOWN_WORKSPACE", "workspace-a"
                "FSGG_BROWSER_UNAVAILABLE_WORKSPACE", "workspace-b"
                "FSGG_BROWSER_CERTIFICATE_SPKI", spki
                "FSGG_BROWSER_ASSERT_UNKNOWN_COUNTS", "1"
            ]
    ...
    [<Fact>]
    let ``real TLS endpoints enforce independent auth scope origin capacity and private headers`` () =
        task {
            let root = directory ()
            let access, config = principal root "reader" [| "workspace-a" |] false
            let port = port ()
            ...
            builder.WebHost.ConfigureKestrel(fun server ->
                server.ListenLocalhost(port, fun listen -> listen.UseHttps(cert) |> ignore))
            |> ignore
            ...
            let provider workspace item =
                ...
                if item = Some "missing" then
                    Error["sensitive /path sqlite failure"]
            ...
            try
                use handler =
                    new HttpClientHandler(UseCookies = false, AutomaticDecompression = DecompressionMethods.None)

                handler.ServerCertificateCustomValidationCallback <-
                    fun _ seen _ errors ->
                        seen.Thumbprint = cert.Thumbprint
                        && (errors &&& SslPolicyErrors.RemoteCertificateNameMismatch) = SslPolicyErrors.None
                ...
                Assert.True(asset.Headers.Contains "Content-Security-Policy")
                ...
                use badOrigin =
                    request
                        ...
                        (Some($"{{\"principalId\":\"reader\",\"accessKey\":\"{access}\"}}"))
                ...
                use malformed =
                    request HttpMethod.Post "/private/dashboard/v1/snapshot" (Some "[]") (Some origin) (Some cookie)
                ...
                use oversized =
                    request
                        HttpMethod.Post
                        "/private/dashboard/v1/snapshot"
                        (Some("{\"workspaceId\":\"" + String('x', 1100) + "\"}"))
                        (Some origin)
                        (Some cookie)
                ...
                let denied item workspace =
                    task {
                        use message =
                            request
                                HttpMethod.Post
                                "/private/dashboard/v1/snapshot"
                                (Some($"{{\"workspaceId\":\"{workspace}\",\"itemId\":\"{item}\"}}"))
                                (Some origin)
                                (Some cookie)
                ...
                Assert.DoesNotContain("sensitive", thrown.Content.ReadAsStringAsync().Result)
                ...
                Assert.Contains("gzip", success.Content.Headers.ContentEncoding)
                Assert.Equal("no-store", string success.Headers.CacheControl)
                use slowStarted = new ManualResetEventSlim(false)
                use slowRelease = new ManualResetEventSlim(false)

                use slow =
                    request HttpMethod.Post "/private/dashboard/v1/snapshot" None (Some origin) (Some cookie)
                ...
                use hold =
                    request
                        HttpMethod.Post
                        "/private/dashboard/v1/snapshot"
                        (Some "{\"workspaceId\":\"workspace-a\",\"itemId\":\"hold\"}")
                        (Some origin)
                        (Some cookie)
                ...
                use refresh =
                    request HttpMethod.Post "/private/dashboard/v1/session/refresh" None (Some origin) (Some cookie)
                ...
                use logout =
                    request HttpMethod.Post "/private/dashboard/v1/logout" None (Some origin) (Some replacement)
