...
for n in kit drivers; do eval "r=\$$n"; printf '<testcase name="%s">' "$n" >>"$out"; [ "$r" -eq 0 ] || printf '<failure message="exit %s"/>' "$r" >>"$out"; printf '<system-out><![CDATA[' >>"$out"; cat "$tmp/$n" >>"$out"; printf ']]></system-out></testcase>' >>"$out"; done
