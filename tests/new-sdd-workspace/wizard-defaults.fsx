...
let recovery =
    securityResumeCommand "./workspace with 'quote" [ "--repo"; "acme/app" ]

let expectedTarget = "'" + recoveryTarget.Replace("'", "'\"'\"'") + "'"

if
    recovery
    <> sprintf "new-sdd-workspace secure %s '--repo' 'acme/app'" expectedTarget
then
