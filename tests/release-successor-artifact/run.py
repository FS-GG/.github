...
def package(path: pathlib.Path, package_id: str) -> None:
    nuspec = (
        f'<package><metadata><id>{package_id}</id><version>0.91.0</version>'
        f'<releaseNotes>0.91.0 fixture</releaseNotes><repository type="git" commit="{SOURCE}" />'
        "</metadata></package>"
    )
...
class CandidateArtifactTests(unittest.TestCase):
    def setUp(self):
        ...
        for package_id in PACKAGES:
            package(self.candidate / f"{package_id}.0.91.0.nupkg", package_id)
        predecessor = {
        ...
        package_sha = hashlib.sha256((self.candidate / "FS.GG.Coord.Cli.0.91.0.nupkg").read_bytes()).hexdigest()
        qualification = {
        ...
        subprocess.run([
            ...
            "--release-id", "github:0.91.0", "--version", "0.91.0", "--source-sha", SOURCE,
