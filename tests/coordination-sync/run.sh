...
for i in "${!SKILLS[@]}"; do
  ...
  # skill).
  [ -e "$RECV/.agents/skills/$s" ] \
    && bad "apply: $s must NOT be distributed into the view root .agents/skills" \
           "a view root is generated by scripts/skill-view, never transported (ADR-0067 §9 stage 2)" \
    || ok "apply: $s is NOT written into the view root .agents/skills"
done
