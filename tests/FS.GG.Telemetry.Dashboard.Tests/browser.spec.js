...
test("login, scoped workspace refresh, expiry and logout use the frozen private contract",async({page})=>{
  ...
  await expect(page.locator("#workspace option")).toHaveCount(2);await expect(page.locator("article h3")).toHaveText("<img src=x onerror=alert(1)>");await expect(page.locator("article img")).toHaveCount(0);expect(snapshots[0]).toEqual({workspaceId:"workspace-a"});
