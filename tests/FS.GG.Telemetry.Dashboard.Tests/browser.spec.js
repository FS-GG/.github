...
test("mocked transport UI journey covers login, scoped refresh, expiry and logout",async({page})=>{
  ...
  await expect(page.locator("#workspace option")).toHaveCount(2);await expect(page.locator("article h3")).toHaveText(["<img src=x onerror=alert(1)>","partial-item","complete-item"]);await expect(page.locator("article img")).toHaveCount(0);await expect(page.locator("article .metric")).toContainText(["Native tokens unavailable","14 observed native tokens · partial population coverage","21 native tokens"]);await expect(page.locator("#health")).toContainText("2 pending receipts · 3 applied receipts · 1 rejected receipts");expect(snapshots[0]).toEqual({workspaceId:"workspace-a"});
