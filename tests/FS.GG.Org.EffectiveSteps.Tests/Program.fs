...
check "checker piped to successful command" false (workflow "python3 scripts/check-alpha.py | cat\nbash tests/alpha/run.sh" "" "") "#!/bin/sh\necho harmless\n"
check "checker followed by unconditional success" false (workflow "python3 scripts/check-alpha.py ; echo ignored\nbash tests/alpha/run.sh" "" "") "#!/bin/sh\necho harmless\n"
check "masked fixture fallback" false (workflow "bash tests/alpha/run.sh || echo ignored" "" "") fixtureBody
check "fixture piped to successful command" false (workflow "bash tests/alpha/run.sh | cat" "" "") fixtureBody
...
check "piped transitive checker" false (workflow "bash tests/alpha/run.sh" "" "") "#!/bin/sh\npython3 scripts/check-alpha.py | cat\n"
check "dead shell branch" false (workflow "if false; then\n  python3 scripts/check-alpha.py\n  bash tests/alpha/run.sh\nfi" "" "") fixtureBody
...
check "tagged duplicate run key" false ((workflow direct "" "") + "        !!str run: echo harmless\n") fixtureBody
check "tagged duplicate jobs key" false ((workflow direct "" "") + "!!str jobs: {}\n") fixtureBody
check "tagged duplicate active condition" false (workflow direct "" "        if: true\n        !!str if: false\n") fixtureBody
check "YAML block scalar anchor executes" true "jobs:\n  gate:\n    steps:\n      - run: &gate |\n          python3 scripts/check-alpha.py\n          bash tests/alpha/run.sh\n      - run: *gate\n" fixtureBody
...
check "multiple YAML documents" false ((workflow direct "" "") + "---\njobs: {}\n") fixtureBody
check "malformed steps shape" false "jobs:\n  gate:\n    steps: nope\n" fixtureBody
printfn "33 executable-step controls passed"
