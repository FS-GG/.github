...
module RoadmapWorkUnitTransactionTests =
    ...
    let private makePlanFor registrationRepository =
        ...
        RoadmapWorkUnit.compilePreparation (Encoding.UTF8.GetBytes roadmap) (Encoding.UTF8.GetBytes catalog) request
        |> unwrap
    ...
    let ``#3210 compiler registrations are byte-stable inputs to the sole staged-intake transaction`` () =
        ...
        let identities =
            plan.Registrations
            |> List.map (fun registration -> registration.Id, IntakeReceipt.digest registration.Draft)
        ...
        for registration in plan.Registrations do
            let path =
                Path.Combine(Path.GetTempPath(), "fsgg-3210-intake-" + Guid.NewGuid().ToString("n") + ".json")
    ...
    let ``#3210 lifecycle authority preserves historical claim generations and binds the terminal winner`` () =
        let expectation: Handlers.LifecycleAuthorityExpectation =
            {
                ...
                Number = 500
                Url = "https://github.com/FS-GG/.github/issues/500"
                Subject = "FS-GG/.github#500"
                CurrentClaimGeneration = "200"
                ImplementationRepository = "FS-GG/.github"
        ...
        let staleTerminal =
            turnover.Replace("\"claim_generation\":\"200\"", "\"claim_generation\":\"100\"")

        Assert.Contains(
            "terminal lifecycle event claim generation is not the live winning claim",
            Handlers.validateLifecycleAuthority expectation staleTerminal
        )

        let staleMerge =
            turnover.Replace(expectation.ImplementationMerge, expectation.ImplementationCandidate)

        Assert.Contains(
            Handlers.validateLifecycleAuthority expectation staleMerge,
            fun error -> error.Contains("source revision is invalid for phase merge", StringComparison.Ordinal)
        )
        ...
        Assert.Contains(
            Handlers.validateLifecycleAuthority expectation staleReconciliation,
            fun error ->
                error.Contains(
                    "source revision is invalid for phase telemetry-reconciliation-merge",
    ...
    let ``#3210 SDD work model refuses any pending task`` () =
        let complete =
            """{"workId":"500-roadmap-gs2-07-3","tasks":[{"id":"T001","status":"done"},{"id":"T002","status":"done"}]}"""

        Assert.Empty(Handlers.validateCompleteSddWorkModel "500-roadmap-gs2-07-3" complete)

        let pending =
            complete.Replace("\"status\":\"done\"}", "\"status\":\"pending\"}", StringComparison.Ordinal)
            ...
            (fun log (order, phase) ->
                let first =
                    LifecycleTelemetry.sealSuccessor
                        "roadmap-unit-gs2-07.3"
                        "GS2-07.3"
                        log
                        (draft order phase "started" "2026-09-05T06:00:00Z" "null" "{\"status\":\"pending\"}")
                    |> unwrap
                ...
                + (LifecycleTelemetry.sealSuccessor
                    ...
                    (draft
                        ...
                        "1"
                        "{\"status\":\"unavailable\",\"reason\":\"post-completion runtime usage lookup failed: test fixture has no source\",\"source\":\"test\"}")
                   |> unwrap))
            ""
    ...
    let private authorityRouteFixtureFor
        ...
        let observation stage status : RoadmapWorkUnit.SddObservation =
            let stageFields =
                if stage = "ship" then
                    $""""sourcesDigest":{{"algorithm":"sha256","value":"%s{String.replicate 64 "7"}"}},"verificationReadiness":{{"status":"verificationReady"}},"disposition":{{"state":"shipReady","blockingFindingIds":[]}},"""
                else
                    $""""sources":[{{"path":"readiness/%s{sddWorkId}/work-model.json"}}],"findings":[],"diagnostics":[],"""

            {
                Stage = stage
                SubjectRevision = candidate
                ArtifactJson =
                    $"""{{"schemaVersion":1,"viewVersion":"1.0","generator":"FS.GG.SDD.Artifacts/1.5.0",%s{stageFields}"stage":"%s{stage}","status":"%s{status}","readiness":"%s{status}","workId":"%s{sddWorkId}"}}"""
            }
        ...
        let executor =
            {|
                id = "implementer"
                role = "implementer"
                implementationSha256 = digest '2'
            |}

        let artifact stage =
            observations
            |> List.find (fun value -> value.Stage = stage)
            |> _.ArtifactJson
        ...
        let operation id kind artifactSha exitCode refusal replay =
            {|
                ...
                tool = tool
                executor = executor
                commandSha256 = digest '3'
                artifactSha256 = [ artifactSha ]
                resultSha256 = digest '5'
                    ...
                    mutations =
                        [
                            {|
                                ...
                                productionImplementationSha256 = digest '2'
                                fixtureImplementationSha256 = digest '6'
                            ...
                            readbacks =
                                [
                                    {|
                                        commentId = 1L
                                        url =
                                            $"https://github.com/FS-GG/%s{registrationRepository}/pull/%d{implementationPr}#issuecomment-1"
                                        author = "bot"
                                    |}
                                ]
                        |}
        ...
        let input: RoadmapWorkUnit.AcceptanceInput =
            {
                ...
                LifecycleLog =
                    fullLifecycle
                        ($"FS-GG/%s{registrationRepository}")
                        candidate
                        implementationMerge
                        acceptanceMerge
                        number
                ...
                ReviewEvidence =
                    $"https://github.com/FS-GG/%s{registrationRepository}/pull/%d{implementationPr}#issuecomment-3"
                StructuredReviewEvidence =
                    $"https://github.com/FS-GG/%s{registrationRepository}/pull/%d{implementationPr}#issuecomment-2"
                ReviewCycleId = "GS2-07.3"
                ReviewReceipt = critique
                SddWorkId = sddWorkId
                SddObservations = observations
                Identities = identities
                ImplementationBinding =
                    binding identities.ImplementationCandidate identities.ImplementationMerge (String.replicate 40 "e")
                AcceptanceBinding =
                    binding identities.AcceptanceCandidate identities.AcceptanceMerge (String.replicate 40 "f")
    ...
    let private authorityRouteFixture () =
        ...
        authorityRouteFixtureFor
            ...
            2
            (String.replicate 40 "c")
            (String.replicate 40 "d")
            "500-roadmap-gs2-07-3"
            "GS2-07.3"
            candidate
    ...
    let private invokeAuthorityRouteFixtureUsingResponder
        ...
        =
        let directory =
            Path.Combine(Path.GetTempPath(), "fsgg-3210-authority-route-" + Guid.NewGuid().ToString("n"))
        ...
        try
            ...
            let baseOptions =
                Options.parse [ "intake"; "apply"; inputPath ] |> Result.defaultWith failwith
            ...
            try
                ...
                let code =
                    if productionAuthorities then
                        ...
                        Handlers.roadmapUnitAcceptWithObservers
                            runQualification
                            observeSdd
                            observeAuthorities
                            context
    ...
    let private invokeAuthorityRouteUsing productionAuthorities observeSdd observeAuthorities =
        invokeAuthorityRouteUsingResponder productionAuthorities observeSdd observeAuthorities (fun _ ->
            Error(
                NotFound(
                    if productionAuthorities then
                        "production authority observer reached"
                    else
                        "injected observers must prevent live transport"
                )
            ))
    ...
    let ``#3210 production composition keeps the live authority observer active`` () =
        let code, output, error =
            invokeAuthorityRouteUsing true (fun _ -> Ok()) (fun _ ->
                failwith "production authority observer was replaced")
    ...
    let ``#3247 production composition executes route and immutable envelope authority`` () =
        ...
        let response (request: Request) =
            ...
            | "repos/FS-GG/.github/issues/1/comments" ->
                ok
                    """[{"id":3,"html_url":"https://github.com/FS-GG/.github/pull/1#issuecomment-3","body":"envelope","created_at":"2026-09-05T06:02:00Z","updated_at":"2026-09-05T06:03:00Z"}]"""
        ...
        let code, output, error =
            invokeAuthorityRouteUsingResponder
                true
                (fun _ -> Ok())
                (fun _ -> failwith "production observer was replaced")
                response
    ...
    let ``#3242 live acceptance does not require ignored generated SDD files from the candidate`` () =
        let accepted, output, error = invokeAuthorityRoute (fun _ -> Ok()) (fun _ -> Ok())
        Assert.Equal(ExitGreen, accepted)
        Assert.Contains("fsgg.roadmap-unit.acceptance-bundle/1", output)
        Assert.Equal("", error)

        let refused, refusedOutput, refusedError =
            invokeAuthorityRoute (fun _ -> Error [ "independent SDD observation refused" ]) (fun _ -> Ok())
    ...
    let ``#3247 immutable acceptance envelope has its own post-create identity`` () =
        ...
        let comment: Reads.AuthorityComment =
            {
                Id = 3L
                Url = input.ReviewEvidence
                Body = body
                CreatedAt = "2026-09-05T06:02:00Z"
                UpdatedAt = "2026-09-05T06:02:00Z"
            }
        ...
        Assert.Contains(
            ...
            Handlers.validateAcceptanceEvidenceComment
                input
                { comment with
                    UpdatedAt = "2026-09-05T06:03:00Z"
                }
        )
    ...
    let ``#3247 acceptance SDD identity comes from current structured route authority`` () =
        ...
        let route: StructuredDecision.RouteRecord =
            {
                ...
                ReasonCodes = [ "public-contract" ]
                Rationale = "acceptance uses SDD"
                SddWorkId = Some input.SddWorkId
        ...
        let route =
            { route with
                Digest = StructuredDecision.routeDigest route
            }
        ...
        let json =
            JsonSerializer.Serialize
                {|
                    ...
                    previousDigest = route.PreviousDigest
                    scope = route.Scope
                    dependencies = route.Dependencies
                    touchSet = route.TouchSet
                    ...
                    sddWorkId = route.SddWorkId
                    specHome = route.SpecHome
        ...
        Assert.Contains(
            "acceptance SDD route authority is missing",
            Handlers.validateAcceptanceSddRoute unit.Issue input.SddWorkId []
        )
    ...
    let ``#3210 production immutable preparation observer has a green route and refuses revision drift`` () =
        ...
        let errors =
            ...
            |> function
                | Error values -> values
                | Ok() -> failwith "stale catalog revision reached the acceptance seal"
    ...
    let ``#3251 production critique route proves the real single-PR artifact-only ancestry`` () =
        ...
        let comparisonPath =
            $"repos/FS-GG/FS.GG.Coordination/compare/%s{reviewed}...%s{candidate}"
                    ...
                    Ok
                        {
                            Status = 200
                            Body =
                                $"""{{"status":"ahead","ahead_by":1,"merge_base_commit":{{"sha":"%s{reviewed}"}},"files":[{{"filename":"reviews/roadmap/%s{cycle}.json","status":"modified"}}]}}"""
                            ETag = None
                            NextLink = None
                            Headers = Map.empty
                        }
