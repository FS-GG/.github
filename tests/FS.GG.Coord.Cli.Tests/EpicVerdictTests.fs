...
module EpicVerdictTests =
    ...
    let private delegated =
        "- [ ] FS-GG/.github#1 do the thing\n- [ ] FS-GG/.github#2 the other"
    ...
    let ``EPIC-NO-CHILDREN fires on an open epic with zero sub-issues`` () =
        let codes =
            Client.epicVerdict IssueState.Open BoardStatus.Ready delegated (graph 0 []) []
            |> codesOf
    ...
    let ``EPIC-CHILDREN-TRUNCATED fires when the graph is short`` () =
        ...
        let codes =
            Client.epicVerdict IssueState.Open BoardStatus.Ready delegated g [] |> codesOf
    ...
    let ``EPIC-DONE-OPEN-CHILD fires when the board says Done but a child is open`` () =
        ...
        let codes =
            Client.epicVerdict IssueState.Open BoardStatus.Done delegated g [] |> codesOf
    ...
    let ``EPIC-UNDELEGATED-ACCEPTANCE fires on an acceptance line naming no child`` () =
        ...
        let codes =
            Client.epicVerdict IssueState.Open BoardStatus.Ready undelegatedBody g []
            |> codesOf
    ...
    let ``EPIC-NO-STATED-ACCEPTANCE fires on a body with no task lines`` () =
        ...
        let codes =
            Client.epicVerdict IssueState.Open BoardStatus.Ready noAcceptanceBody g []
            |> codesOf
    ...
    let ``EPIC-NO-STATED-ACCEPTANCE and EPIC-UNDELEGATED-ACCEPTANCE are mutually exclusive`` () =
        ...
        for body in bodies do
            let codes =
                Client.epicVerdict IssueState.Open BoardStatus.Ready body g [] |> codesOf
    ...
    let ``a CLOSED epic fires none of the open-only rules`` () =
        ...
        let codesEmpty =
            Client.epicVerdict IssueState.Closed BoardStatus.Done noAcceptanceBody (graph 0 []) []
            |> codesOf
        ...
        let codesUndelegated =
            Client.epicVerdict IssueState.Closed BoardStatus.Done undelegatedBody g []
            |> codesOf
    ...
    let ``EPIC-DONE-OPEN-CHILD does NOT fire when the board is not Done`` () =
        ...
        let codes =
            Client.epicVerdict IssueState.Open BoardStatus.InProgress delegated g []
            |> codesOf
