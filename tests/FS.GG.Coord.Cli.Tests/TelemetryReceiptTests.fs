...
module TelemetryReceiptTests =
    ...
    let private withStore action =
        let root =
            Path.Combine(Path.GetTempPath(), "fsgg-receipt-test-" + Guid.NewGuid().ToString("N"))
    ...
    let ``receipt canonical identity rejects ambiguity and ignores object ordering`` () =
        ...
        let reordered =
            text.Replace(
                "\"workspaceId\":\"workspace-a\",\"producerId\":\"producer-a\"",
                "\"producerId\":\"producer-a\",\"workspaceId\":\"workspace-a\""
            )
        ...
        for invalid in
            [
                text.Replace("\"revision\":0", "\"revision\":0,\"revision\":1")
                text.Replace("\"revision\":0", "\"revision\":-0")
                text.Replace("\"revision\":0", "\"revision\":0.0")
                text.Replace("\"batchId\":", "\"unknown\":true,\"batchId\":")
            ] do
            Assert.True(TelemetryReceipt.parse (Encoding.UTF8.GetBytes invalid) |> Result.isError)

        Assert.Equal(
            Error [ "unsupported-version" ],
            TelemetryReceipt.parse (Encoding.UTF8.GetBytes(text.Replace("envelope/1", "envelope/2")))
        )
    ...
    let ``receipt acceptance replay conflict and applied expiry preserve identity`` () =
        withStore (fun root ->
            ...
            Assert.Equal(
                ...
                TelemetryStoreApplication.lookupReceipt root approved { scope with Workspace = "other" } "batch-a"
            )

            Assert.Equal(
                Error [ "unauthorized-scope" ],
                TelemetryStoreApplication.submitReceipt root approved { scope with Workspace = "other" } bytes
            )
        ...
        withStore (fun root ->
            ...
            let hook current =
                if current = stage then
                    raise (IOException "injected storage fault")

            Assert.True(
                TelemetryStoreApplication.submitReceiptWithHook root approved scope bytes hook
                |> Result.isError
            )

            Assert.Equal(
                "durably-received",
                status (TelemetryStoreApplication.submitReceipt root approved scope bytes)
            )
    ...
    let ``receipt application and terminal outcome survive interrupted cleanup`` stage expected =
        withStore (fun root ->
            ...
            TelemetryStoreApplication.submitReceipt root approved scope bytes
            |> unwrap
            |> ignore
            ...
            Assert.True(
                TelemetryStoreApplication.drainReceiptsWithHook root approved scope.Workspace hook
                |> Result.isError
            )
    ...
    let ``receipt exhaustion admits retries without another obligation and fair drain serves peers`` () =
        withStore (fun root ->
            for n in 1..128 do
                TelemetryStoreApplication.submitReceipt root approved scope (envelope scope (string n) 0)
                |> unwrap
                |> ignore

            Assert.Equal(
                "durably-received",
                status (TelemetryStoreApplication.submitReceipt root approved scope (envelope scope "1" 0))
            )

            Assert.Equal(
                Error [ "overload" ],
                TelemetryStoreApplication.submitReceipt root approved scope (envelope scope "129" 0)
            )

            let peer = { scope with Producer = "producer-b" }

            TelemetryStoreApplication.enrollReceiptProducer root approved peer
            |> unwrap
            |> ignore

            TelemetryStoreApplication.submitReceipt root approved peer (envelope peer "peer" 0)
            |> unwrap
            |> ignore
    ...
    let ``receipt semantic conflict is terminal and missing accepted input refuses recovery`` () =
        withStore (fun root ->
            ...
            TelemetryStoreApplication.submitReceipt root approved scope initial
            |> unwrap
            |> ignore
            ...
            let changed =
                Encoding.UTF8.GetString(envelope scope "second" 0)
                |> fun s ->
                    s.Replace("\"itemId\":\"item-a\"", "\"itemId\":\"item-b\"")
                    |> Encoding.UTF8.GetBytes

            TelemetryStoreApplication.submitReceipt root approved scope changed
            |> unwrap
            |> ignore
            ...
            TelemetryStoreApplication.submitReceipt root approved scope (envelope scope "third" 1)
            |> unwrap
            |> ignore

            let file =
                Directory.GetFiles(Path.Combine(root, "receipt-inbox"), "*.ready")
                |> Array.exactlyOne
            ...
            Assert.Equal(
                ...
                TelemetryStoreApplication.lookupReceipt root approved scope "third"
            )
    ...
    let ``receipt workspace association cannot move and producer keys are independent`` () =
        withStore (fun root ->
            Assert.Equal(
                Error [ "unauthorized-scope" ],
                TelemetryStoreApplication.enrollReceiptProducer root approved { scope with Workspace = "other" }
            )
            ...
            for who in [ scope; peer ] do
                TelemetryStoreApplication.submitReceipt root approved who (envelope who "same-batch" 0)
                |> unwrap
                |> ignore
            ...
            for who in [ scope; peer ] do
                Assert.Equal(
                    "applied",
                    status (TelemetryStoreApplication.lookupReceipt root approved who "same-batch")
                ))

    [<Fact>]
    let ``receipt corrupt pending artifact cannot acknowledge a processing obligation`` () =
        withStore (fun root ->
            TelemetryStoreApplication.submitReceipt root approved scope (envelope scope "corrupt" 0)
            |> unwrap
            |> ignore
            ...
            Assert.Equal(
                Error [ "storage-unavailable" ],
                TelemetryStoreApplication.lookupReceipt root approved scope "corrupt"
            )

            Assert.Equal(
                Error [ "storage-unavailable" ],
                TelemetryStoreApplication.submitReceipt root approved scope (envelope scope "next" 0)
            ))

    [<Fact>]
    let ``receipt recovery refuses a missing artifact even when enrollment is also corrupt`` () =
        withStore (fun root ->
            TelemetryStoreApplication.submitReceipt root approved scope (envelope scope "missing" 0)
            |> unwrap
            |> ignore
            ...
            Assert.Equal(
                Error [ "storage-unavailable" ],
                TelemetryStoreApplication.recoverReceiptCapacity root approved
            ))

    [<Fact>]
    let ``receipt recovery refuses pending content after workspace metadata corruption`` () =
        withStore (fun root ->
            TelemetryStoreApplication.submitReceipt root approved scope (envelope scope "workspace-corrupt" 0)
            |> unwrap
            |> ignore
    ...
    let ``receipt migration preserves legacy history without assigning it`` () =
        ...
        try
            ...
            TelemetryStoreApplication.ingest
                root
                approved
                (Encoding.UTF8.GetBytes(document.RootElement.GetProperty("payload").GetRawText()))
            |> unwrap
            |> ignore
        ...
        withStore (fun root ->
            ...
            let references =
                [
                    ...
                    "FS.GG.Coord.Core.dll"
                    "FS.GG.Telemetry.Store.dll"
                ]
                |> List.map (fun name -> "#r @\"" + Path.Combine(AppContext.BaseDirectory, name) + "\"")

            let call =
                if draining then
                    "TelemetryStoreApplication.drainReceiptsWithHook root TelemetryStore.ApprovedLocalDurable scope.Workspace hook"
                else
                    "TelemetryStoreApplication.submitReceiptWithHook root TelemetryStore.ApprovedLocalDurable scope (File.ReadAllBytes input) hook"
            ...
            File.WriteAllLines(
                ...
                @ [
                    ...
                    "let input = fsi.CommandLineArgs[2]"
                    "let stage = fsi.CommandLineArgs[3]"
            ...
            [ "fsi"; "--exec"; script; root; input; stage ]
            ...
            if not (child.WaitForExit(30000)) then
                child.Kill(true)
                failwith "crash fixture timed out"
