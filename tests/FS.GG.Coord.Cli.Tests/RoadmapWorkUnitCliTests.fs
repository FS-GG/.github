...
module RoadmapWorkUnitCliTests =
    ...
    let private fixture () =
        let directory =
            Path.Combine(Path.GetTempPath(), "fsgg-3210-cli-" + Guid.NewGuid().ToString("n"))
        ...
        let unit id title prerequisites qGates commands =
            let unsigned =
                $"""{{"exitGate":"test","gateCommands":%s{commands},"id":"%s{id}","owner":"FS.GG.Coordination","permissionCeiling":["test"],"prerequisites":%s{prerequisites},"qGates":%s{qGates},"title":"%s{title}"}}"""
            ...
            + $",\"contractSha256\":\"%s{contractDigest}\"}}"
        ...
        let inputPath, roadmapPath, catalogPath =
            Path.Combine(directory, "input.json"),
            Path.Combine(directory, "roadmap.md"),
            Path.Combine(directory, "catalog.json")

        let sourcePath, candidatePath =
            Path.Combine(directory, "registry.md"), Path.Combine(directory, "candidate.md")
        ...
        File.WriteAllText(sourcePath, registry, UTF8Encoding(false))
    ...
    let private acceptanceInput (plan: RoadmapWorkUnit.PreparationPlan) =
        ...
        let applied =
            ...
            |> List.mapi (fun index registration ->
                ...
                ({
                    ...
                    Issue = $"FS-GG/.github#%d{number}"
                    IssueUrl = $"https://github.com/FS-GG/.github/issues/%d{number}"
                }
        ...
        let observation stage status : RoadmapWorkUnit.SddObservation =
            let stageFields =
                if stage = "ship" then
                    ...
                    """"sources":[{"path":"readiness/500-roadmap-gs2-07-3/work-model.json"}],"findings":[],"diagnostics":[],"""

            {
                Stage = stage
                SubjectRevision = candidate
                ArtifactJson =
                    $"""{{"schemaVersion":1,"viewVersion":"1.0","generator":"FS.GG.SDD.Artifacts/1.5.0",%s{stageFields}"stage":"%s{stage}","status":"%s{status}","readiness":"%s{status}","workId":"500-roadmap-gs2-07-3"}}"""
            }
...

        {
            ...
            LifecycleLog = lifecycle candidate
            ...
            SddWorkId = "500-roadmap-gs2-07-3"
            SddObservations =
                [
    ...
    let ``#3210 prepare inspect render verify is a deterministic CLI round trip`` () =
        ...
        let inspectCode, plan, inspectError =
            invoke ([ "roadmap"; "unit"; "prepare"; "inspect" ] @ common)
        ...
        let renderCode, _, renderError =
            invoke (
                [ "roadmap"; "unit"; "prepare"; "render" ]
                @ common
                @ [ "--registry"; source; "--output"; candidate ]
            )

        Assert.Equal(ExitCode.toInt ExitCode.Green, renderCode)
        Assert.Equal("", renderError)

        let verifyCode, stdout, verifyError =
            invoke (
                [ "roadmap"; "unit"; "prepare"; "verify" ]
                @ common
                @ [ "--source-registry"; source; "--registry"; candidate ]
            )

        Assert.Equal(ExitCode.toInt ExitCode.Green, verifyCode)
        Assert.Equal("", verifyError)
    ...
    let ``#3210 pure acceptance actions emit only an internally coherent candidate`` () =
        ...
        let inputPath, candidatePath =
            Path.Combine(directory, "acceptance.json"), Path.Combine(directory, "candidate.json")

        File.WriteAllText(inputPath, RoadmapWorkUnit.canonicalAcceptanceInput input, UTF8Encoding(false))

        let inspectCode, stdout, inspectError =
            invoke [ "roadmap"; "unit"; "accept"; "inspect"; "--input"; inputPath ]
        ...
        let verifyCode, verified, verifyError =
            invoke
                [
                    "roadmap"
                    "unit"
    ...
    let ``#3210 prepare refuses unrecognized arguments before file IO`` () =
        let code, stdout, stderr =
            invoke [ "roadmap"; "unit"; "prepare"; "inspect"; "--input"; "missing"; "--invented" ]
    ...
    let ``#3210 revision inspect emits a sealed binding from observed Git identities`` () =
        ...
        let binding =
            RoadmapWorkUnit.parseRevisionBinding (Encoding.UTF8.GetBytes stdout) |> unwrap
