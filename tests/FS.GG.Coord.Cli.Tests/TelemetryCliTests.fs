...
module TelemetryCliTests =
    ...
    let private temporary (content: string) =
        let path =
            Path.Combine(Path.GetTempPath(), "fsgg-3208-" + Guid.NewGuid().ToString("n"))
    ...
    let private shaText (content: string) =
    ...
    let private command (workingDirectory: string) (executable: string) (arguments: string list) =
        ...
        let stdout, stderr =
            childProcess.StandardOutput.ReadToEnd(), childProcess.StandardError.ReadToEnd()

        childProcess.WaitForExit()

        if childProcess.ExitCode <> 0 then
            failwithf "%s %A failed: %s" executable arguments stderr
    ...
    let private roadmapEvidence digest =
        let directory =
            Path.Combine(Path.GetTempPath(), "fsgg-3208-evidence-" + Guid.NewGuid().ToString("n"))

        Directory.CreateDirectory directory |> ignore

        let write (name: string) (content: string) =
            File.WriteAllText(Path.Combine(directory, name), content, UTF8Encoding(false))
        ...
        let audit =
            $"""{{"auditSchema":1,"report":"report.md","reportSha256":"%s{shaText report}","findings":[]}}"""
        ...
        write
            "accepted.json"
            (sealedReceipt
                $"""{{"acceptedAt":"2026-01-01T00:00:00Z","artifacts":[{{"name":"implementation-candidate-%s{candidate}","sha256":"%s{filler}"}}],"schema":"fsgg.coordination.unit-acceptance/1","sourceRevision":"%s{candidate}","state":"accepted","unitContractSha256":"%s{filler}","unitId":"GS2-01.1"}}""")
        ...
        write
            "feedback.json"
            (sealedReceipt
                $"""{{"auditSha256":"%s{shaText audit}","cycleId":"cycle-1","head":"%s{acceptance}","reportSha256":"%s{shaText report}","schema":"fsgg.roadmap.feedback-binding/1","unitId":"GS2-01.1"}}""")
    ...
    let private codex total =
        let counts =
            $"""{{"input_tokens":10,"cached_input_tokens":4,"cache_write_input_tokens":0,"output_tokens":5,"reasoning_output_tokens":2,"total_tokens":%d{total}}}"""
    ...
    let private qualificationInput checkoutClean =
        ...
        let operation id kind exitCode refusal replay =
            {|
                ...
                tool = tool
                executor = executor
                commandSha256 = digest 'c'
                artifactSha256 = [ digest 'd' ]
                resultSha256 = digest 'e'
            ...
            {|
                ...
                claims =
                    [
                        {|
                            id = "all"
                            subjectRevision = revision
                            requiredKinds = [ "analyze"; "verify"; "ship"; "hosted"; "fixed-point" ]
                ...
                mutations =
                    [
                        {|
                            ...
                            productionImplementationSha256 = digest 'b'
                            fixtureImplementationSha256 = digest 'f'
                ...
                semanticReview =
                    {|
                        subjectRevision = revision
                        accepted = true
                        evidence = "https://github.com/FS-GG/.github/pull/1#issuecomment-1"
                    |}
            |}

    let private qualificationRunFixture () =
        let directory =
            Path.Combine(Path.GetTempPath(), "fsgg-3209-run-" + Guid.NewGuid().ToString("n"))
        ...
        let fixtureTool =
            {|
                id = "fixture"
                version = "fixture-tool 1.0"
                sha256 = shaFile toolPath
            |}

        let gitTool =
            {|
                id = "git"
                version = command directory "/usr/bin/git" [ "--version" ]
                sha256 = shaFile "/usr/bin/git"
            |}
        ...
        let input =
            JsonSerializer.Serialize
                {|
                    ...
                    subjectRevision = revision
                    checkoutClean = true
                    toolManifest = [ fixtureTool; gitTool ]
                    executor = executor
                    operations =
                        [
                            operation "analyze" "analyze" 0 None None
        ...
        let hostedPath1, hostedPath2, obligationPath =
            "evidence/hosted-1.json", "evidence/hosted-2.json", "evidence/obligation.txt"

        let hostedJson =
            JsonSerializer.Serialize
                {|
                    schema = QualificationEvidence.HostedSchema
                    complete = true
                    items =
                        [
                            {|
        ...
        let obligationComment =
            QualificationEvidence.renderObligationComment revision Qualification.NoObligations
        ...
        let execution =
            JsonSerializer.Serialize
                {|
                    schema = "fsgg.qualification.execution/1"
                    checkout = directory
                    timeoutSeconds = 10
                    ...
                    fixtures =
                        [
                            {|
                                mutationId = "wrong-subject"
                                executorId = "fixture-executor"
                                executorRole = "mutation-fixture"
                                path = fixturePath
        ...
        let inputPath, executionPath =
            Path.Combine(directory, "input.json"), Path.Combine(directory, "execution.json")
    ...
    let ``#3259 collector durably archives and resolver returns the exact receipt`` () =
        ...
        let resolveCode, resolved, resolveError =
            invoke
                [
                    "telemetry"
                    "usage"
        ...
        let unsafeCode, unsafeOutput, unsafeError =
            invoke
                [
                    "telemetry"
                    "usage"
    ...
    let ``#3208 CSV append preserves one header and deduplicates response identity`` () =
        ...
        let outputDirectory =
            Path.Combine(Path.GetTempPath(), "fsgg-3208-append-" + Guid.NewGuid().ToString("n"))
        ...
        use _output =
            { new IDisposable with
                member _.Dispose() = Directory.Delete(outputDirectory, true)
            }
    ...
    let ``#3210 require-reconciled is an active exact-digest lifecycle gate`` () =
        ...
        let started =
            LifecycleTelemetry.sealSuccessor
                ...
                "unit"
                ""
                (draft "started" "2026-09-04T08:00:00Z" "null" "{\"status\":\"pending\"}")
        ...
        let completed =
            LifecycleTelemetry.sealSuccessor
                ...
                (draft
                    ...
                    "1"
                    "{\"status\":\"unavailable\",\"reason\":\"final usage is written after this response\",\"source\":\"legacy\"}")
            |> Result.defaultWith (sprintf "%A" >> failwith)

        let disposable, path = temporary (started + completed)
        use _ = disposable

        let ordinary, _, ordinaryError =
            invoke
                [
                    "telemetry"
                    "lifecycle"
    ...
    let ``#3208 roadmap render is bounded deterministic and does not mutate its input`` () =
        ...
        let digest =
            "sha256:"
            + (SHA256.HashData(Encoding.UTF8.GetBytes roadmap)
               |> Convert.ToHexString
               |> _.ToLowerInvariant())
    ...
    let ``#3209 qualification CLI rejects dirty checkout and unknown fields`` () =
        ...
        let dirtyCode, dirtyOutput, dirtyError =
            invoke [ "telemetry"; "qualification"; "validate"; "--input"; dirtyPath ]
        ...
        let unknown =
            (qualificationInput true).Replace("\"schema\":", "\"unknown\":true,\"schema\":")

        let unknownDisposable, unknownPath = temporary unknown
        use _unknown = unknownDisposable

        let unknownCode, unknownOutput, unknownError =
            invoke [ "telemetry"; "qualification"; "validate"; "--input"; unknownPath ]
    ...
    let ``#3209 qualification command contract rejects loose argv`` () =
        for args, expected in
            [
                ...
                [ "telemetry"; "qualification"; "validate"; "loose" ], "unexpected positional argument 'loose'"
            ] do
            let code, stdout, stderr = invoke args
            Assert.NotEqual(0, code)
            Assert.Equal("", stdout)
    ...
    let ``#3209 qualification runner executes exact clean checkout and fixed point`` () =
        ...
        Directory.CreateDirectory(Path.Combine(Path.GetDirectoryName(inputPath), "evidence"))
        |> ignore
    ...
    let ``#3210 production qualification pins trusted git tree and environment`` () =
        ...
        Assert.True(
            QualificationApplication.runBoundToTree tree inputPath executionPath
            |> Result.isOk
        )

        Assert.True(
            QualificationApplication.runBoundToTree (String.replicate 40 "0") inputPath executionPath
            |> Result.isError
        )
        ...
        command checkout "/usr/bin/git" [ "update-index"; "--skip-worktree"; "tracked-verdict.txt" ]
        |> ignore
        ...
        command checkout "/usr/bin/git" [ "update-index"; "--no-skip-worktree"; "tracked-verdict.txt" ]
        |> ignore

        let fakeGitPath =
            JsonSerializer.Serialize(Path.Combine(checkout, "qualification-tool.sh"))

        File.WriteAllText(
            executionPath,
            execution.Replace("\"id\":\"git\",\"path\":\"/usr/bin/git\"", $"\"id\":\"git\",\"path\":%s{fakeGitPath}")
        )
    ...
    let ``#3209 qualification runner requires two distinct hosted observation artifacts`` () =
        ...
        File.WriteAllText(
            executionPath,
            execution.Replace(
                "\"hostedObservationPaths\":[\"evidence/hosted-1.json\",\"evidence/hosted-2.json\"]",
                "\"hostedObservationPaths\":[\"evidence/hosted-1.json\",\"evidence/hosted-1.json\"]"
            )
        )
    ...
    let ``#3209 qualification runner independently executes exact mutation fixture refusal`` () =
        ...
        let fixturePath =
            Path.Combine(Path.GetDirectoryName(inputPath), "mutation-fixture.sh")
    ...
    let ``#3209 qualification runner requires a distinct mutation fixture role`` () =
        ...
        File.WriteAllText(
            executionPath,
            execution.Replace("\"executorRole\":\"mutation-fixture\"", "\"executorRole\":\"implementer\"")
        )
    ...
    let ``#3209 qualification runner refuses non-authoritative obligation readback receipts`` () =
        ...
        File.WriteAllText(
            executionPath,
            execution.Replace("https://github.com/FS-GG/.github/pull/3221#issuecomment-123", "file:///tmp/asserted")
        )
    ...
    let ``#3209 fixed point refuses replay that changes only artifact bytes`` () =
        ...
        File.WriteAllText(
            executionPath,
            execution.Replace("[\"fixed\",\"evidence/fixed.txt\"]", "[\"fixed-unstable\",\"evidence/fixed.txt\"]")
        )
