...
40 cost %d{large.RestCalls}. That is .github#2382 — the per-row read is back."
    ...
    let private notDriven: (Options.Command * string) list =
        [ Options.Decide,
          ...
          Options.Issues,
          "`Client.issues` is a compatibility seam over the family-owned handler; audited by reading — stdout is the raw REST body (`[]` on a repo with no issues), and BOTH its refusal arms are stderr at a non-zero code: the missing-repo refusal and the read failure, the latter through `fail` so a rate limit keeps EX_RATE"
          Options.IntakeCmd,
          "`Client.intakeCmd` is a compatibility seam over the family-owned handler and is audited by transaction tests. Its validate arm emits one typed zero-write receipt, while apply emits one receipt-bound issue/projection result; malformed drafts and unreadable receipts fail on stderr before a POST."
          Options.RouteCmd,
