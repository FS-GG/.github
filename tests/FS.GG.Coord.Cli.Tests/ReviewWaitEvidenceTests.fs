...
let private record kind critic digest : StructuredDecision.ReviewRecord =
    {
        ...
        Critic = critic
        Verdict =
            if kind = StructuredDecision.RepairPhase then
...
let private candidates =
    [
        ({
            Id = 20L
            Url = "https://example.test/reviews/20"
            Body = "repair phase"
        }
        : Reads.CommentBody),
        ...
        ({
            Id = 21L
            Url = "https://example.test/reviews/21"
            Body = "confirmation"
        }
        : Reads.CommentBody),
...
let ``exact evidence selects confirmation when repair phase shares its generation`` evidence =
    let actual =
        LiveHandlers.selectCompletionEvidence
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa:repair-confirmation:1"
            evidence
            candidates
