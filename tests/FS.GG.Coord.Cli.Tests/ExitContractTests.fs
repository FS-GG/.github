...
module ExitContractTests =
    ...
    let ``the documented EX_* codes are the literals the engine returns`` () =
        Assert.Equal(Some Kernel.ExitNone, codeFor "EX_NONE")
        Assert.Equal(Some Kernel.ExitContended, codeFor "EX_CONTENDED")
    ...
    let ``the documented success and read-failure codes are the engine's`` () =
        Assert.True(documented Kernel.ExitGreen, "0 (claimed) is not documented")
        Assert.True(documented Kernel.ExitError, "1 (could not read the board) is not documented")
    ...
    /// and `Kernel.ExitNoVerdict` were BOTH 4 (the collision #918 has since fixed — they are now 9 and 4).
    ...
    let ``every module's exit constant is the union's number`` () =
        Assert.Equal(ExitCode.toInt ExitCode.Green, Kernel.ExitGreen)
        Assert.Equal(ExitCode.toInt ExitCode.Error, Kernel.ExitError)
        Assert.Equal(ExitCode.toInt ExitCode.Red, Kernel.ExitRed)
        Assert.Equal(ExitCode.toInt ExitCode.NoVerdict, Kernel.ExitNoVerdict)
        Assert.Equal(ExitCode.toInt ExitCode.NoneStartable, Kernel.ExitNone)
        Assert.Equal(ExitCode.toInt ExitCode.Contended, Kernel.ExitContended)
        Assert.Equal(ExitCode.toInt ExitCode.Pending, Kernel.ExitPending)
        Assert.Equal(ExitCode.toInt ExitCode.NotOpen, Kernel.ExitNotOpen)
    ...
    /// #918 — AND THE COLLISION IS GONE. `Errors.ExOffboard` was `3` (colliding with `Kernel.ExitRed`)
    /// and `Errors.ExPartial` `4` (colliding with `Kernel.ExitNoVerdict`). Distinct meanings — off-board
    ...
    let ``the github-layer codes no longer collide with the verdict codes`` () =
        Assert.NotEqual(Errors.ExOffboard, Kernel.ExitRed)
        Assert.NotEqual(Errors.ExPartial, Kernel.ExitNoVerdict)
    ...
    // nothing tied `Code = 7` to `Kernel.ExitPending`. Retype `ExitPending = 8` and every gate stayed
    ...
    // `Kernel.ExitPending`/`ExitRed`". Until this block, it did not. A comment claiming a gate that does
    ...
    // bearing twice over, because `Kernel.ExitRed`/`Errors.ExOffboard` were BOTH 3 and
    // `Kernel.ExitNoVerdict`/`Errors.ExPartial` BOTH 4, so a digit could not tell a verdict from a
    ...
    /// `Kernel.ExitPending`'s VALUE is the row that says PENDING — so the two halves compose into
    ...
    let ``landable's wait code and its stop code are pinned to the engine's constants`` () =
        match landableMeaningOf Kernel.ExitPending with
        | None ->
            Assert.Fail
                "Kernel.ExitPending names no documented landable row — the ONE retryable verdict is undocumented, so a poll loop reads it as an unrecognised failure and stops waiting on a PR that is merely still running (#900)"
        | Some m ->
            Assert.True(
    ...
    let ``landable's pending code is distinct from every code that means stop`` () =
        Assert.NotEqual(Kernel.ExitPending, Kernel.ExitGreen)
        Assert.NotEqual(Kernel.ExitPending, Kernel.ExitRed)
        Assert.NotEqual(Kernel.ExitPending, Kernel.ExitNoVerdict)
        Assert.NotEqual(Kernel.ExitPending, Kernel.ExitError)
    ...
    let ``#1680 landable's not-open code is pinned to the engine's constant, and is not the wait code`` () =
        match landableMeaningOf Kernel.ExitNotOpen with
        | None ->
            Assert.Fail
                "Kernel.ExitNotOpen names no documented landable row — a merged PR's exit code is undocumented, so a recovery path reads it as an unrecognised failure"
        | Some m ->
            Assert.True(
                m.StartsWith "MERGED",
        ...
        Assert.NotEqual(Kernel.ExitNotOpen, Kernel.ExitPending)
        // Nor through the failure code — `merged` is a success whose next act is to STAMP.
        Assert.NotEqual(Kernel.ExitNotOpen, Kernel.ExitRed)
        Assert.NotEqual(Kernel.ExitNotOpen, Kernel.ExitGreen)
