...
module TelemetryRuntimeApplicationTests =
    ...
    let private assignment producer : TelemetryRuntime.Assignment =
        {
            ...
            ItemId = "UTEL-03A"
            AttemptId = producer
            ParentAttemptId = Some "parent-1"
            ProducerStream = producer
        }

    let private temp () =
        let path =
            Path.Combine(Path.GetTempPath(), "fsgg-utel03a-" + Guid.NewGuid().ToString("N"))
    ...
    let private batchFacts bytes =
        TelemetryStore.parseBatch bytes |> unwrap |> _.Facts
    ...
    let private observed executable parent relation exitCode =
        ...
        let actual =
            TelemetryRuntimeApplication.runObservedCodexExecWith
                executable
                (assignment "worker-observer")
                parent
                relation
                (Some "/durable/private/fsgg-telemetry")
                60L
                None
                [
                    "--json"
    ...
    let ``UTEL-03A assignment is closed and projector discards content-bearing fields`` () =
        let valid =
            Encoding.UTF8.GetBytes
                """{"schema":"fsgg.telemetry.codex-assignment/1","featureId":"UTEL-03A","itemId":"UTEL-03A","attemptId":"a1","parentAttemptId":null,"producerStream":"worker-1"}"""

        Assert.True(TelemetryRuntime.parseAssignment valid |> Result.isOk)

        Assert.True(
            TelemetryRuntime.parseAssignment (
                Encoding.UTF8.GetBytes
                    """{"schema":"fsgg.telemetry.codex-assignment/1","featureId":"UTEL-03A","itemId":"UTEL-03A","attemptId":"a1","producerStream":"worker-1","task":"PRIVATE-SENTINEL"}"""
            )
            |> Result.isError
        )

        Assert.True(
            TelemetryRuntime.projectLine
                (Some "thread-1")
                1L
                """{"type":"item.completed","item":{"text":"PRIVATE-SENTINEL","command":"PRIVATE-COMMAND","path":"/private/path"}}"""
            |> Option.isNone
        )
        ...
        match
            TelemetryRuntime.projectLine
                (Some "thread-1")
                2L
                """{"type":"turn.completed","turn_id":"turn-2","model":"observed","effort":"high","usage":{"input_tokens":5,"cached_input_tokens":1,"output_tokens":2,"reasoning_output_tokens":1}}"""
        with
        | Some(TelemetryRuntime.TurnUsageCompleted usage) ->
        ...
        Assert.Equal(
            Some(TelemetryRuntime.TurnStarted("thread-1", Some "turn-3", 3L)),
            TelemetryRuntime.projectLine (Some "thread-1") 3L """{"type":"turn.started","turn_id":"turn-3"}"""
        )
        ...
        if not (OperatingSystem.IsWindows()) then
            ...
            let success =
                script
                    ...
                    [
                        ...
                        "{\"type\":\"item.completed\",\"item\":{\"text\":\"PRIVATE-SENTINEL\",\"path\":\"/private/path\"}}"
                        "{\"type\":\"turn.started\",\"turn_id\":\"native-turn\"}"
                        "{\"type\":\"turn.completed\",\"turn_id\":\"native-turn\",\"usage\":{\"input_tokens\":10,\"cached_input_tokens\":2,\"output_tokens\":5}}"
                    ]
                    0
            ...
            let failure =
                script
                    ...
                    [
                        "{\"type\":\"thread.started\",\"thread_id\":\"native-thread\"}"
                        "{\"type\":\"error\",\"message\":\"discarded-runtime-error\"}"
                    ]
                    7

            let publish bytes =
                TelemetryStoreApplication.publish root approved bytes

            Assert.Equal(
                0,
                TelemetryRuntimeApplication.runCodexExecWith
                    success
                    (assignment "worker-1")
                    [ "--json"; "--ephemeral"; "synthetic" ]
                    publish
            )
            ...
            let privateFiles =
                ...
                |> Seq.filter (fun path -> not (path.EndsWith(".sqlite3-wal") || path.EndsWith(".sqlite3-shm")))
            ...
            for path in privateFiles do
                ...
                if
                    bytes
                    |> Array.forall (fun value -> value = 9uy || value = 10uy || value = 13uy || value >= 32uy)
                then
                    Assert.DoesNotContain("PRIVATE-SENTINEL", Encoding.UTF8.GetString bytes)

            let exported = Path.Combine(runtimeRoot, "public.json")

            TelemetryStoreApplication.exportPublic root approved (Some "UTEL-03A") exported
            |> unwrap
            |> ignore
    ...
    let ``UTEL-03A native exit and output survive total publication failure`` () =
        if not (OperatingSystem.IsWindows()) then
            ...
            let failing =
                script root "fake-codex" [ "{\"type\":\"thread.started\",\"thread_id\":\"late-thread\"}" ] 9

            let code =
                TelemetryRuntimeApplication.runCodexExecWith
                    failing
                    (assignment "worker-fail")
                    [ "--json"; "--ephemeral" ]
                    (fun _ -> Error [ "offline" ])
    ...
    let ``workspace binding is inherited by the launched child`` () =
        if OperatingSystem.IsLinux() then
            ...
            let code =
                TelemetryRuntimeApplication.runObservedCodexExecWith
                    executable
                    (assignment "worker-binding")
                    None
                    TelemetryRuntime.Root
                    None
                    60L
                    (Some("/private/config.json", "FS-GG/.github", String('a', 64)))
                    [ "--json"; "--ephemeral" ]
                    (fun _ -> Error [ "offline" ])
    ...
    let ``UTEL-03A bounded publisher records exhaustion without delaying child`` () =
        if not (OperatingSystem.IsWindows()) then
            ...
            let facts =
                published
                |> Seq.collect (fun bytes -> (TelemetryStore.parseBatch bytes |> unwrap).Facts)
                |> Seq.toList
            ...
            Assert.Contains(
                ...
                function
                | {
                      Payload = TelemetryStore.RuntimeGap(_, "publisher-queue-exhausted")
                  } -> true
                | _ -> false
            )
    ...
    let ``UTEL-03A assignment file must be bounded private absolute and non-symlinked`` () =
        if not (OperatingSystem.IsWindows()) then
            ...
            Assert.Equal(
                2,
                TelemetryRuntimeApplication.runCodexExec [ "--assignment"; path; "--"; "--json"; "--ephemeral" ]
            )
            ...
            Assert.Equal(
                2,
                TelemetryRuntimeApplication.runCodexExec [ "--assignment"; link; "--"; "--json"; "--ephemeral" ]
            )
            ...
            Assert.Equal(
                2,
                TelemetryRuntimeApplication.runCodexExec [ "--assignment"; oversized; "--"; "--json"; "--ephemeral" ]
            )

    [<Fact>]
    let ``UTEL-03A runtime batches replay and conflicting native usage stays atomic`` () =
        let eventJson identity input =
            $"""{{"schema":"%s{TelemetryStore.BatchSchema}","ingestId":"batch-{input}","sourceIdentity":"worker-1","generation":"g1","cursor":"{input}","eventCount":1,"events":[{{"kind":"runtime-turn-usage","identity":"{identity}","itemId":"UTEL-03A","revision":0,"invocationId":"inv-1","threadId":"thread-1","turnId":null,"turnSequence":1,"provider":null,"requestedModel":null,"observedModel":null,"requestedEffort":null,"observedEffort":null,"backend":null,"scope":"completed-turn","provenance":"codex-exec-jsonl","input":{input},"cachedInput":0,"output":1,"reasoning":null,"total":{input + 1}}}]}}"""
            |> Encoding.UTF8.GetBytes
        ...
        if not (OperatingSystem.IsWindows()) then
            ...
            let executable name delay exitCode =
                ...
                let content =
                    ...
                    + (sprintf
                        "printf '%%s' \"$%s\" > '%s'\n"
                        TelemetryRuntime.InvocationContextEnvironment
                        (contextPath name))
                    + (sprintf "printf '%%s\\n' \"$@\" > '%s'\n" (argumentPath name))
                    + (sprintf "pwd > '%s'\n" (workingPath name))
            ...
            let parseContext name =
                File.ReadAllBytes(contextPath name)
                |> TelemetryRuntime.parseInvocationContext
                |> unwrap
            ...
            observed childExecutable (Some rootContext) TelemetryRuntime.Child 0
            |> List.iter facts.Add

            let childContext = parseContext "child"
            let grandchildExecutable = executable "grandchild" false 0

            observed grandchildExecutable (Some childContext) TelemetryRuntime.Child 0
            |> List.iter facts.Add

            let grandchildContext = parseContext "grandchild"
            let followExecutable = executable "follow" false 0

            observed followExecutable (Some childContext) TelemetryRuntime.FollowUp 0
            |> List.iter facts.Add

            let followContext = parseContext "follow"
            let retryExecutable = executable "retry" false 0

            observed retryExecutable (Some rootContext) TelemetryRuntime.Child 0
            |> List.iter facts.Add
            ...
            let activations =
                ...
                |> Seq.choose (fun fact ->
                    ...
                    | TelemetryStore.OperationalActivation _ -> Some fact
                    | _ -> None)
                |> Seq.length
            ...
            let dispatches =
                ...
                |> Seq.choose (fun fact ->
                    match fact.Payload with
                    | TelemetryStore.ExpectedDispatch(a, b, c, d, e, f, g) -> Some(a, b, c, d, e, f, g)
                    | _ -> None)
                |> Seq.toList
            ...
            let lineages =
                ...
                |> Seq.choose (fun fact ->
                    match fact.Payload with
                    | TelemetryStore.InvocationLineage(a, b, c, d, e, f) -> Some(a, b, c, d, e, f)
                    | _ -> None)
                |> Seq.toList
            ...
            let admissions =
                ...
                |> Seq.filter (fun fact ->
                    match fact.Payload with
                    | TelemetryStore.RuntimeAdmission _ -> true
                    | _ -> false)
                |> Seq.length
            ...
            let eventTimes =
                ...
                |> Seq.choose (fun fact ->
                    match fact.Payload with
                    | TelemetryStore.EventTime(_, event, _, _, _, _) -> Some event
                    | _ -> None)
                |> Seq.toList
            ...
            Assert.Contains(
                dispatches,
                fun (_, _, relation, parent, _, _, _) -> relation = "follow-up" && parent = Some childContext.DispatchId
            )

            Assert.Contains(
                lineages,
                fun (_, invocation, relation, parent, rootInvocation, _) ->
                    invocation = grandchildContext.InvocationId
                    && relation = "child"
                    && parent = Some childContext.InvocationId
                    && rootInvocation = rootContext.InvocationId
            )
            ...
            Assert.Contains(
                ...
                function
                | {
                      Payload = TelemetryStore.RuntimeAdmission(_, _, _, _, _, Some "gpt-test", Some "high", _)
                  } -> true
                | _ -> false
            )
    ...
    let ``UTEL-06_2 launch failure cancellation and no-op stay native while facts remain machine authored`` () =
        if not (OperatingSystem.IsWindows()) then
            ...
            let failedFacts =
                observed (Path.Combine(root, "missing-codex")) None TelemetryRuntime.Root 127

            Assert.Contains(
                noOpFacts,
                function
                | {
                      Payload = TelemetryStore.RuntimeGap(_, "exit-zero-without-usage")
                  } -> true
                | _ -> false
            )
            ...
            Assert.Contains(
                ...
                function
                | {
                      Payload = TelemetryStore.RuntimeTerminal(_, _, "cancelled", 130L)
                  } -> true
                | _ -> false
            )
            ...
            Assert.Contains(
                ...
                function
                | {
                      Payload = TelemetryStore.RuntimeTerminal(_, _, "launch-failed", 127L)
                  } -> true
                | _ -> false
            )
            ...
            Assert.DoesNotContain(
                ...
                function
                | {
                      Payload = TelemetryStore.EventTime(_, "start", _, _, _, _)
                  } -> true
                | _ -> false
            )
            ...
            Assert.Contains(
                ...
                function
                | {
                      Payload = TelemetryStore.EventTime(_, "terminal", _, _, _, _)
                  } -> true
                | _ -> false
            )
    ...
    let ``UTEL-06_2 executable launcher facts ingest through migration five and reconcile without authored batches``
        ...
        if not (OperatingSystem.IsWindows()) then
            ...
            let executable =
                script
                    runtimeRoot
                    "fake-codex-reconcile"
                    [
                        "{\"type\":\"thread.started\",\"thread_id\":\"native-thread\"}"
                        "{\"type\":\"turn.completed\",\"turn_id\":\"native-turn\",\"usage\":{\"input_tokens\":2,\"cached_input_tokens\":0,\"output_tokens\":1}}"
                    ]
                    0
            ...
            let code =
                TelemetryRuntimeApplication.runObservedCodexExecWith
                    executable
                    (assignment "migration-five-worker")
                    None
                    TelemetryRuntime.Root
                    (Some root)
                    60L
            ...
            let reconciliation =
                TelemetryStoreApplication.reconcile root approved "UTEL-03A" |> unwrap
    ...
    let ``UTEL-06_2 packaged command consumes inherited context after root process death`` () =
        if not (OperatingSystem.IsWindows()) then
            ...
            File.WriteAllText(
                assignmentPath,
                """{"schema":"fsgg.telemetry.codex-assignment/1","featureId":"UTEL-06","itemId":"UTEL-06.2","attemptId":"attempt-1","parentAttemptId":null,"producerStream":"worker-command"}"""
            )
            ...
            try
                ...
                Assert.Equal(
                    0,
                    TelemetryRuntimeApplication.runCodexExec
                        [ "--assignment"; assignmentPath; "--"; "--json"; "--ephemeral"; "root" ]
                )

                let rootContext =
                    File.ReadAllBytes contextPath
                    |> TelemetryRuntime.parseInvocationContext
                    |> unwrap
                ...
                Assert.Equal(
                    0,
                    TelemetryRuntimeApplication.runCodexExec
                        [ "--relation"; "follow-up"; "--"; "--json"; "--ephemeral"; "follow" ]
                )
