...
module SelfHostCliTests =
    ...
    let private withReceipt action =
        let directory =
            Path.Combine(Path.GetTempPath(), "fsgg-self-host-" + Guid.NewGuid().ToString "n")
    ...
    let ``host mint binds measured candidate and snapshot bytes`` () =
        withReceipt (fun receipt candidate snapshot ->
            ...
            let code, stdout, stderr =
                run [ "self-host"; "mint"; proposal; candidate; snapshot; output ]
    ...
    let ``stable verifier binds receipt base and head to the candidate checkout`` () =
        let directory =
            Path.Combine(Path.GetTempPath(), "fsgg-self-host-git-" + Guid.NewGuid().ToString "n")
        ...
        try
            ...
            let code, _, stderr =
                run [ "self-host"; "verify"; receiptPath; candidate; directory ]
