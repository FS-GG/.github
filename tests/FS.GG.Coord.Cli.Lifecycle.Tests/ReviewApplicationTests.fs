...
module ReviewApplicationTests =
    ...
    let private run (projection: string) (raw: string) =
        ...
        try
            let opts =
                Options.parse [ "review"; "--snapshot"; path; projection ]
                |> function
                    | Ok value -> value
            ...
            try
                ReviewApplication.run opts, stdout.ToString(), stderr.ToString()
            finally
    ...
    let private renderWithWait binding facts waitState =
        let opts =
            Options.parse [ "review"; "FS-GG/.github#2175"; "--pr"; "42"; "--json" ]
            |> function
                | Ok value -> value
                | Error error -> failwith error
        ...
        try
            ReviewApplication.renderWithWait opts binding facts waitState, stdout.ToString(), stderr.ToString()
        finally
            Console.SetOut oldOut
            Console.SetError oldErr
    ...
    let ``ordinary exhaustion consumers cannot restore a local terminal predicate`` () =
        let readSide =
            File.ReadAllText(Path.Combine(repositoryRoot, "src/FS.GG.Coord.Cli.Lifecycle/ReviewApplication.fs"))
    ...
    let typed_acceptance_reaches_accept () =
        let chain =
            StructuredFixtures.acceptedReviewComments subject head "kestrel-1" |> comments
    ...
    let malformed_structured_record_fails_closed () =
        let malformed =
            comments [ 1L, "https://reviews/1", "<!-- fsgg:review-decision/v2 -->\n{}" ]
    ...
    let successor_wait_uses_typed_state_round_not_live_binding_default () =
        ...
        let wrongRound =
            { receipt with
                ReviewGeneration = ReviewWait.generationToken movedHead ReviewWait.RepairConfirmation binding.Round
            }

        let wrongCode, wrongOutput, _ =
            renderWithWait binding facts (ReviewWait.Waiting wrongRound)
    ...
    let pass_at_completed_round_three_turnover_enters_exhaustion_only_for_settled_red_checks () =
        ...
        let redCode, redOutput, redError =
            renderWithWait binding (facts Types.PrRed) completed
        ...
        let pendingCode, pendingOutput, pendingError =
            renderWithWait binding (facts Types.PrPending) completed
        ...
        let greenCode, greenOutput, greenError =
            renderWithWait binding (facts Types.PrGreen) completed
        ...
        let prePassFacts =
            { (facts Types.PrRed) with
                Comments = preRoundPassComments
            }
        ...
        let retiredComments =
            StructuredFixtures.ordinaryRoundThreePassCommentsWithRetiredGeneration subject head "critic-1"
            |> List.map (fun (id, url, body) -> ({ Id = id; Url = url; Body = body }: Driver.ReviewComment))

        let retiredFacts =
            { (facts Types.PrRed) with
                Comments = retiredComments
            }
