...
module HandlerRegistrationTests =
    ...
    let ``Lifecycle registration rejects duplicate missing and unexpected handlers`` () =
        ...
        Assert.True(
            Result.isError (
                HandlerRegistration.validate HandlerRegistration.commands ((Options.Help, inert) :: registrations)
            )
        )
    ...
    let ``production composition owns every parsed command exactly once`` () =
        ...
        let result =
            FS.GG.Coord.Cli.BoardOps.HandlerRegistration.validate Options.allCommands registrations
