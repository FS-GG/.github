...
module QualificationTests =
    ...
    let ``#3209 exact clean qualification accepts and replays canonical bytes`` () =
        ...
        let changed =
            { input with
                Operations =
                    { input.Operations.Head with
                        CommandSha256 = sha '9'
                    }
                    :: input.Operations.Tail
            }
    ...
    let ``#3209 dirty checkout and stale subject both fail closed`` () =
        let stale =
            { operation "analyze" Analyze with
                SubjectRevision = System.String('2', 40)
            }
        ...
        let values =
            findings
                { input with
                    CheckoutClean = false
                    Operations = stale :: input.Operations.Tail
                }
    ...
    let ``#3209 undeclared and drifted tool identities are distinct refusals`` () =
        ...
        let undeclared =
            { input.Operations.Head with
                Tool = { tool with Id = "fake" }
            }

        contains
            (function
            | UndeclaredTool("analyze", "fake") -> true
            | _ -> false)
            (findings
                { input with
                    Operations = undeclared :: input.Operations.Tail
                })

        let drifted =
            { input.Operations.Head with
                Tool = { tool with Version = "9.9.9" }
            }

        contains
            (function
            | ToolIdentityMismatch("analyze", "dotnet") -> true
            | _ -> false)
            (findings
                { input with
                    Operations = drifted :: input.Operations.Tail
                })
    ...
    let ``#3209 wrong executor and malformed provenance digest are refused`` () =
        ...
        let wrong =
            { input.Operations.Head with
                Executor = { executor with Id = "other" }
        ...
        let values =
            findings
                { input with
                    Operations = wrong :: input.Operations.Tail
                }

        contains
            (function
            | WrongExecutor("analyze", "other") -> true
            | _ -> false)
            values

        contains
            (function
            | InvalidDigest(field, _) when field.Contains "commandSha256" -> true
            | _ -> false)
            values
    ...
    let ``#3209 operation order and required lifecycle kinds are closed`` () =
        ...
        let withoutShip =
            input.Operations |> List.filter (fun operation -> operation.Kind <> Ship)

        let values =
            findings
                { input with
                    Operations = List.rev withoutShip
                }

        contains
            (function
            | MissingOperationKind Ship -> true
            | _ -> false)
            values

        contains
            (function
            | OperationOrderMismatch _ -> true
            | _ -> false)
            values
    ...
    let ``#3209 fixed-point replay binds exact result digest`` () =
        ...
        let operations =
            ...
            |> List.map (fun operation ->
                if operation.Kind = FixedPoint then
                    { operation with
                        ReplayResultSha256 = Some(sha '9')
                    }
                else
                    operation)
        ...
        contains
            (function
            | FixedPointMismatch "fixed" -> true
            | _ -> false)
            (findings { input with Operations = operations })
    ...
    let ``#3209 claims require existing subject-matched adequate evidence`` () =
        ...
        let claim =
            { input.Claims.Head with
                RequiredKinds = [ Ship ]
                EvidenceIds = [ "missing"; "verify" ]
            }
        ...
        contains
            (function
            | ClaimEvidenceMissing("full-qualification", "missing") -> true
            | _ -> false)
            values

        contains
            (function
            | InadequateClaimEvidence("full-qualification", Ship) -> true
            | _ -> false)
            values
    ...
    let ``#3209 mutation fixture must be independent and observe exact refusal`` () =
        ...
        contains
            (function
            | MutationRefusalMismatch "wrong-subject" -> true
            | _ -> false)
            values

        contains
            (function
            | MutationFixtureNotIndependent "wrong-subject" -> true
            | _ -> false)
            values
    ...
    let ``#3209 hosted observations reject pending foreign and growing sets`` () =
        ...
        let second =
            {
                Complete = false
                Checks = pending :: first.Checks.Tail @ [ { first.Checks.Head with Id = "new" } ]
            }

        let values =
            findings
                { input with
                    HostedObservations = [ first; second ]
                }

        contains
            (function
            | HostedObservationIncomplete 2 -> true
            | _ -> false)
            values

        contains
            (function
            | HostedForeignSubject("100", _) -> true
            | _ -> false)
            values

        contains
            (function
            | HostedCheckPending "100" -> true
            | _ -> false)
            values

        contains
            (function
            | HostedSetNotConverged -> true
            | _ -> false)
            values
    ...
    let ``#3209 obligation declaration is exact current-head singleton`` () =
        ...
        contains
            (function
            | ObligationHeadMismatch _ -> true
            | _ -> false)
            values

        contains
            (function
            | ObligationDeclarationDuplicate 2 -> true
            | _ -> false)
            values
    ...
    let ``#3209 duplicate obligation ids and absent declaration are refused`` () =
        ...
        contains
            (function
            | ObligationDeclarationMissing -> true
            | _ -> false)
            (findings
                { input with
                    Obligations =
        ...
        contains
            (function
            | ObligationIdDuplicate "publish" -> true
            | _ -> false)
            (findings
                { input with
                    Obligations =
                        { input.Obligations with
                            ...
                            Readbacks = [ input.Obligations.Readbacks.Head; input.Obligations.Readbacks.Head ]
                        }
                })
    ...
    let ``#3209 obligation acceptance requires authoritative readback identity`` () =
        ...
        contains
            (function
            | ObligationReadbackMissing -> true
            | _ -> false)
            (findings
                { input with
                    Obligations =
        ...
        let invalid =
            {
                CommentId = 0L
                Url = "file:///tmp/asserted"
                Author = ""
            }

        contains
            (function
            | ObligationReadbackInvalid -> true
            | _ -> false)
            (findings
                { input with
                    Obligations =
    ...
    let ``#3209 semantic review is mandatory and exact-subject`` () =
        ...
        contains
            (function
            | SemanticReviewMissing -> true
            | _ -> false)
            values

        contains
            (function
            | SemanticReviewStale _ -> true
            | _ -> false)
            values
        ...
        let values =
            findings
                { input with
                    ...
                    Operations = input.Operations @ [ input.Operations.Head ]
                }

        contains
            (function
            | DuplicateIdentity("tool", "dotnet") -> true
            | _ -> false)
            values

        contains
            (function
            | DuplicateIdentity("operation", "analyze") -> true
            | _ -> false)
            values
