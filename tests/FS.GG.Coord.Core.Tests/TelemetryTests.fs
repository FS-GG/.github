...
module TelemetryTests =
    ...
    let private sha (value: byte array) =
    ...
    let private usageRow task : RuntimeUsage.UsageRow =
...

        {
            ...
            CoordinationVersion = "4.5.6"
            SddVersion = "7.8.9"
            ContractsVersion = "10.0.0"
            LedgerSchema = 1
            Response = counts
            Turn = counts
            Thread = Some counts
    ...
    let ``#3259 usage receipts archive idempotently and corrupted canonical bytes fail closed`` () =
        ...
        let receiptBytes =
            RuntimeUsage.renderCsv [ usageRow "FS-GG/.github#42/claim" ] |> bytes
        ...
        Assert.True(
            receiptBytes
                .AsSpan()
                .SequenceEqual((UsageReceiptStore.resolve (Some store) first.Source |> unwrap).AsSpan())
        )
        ...
        if not (OperatingSystem.IsWindows()) then
            ...
            Assert.Contains(
                ...
                sprintf "%A" (UsageReceiptStore.resolve (Some store) first.Source)
            )

        Assert.True(
            UsageReceiptStore.archive (Some(Path.GetTempPath())) receiptBytes
            |> Result.isError
        )

        Assert.True(
            UsageReceiptStore.archive (Some(Path.GetPathRoot store)) receiptBytes
            |> Result.isError
        )
        ...
        if not (OperatingSystem.IsWindows()) then
            ...
            Assert.Contains(
                "symbolic link",
                sprintf "%A" (UsageReceiptStore.archive (Some(Path.Combine(link, "nested"))) receiptBytes)
            )
    ...
    let ``#3259 reviewed legacy proof excludes a missing measured receipt without reconstructing counts`` () =
        let tooling =
            """{"ledger_schema":1,"runtime":{"status":"recorded","name":"codex","version":"1.2.3","source":"session"},"coordination":{"status":"recorded","name":"fsgg-coord","version":"4.5.6","source":"cli"},"sdd":{"status":"recorded","name":"fsgg-sdd","version":"7.8.9","source":"cli"},"contracts":{"status":"recorded","name":"fsgg-contracts","version":"10.0.0","source":"registry"}}"""

        let model =
            """{"status":"recorded","provider":"OpenAI","name":"gpt-test","effort":"high","source":"runtime receipt"}"""
        ...
        let receipt =
            RuntimeUsage.renderCsv [ usageRow "FS-GG/.github#42/claim" ]
            |> bytes
            |> RuntimeUsage.parseCsvReceipt
            |> unwrap

        let source, _ = receipt

        let started =
            LifecycleTelemetry.sealSuccessor
                "run"
                "unit"
                ""
                (draft 1 "claim" "started" "2026-09-04T08:00:00Z" "null" "{\"status\":\"pending\"}" [ "claim" ])
            |> unwrap

        let measured =
            $"""{{"status":"measured","input":10,"cached_input":4,"cache_write_input":0,"output":5,"reasoning":2,"total":15,"source":"%s{source}","session_ids":["session-1"],"turn_ids":["turn-1"]}}"""

        let completed =
            LifecycleTelemetry.sealSuccessorWithEvidence
                "run"
                "unit"
                [ receipt ]
                []
                started
                (draft 1 "claim" "completed" "2026-09-04T08:01:00Z" "1" measured [ "usage" ])
            |> unwrap

        use completedJson = JsonDocument.Parse completed
        let eventDigest = completedJson.RootElement.GetProperty("digest").GetString()

        let proofWithoutDigest =
            $"""{{"schema":"%s{LegacyReceiptProof.Schema}","original_event_digest":"%s{eventDigest}","missing_receipt_source":"%s{source}","authority":{{"subject":"FS-GG/.github#42","comment_id":123}},"lookup_evidence":["canonical-store:absent","source-session:absent"],"author":"recovery-1a2b","reviewer":"critic-2c3d","review_evidence":["https://github.com/FS-GG/.github/issues/42#issuecomment-124"],"decision":"irrecoverable-exclude-usage"}}"""

        let proofDigest =
            CanonicalJson.canonicalize (bytes proofWithoutDigest) |> unwrap |> bytes |> sha

        let proofJson =
            proofWithoutDigest[.. proofWithoutDigest.Length - 2]
            + $",\"digest\":\"%s{proofDigest}\"}}"

        let proof = LegacyReceiptProof.parse (bytes proofJson) |> unwrap
        let existing = started + completed

        Assert.True(
            LifecycleTelemetry.validateWithEvidenceAndLegacy "run" "unit" false false [] [] [] [] existing
            |> Result.isError
        )

        let recovery =
            LifecycleTelemetry.sealSuccessorWithEvidenceAndLegacy
                "run"
                "unit"
                []
                [ proof ]
                ...
                (draft
                    2
                    "legacy-receipt-recovery-claim"
                    "started"
                    "2026-09-04T08:02:00Z"
                    "null"
                    "{\"status\":\"pending\"}"
                    [ "legacy-receipt-proof:sha256:" + proofDigest ])
            |> unwrap

        LifecycleTelemetry.validateWithEvidenceAndLegacy
            "run"
            "unit"
            false
            false
        ...
        Assert.True(
            LegacyReceiptProof.parse (bytes (proofJson.Replace("\"critic-2c3d\"", "\"recovery-1a2b\"")))
            |> Result.isError
        )
    ...
    let ``Codex collection preserves stable schema and exact counter arithmetic`` () =
        let counts =
            """{"input_tokens":10,"cached_input_tokens":4,"cache_write_input_tokens":0,"output_tokens":5,"reasoning_output_tokens":2,"total_tokens":15}"""
        ...
        let session =
            [
                ...
                $"""{{"timestamp":"2026-01-01T00:01:00Z","type":"token_usage_record","payload":{{"thread_id":"thread-1","turn_id":"turn-1","%s{sessionIdKey}":"session-1","response_id":"response-1","usage":%s{counts},"turn_token_usage":%s{counts},"thread_token_usage":%s{counts}}}}}"""
            ]
            |> String.concat "\n"

        let result =
            RuntimeUsage.collectCodex "repo#1/claim" None false None None "4.5.6" "7.8.9" "10.0.0" (bytes session)
            |> unwrap
        ...
        Assert.StartsWith(
            "timestamp,task,session_id,thread_id,turn_id,response_id,provider,model,effort,runtime_version,coordination_version,sdd_version,contracts_version,ledger_schema",
            RuntimeUsage.renderCsv result.Rows
        )
    ...
    let ``Claude collection binds model runtime and deterministic response identity`` () =
        let snapshot =
            $"""{{"timestamp":"2026-01-01T00:02:00Z","%s{sessionIdKey}":"claude-session","prompt_id":"prompt-1","version":"2.3.4","model":{{"id":"claude-test"}},"effort":{{"level":"high"}},"context_window":{{"current_usage":{{"input_tokens":7,"cache_read_input_tokens":2,"cache_creation_input_tokens":1,"output_tokens":3}}}}}}"""

        let first = RuntimeUsage.collectClaude "task" "1" "2" "3" (bytes snapshot) |> unwrap

        let second =
            RuntimeUsage.collectClaude "task" "1" "2" "3" (bytes snapshot) |> unwrap
    ...
    let ``lifecycle sealing is canonical and terminal pending usage is refused`` () =
        ...
        let started =
            common "started" "2026-09-04T08:00:00Z" "null" "{\"status\":\"pending\"}"

        let sealedStarted =
            LifecycleTelemetry.sealSuccessor "run" "unit" "" started |> unwrap
        ...
        let pendingCompletion =
            common "completed" "2026-09-04T08:01:00Z" "1" "{\"status\":\"pending\"}"

        Assert.True(
            LifecycleTelemetry.sealSuccessor "run" "unit" sealedStarted pendingCompletion
            |> Result.isError
        )
        ...
        let invented =
            common
                ...
                "1"
                "{\"status\":\"measured\",\"input\":1,\"cached_input\":0,\"cache_write_input\":0,\"output\":1,\"reasoning\":0,\"total\":2,\"source\":\"runtime-usage-csv:sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\",\"session_ids\":[\"s\"],\"turn_ids\":[\"t\"]}"

        Assert.True(
            LifecycleTelemetry.sealSuccessor "run" "unit" sealedStarted invented
            |> Result.isError
        )
    ...
    let ``#3210 reconciled validation requires an exact-digest measured recovery for timing placeholders`` () =
        ...
        let legacy =
            LifecycleTelemetry.sealSuccessor
                ...
                (draft
                    ...
                    "2026-09-04T08:01:00Z"
                    "1"
                    "{\"status\":\"unavailable\",\"reason\":\"final usage is written after this response\",\"source\":\"legacy child\"}"
                    [ "legacy" ])
            |> unwrap
        ...
        Assert.True(
            LifecycleTelemetry.validateWithEvidence "run" "unit" true [] [] [] unreconciled
            |> Result.isOk
        )

        Assert.True(
            LifecycleTelemetry.validateReconciledWithEvidence "run" "unit" true [] [] [] unreconciled
            |> Result.isError
        )
        ...
        let row: RuntimeUsage.UsageRow =
            {
                Timestamp = "2026-09-04T08:03:00Z"
                Task = "FS-GG/.github#42/telemetry-reconciliation-claim"
                SessionId = "session-1"
                ThreadId = "thread-1"
                TurnId = "turn-1"
                ResponseId = "response-1"
        ...
        let recoveryStarted =
            LifecycleTelemetry.sealSuccessor
                ...
                (draft
                    2
                    "telemetry-reconciliation-claim"
                    "started"
                    "2026-09-04T08:02:00Z"
                    "null"
                    "{\"status\":\"pending\"}"
        ...
        let measured =
            $"""{{"status":"measured","input":10,"cached_input":4,"cache_write_input":0,"output":5,"reasoning":2,"total":15,"source":"%s{usageDigest}","session_ids":["session-1"],"turn_ids":["turn-1"]}}"""
        ...
        let recovered =
            LifecycleTelemetry.sealSuccessorWithEvidence
                ...
                (draft
                    ...
                    "1"
                    measured
                    [ "supersedes-lifecycle-digest:" + legacyDigest ])
            |> unwrap

        let complete = withRecoveryStarted + recovered

        LifecycleTelemetry.validateReconciledWithEvidence "run" "unit" true [] [ usageReceipt ] [] complete
        |> unwrap
        |> ignore
        ...
        Assert.True(
            LifecycleTelemetry.validateReconciledWithEvidence
                ...
                [ usageReceipt ]
                []
                (withRecoveryStarted + recoveredWithExtraEvidence)
            |> Result.isError
        )
        ...
        LifecycleTelemetry.validateReconciledWithEvidence "run" "unit" true [] [] [] (started + genuineFailure)
        |> unwrap
        |> ignore
    ...
    let ``#3263 human-authorized synthetic checkpoint establishes one strict new anchor`` () =
        ...
        let malformedReceipt =
            RuntimeUsage.renderCsv [ usageRow "FS-GG/.github#42/telemetry-reconciliation-claim" ]
            |> bytes
            |> RuntimeUsage.parseCsvReceipt
            |> unwrap
        ...
        let malformedMeasured =
            $"""{{"status":"measured","input":10,"cached_input":4,"cache_write_input":0,"output":5,"reasoning":2,"total":15,"source":"%s{malformedSource}","session_ids":["session-1"],"turn_ids":["turn-1"]}}"""

        let malformedComplete =
            LifecycleTelemetry.sealSuccessorWithEvidence
                "run"
                "unit"
                [ malformedReceipt ]
                []
                (initial + legacy + malformedStart)
                (draft
                    2
                    "telemetry-reconciliation-claim"
                    "completed"
                    ...
                    "1"
                    malformedMeasured
                    [ "supersedes unavailable lifecycle event digest in prose" ])
            |> unwrap
        ...
        let proof =
            SyntheticCheckpointProof.parse (bytes (proofJson "FS-GG/.github" 42 "run" "unit" 4 frontierDigest "passed"))
            |> unwrap
        ...
        let checkpointStart =
            LifecycleTelemetry.sealSuccessorWithEvidenceAndCheckpoints
                ...
                (draft
                    3
                    "synthetic-evidence-checkpoint"
                    "started"
                    "2026-09-04T08:04:00Z"
                    "null"
                    "{\"status\":\"pending\"}"
                    [ "synthetic-checkpoint:sha256:" + proof.Digest ])
            |> unwrap
        ...
        Assert.True(
            LifecycleTelemetry.sealSuccessorWithEvidenceAndCheckpoints
                ...
                (draft
                    4
                    "ordinary-after-unfinished-checkpoint"
                    "started"
                    "2026-09-04T08:04:30Z"
                    "null"
                    "{\"status\":\"pending\"}"
                    [ "must-refuse" ])
            |> Result.isError
        ...
        let receipt =
            RuntimeUsage.renderCsv [ usageRow "FS-GG/.github#42/synthetic-evidence-checkpoint" ]
            |> bytes
            |> RuntimeUsage.parseCsvReceipt
            |> unwrap
        ...
        let wrongScope =
            SyntheticCheckpointProof.parse (bytes (proofJson "FS-GG/other" 42 "run" "unit" 4 frontierDigest "passed"))
            |> unwrap
        ...
        let wrongFrontier =
            SyntheticCheckpointProof.parse (
                bytes (proofJson "FS-GG/.github" 42 "run" "unit" 1 (String.replicate 64 "c") "passed")
            )
            |> unwrap
        ...
        Assert.True(
            SyntheticCheckpointProof.parse (bytes (proofJson "FS-GG/.github" 42 "run" "unit" 2 frontierDigest "failed"))
            |> Result.isError
        )

        let tampered =
            SyntheticCheckpointProof.canonicalize proof
            |> fun value -> value.Replace("tool-version-incompatibility", "extraordinary-other")
        ...
        let laterStart =
            LifecycleTelemetry.sealSuccessorWithEvidenceAndCheckpoints
                ...
                []
                checkpointed
                (draft 4 "normal" "started" "2026-09-04T08:06:00Z" "null" "{\"status\":\"pending\"}" [ "normal" ])
            |> unwrap

        let laterReceipt =
            RuntimeUsage.renderCsv [ usageRow "FS-GG/.github#42/normal" ]
            |> bytes
            |> RuntimeUsage.parseCsvReceipt
            |> unwrap
        ...
        let laterComplete =
            LifecycleTelemetry.sealSuccessorWithEvidenceAndCheckpoints
                ...
                [ proof ]
                []
                (checkpointed + laterStart)
                (draft 4 "normal" "completed" "2026-09-04T08:07:00Z" "1" laterMeasured [ "normal-complete" ])
            |> unwrap
...
"""
        ...
        let audit =
            $"""{{"auditSchema":1,"report":"feedback/report.md","reportSha256":"%s{digest}","findings":[]}}"""
        ...
        Assert.True(
            FeedbackReceipt.validate
                ...
                (bytes report)
                (bytes audit)
                (Some "not-json-and-wrong-cycle")
            |> Result.isError
        )

    let private closureInputs sourceDigest =
        let candidate, implementation, acceptance =
            String.replicate 40 "a", String.replicate 40 "b", String.replicate 40 "c"
        ...
        let reportHash, auditArtifactHash, contractHash =
            sha report, String.replicate 64 "d", String.replicate 64 "e"

        let audit =
            $"""{{"auditSchema":1,"report":"feedback/report.md","reportSha256":"%s{reportHash}","findings":[]}}"""
            |> bytes
        ...
        let accepted =
            sealedReceipt
                $"""{{"acceptedAt":"2026-01-01T00:00:00Z","artifacts":[{{"name":"implementation-candidate-%s{candidate}","sha256":"%s{auditArtifactHash}"}}],"schema":"fsgg.coordination.unit-acceptance/1","sourceRevision":"%s{candidate}","state":"accepted","unitContractSha256":"%s{contractHash}","unitId":"GS2-01.1"}}"""

        let delivery =
            sealedReceipt
                $"""{{"acceptanceMergeHead":"%s{acceptance}","candidateHead":"%s{candidate}","claimsRemaining":0,"implementationMergeHead":"%s{implementation}","issueUrl":"https://github.com/FS-GG/repo/issues/1","pullRequestUrl":"https://github.com/FS-GG/repo/pull/2","schema":"fsgg.roadmap.delivery/1","unitId":"GS2-01.1"}}"""

        let feedbackBinding =
            sealedReceipt
                $"""{{"auditSha256":"%s{auditHash}","cycleId":"cycle-1","head":"%s{acceptance}","reportSha256":"%s{reportHash}","schema":"fsgg.roadmap.feedback-binding/1","unitId":"GS2-01.1"}}"""

        let cycle =
            sealedReceipt
                $"""{{"cycleId":"cycle-1","head":"%s{acceptance}","schema":"fsgg.roadmap.cycle-update/1","unitId":"GS2-01.1"}}"""

        let check =
            sealedReceipt
                $"""{{"head":"%s{acceptance}","name":"required","owner":null,"passed":true,"required":true,"schema":"fsgg.roadmap.check/1","unitId":"GS2-01.1"}}"""

        {
            UnitId = "GS2-01.1"
            Title = "Typed thing"
            RoadmapSourceDigest = sourceDigest
            ...
            FeedbackAudit = audit
            FeedbackPhases = [ "claim"; "implementation" ]
            FeedbackCheckpoint = None
            FeedbackBinding = feedbackBinding
            CycleUpdate = cycle
            CheckReceipts = [ check ]
        }
        : RoadmapClosure.Inputs

    let private closure sourceDigest =
        closureInputs sourceDigest |> RoadmapClosure.inspect |> unwrap
    ...
    let ``roadmap rendering is bounded deterministic and keeps unrelated failures external`` () =
        ...
        let rendered =
            RoadmapProjection.render sourceDigest (bytes original) accepted |> unwrap
        ...
        Assert.True(
            RoadmapProjection.verify sourceDigest (bytes original) (bytes tamperedOutside) accepted
            |> Result.isError
        )

        Assert.True(
            RoadmapProjection.verify sourceDigest (bytes original) (bytes rendered) accepted
            |> Result.isOk
        )

        Assert.True(
            RoadmapProjection.render ("sha256:" + sha (bytes rendered)) (bytes rendered) accepted
            |> Result.isError
        )
    ...
    let ``roadmap closure refuses required failure and ambiguous marker authority`` () =
        ...
        let tampered =
            { inputs with
                AcceptedReceipt = inputs.AcceptedReceipt |> Array.copy
            }
        ...
        let ownerless =
            sealedReceipt
                $"""{{"head":"%s{acceptance}","name":"unrelated","owner":null,"passed":false,"required":false,"schema":"fsgg.roadmap.check/1","unitId":"GS2-01.1"}}"""

        Assert.True(
            RoadmapClosure.inspect
                { inputs with
                    CheckReceipts = [ ownerless ]
                }
            |> Result.isError
        )
        ...
        Assert.True(
            RoadmapProjection.render digest (bytes ambiguous) (closure digest)
            |> Result.isError
        )
