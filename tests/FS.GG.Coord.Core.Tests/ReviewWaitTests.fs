...
module ReviewWaitTests =
    ...
    let ``2756 a current receipt reserves after the active lease duration`` () =
        match
            ReviewWait.project
                receipt.Item
                (Some receipt.ClaimGeneration)
                true
                (entered.AddHours 3.0)
    ...
    let ``2756 a changed claim generation never resurrects mutation authority`` () =
        match
            ReviewWait.project receipt.Item (Some "new-claim") true (entered.AddHours 1.0) [ ReviewWait.Enter receipt ]
        with
    ...
    let ``2756 a terminal event cannot hide a changed claim generation`` () =
        let completed =
            ReviewWait.Complete(receipt.ReviewGeneration, entered.AddMinutes 1.0, "old-claim-pass")
        ...
        match
            ReviewWait.project
                ...
                (Some "new-claim")
                true
                (entered.AddHours 1.0)
                [ ReviewWait.Enter receipt; completed ]
        with
        | ReviewWait.Recoverable(_, reason) -> Assert.Contains("reacquire", reason)
    ...
    let ``2756 completion recorded before expiry wins a later timeout race`` () =
        let completed =
            ReviewWait.Complete(receipt.ReviewGeneration, receipt.ExpiresAt.AddSeconds(-1.0), "https://review/pass")

        let timeout =
            ReviewWait.Timeout(receipt.ReviewGeneration, receipt.ExpiresAt, "timer")
        ...
        match
            ReviewWait.project
                ...
                (Some receipt.ClaimGeneration)
                true
                (receipt.ExpiresAt.AddMinutes 1.0)
                [ ReviewWait.Enter receipt; completed; timeout ]
        with
    ...
    let ``2756 first durable terminal wins even when a later comment is backdated`` () =
        ...
        let backdated =
            ReviewWait.Complete(receipt.ReviewGeneration, receipt.ExpiresAt.AddHours(-1.0), "late-comment")
        ...
        match
            ReviewWait.project
                ...
                true
                receipt.ExpiresAt
                [ ReviewWait.Enter receipt; timeout; backdated ]
        with
    ...
    let ``2756 receipt is bounded and cannot reserve forever`` () =
        let unbounded =
            ReviewWait.Enter
                { receipt with
                    ExpiresAt = entered.AddHours 25.0
                }
    ...
    let ``2756 a premature timeout cannot consume the bounded wait`` () =
        let early =
            ReviewWait.Timeout(receipt.ReviewGeneration, receipt.ExpiresAt.AddSeconds(-1.0), "early-timer")
        ...
        match
            ReviewWait.project
                ...
                true
                (entered.AddHours 1.0)
                [ ReviewWait.Enter receipt; early ]
        with
    ...
    let ``2756 two different unconsumed generations are invalid rather than latest wins`` () =
        ...
        match
            ReviewWait.project
                ...
                true
                (entered.AddHours 1.0)
                [ ReviewWait.Enter receipt; ReviewWait.Enter second ]
        with
        | ReviewWait.Invalid errors ->
            Assert.Contains("multiple review generations are unconsumed", String.concat "; " errors)
    ...
    let ``2756 a new generation is admitted after the preceding one is consumed`` () =
        let completed =
            ReviewWait.Complete(receipt.ReviewGeneration, entered.AddMinutes 1.0, "first-complete")
        ...
        match
            ReviewWait.project
                ...
                true
                (entered.AddHours 1.0)
                [ ReviewWait.Enter receipt; completed; ReviewWait.Enter second ]
        with
    ...
    let ``2797 claim turnover preserves no non-exhaustion generation`` generation =
        ...
        let completed =
            ReviewWait.Complete(generation, entered.AddMinutes 1.0, "old-review")
        ...
        match
            ReviewWait.project
                ...
                false
                (entered.AddHours 1.0)
                [ ReviewWait.Enter other; completed ]
        with
        | ReviewWait.Recoverable(_, reason) -> Assert.Contains("reacquire", reason)
