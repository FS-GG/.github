...
module CycleLedgerTests =
    ...
    let critiqueBytes cycleId head game journey =
        let journeys =
            if journey then
                "[{\"entry_point\":\"product-boot\",\"input_surface\":\"player-control-messages\",\"reached\":true}]"
            else
                "[]"
    ...
    let ``provider artifacts are byte-bound and reject unsupported generator provenance`` () =
        ...
        let valid =
            receipt providerCycle target "base" "head" "critique" (critiqueBytes providerCycle "head" false false)
        ...
        Assert.True(
            validateProvider
                ...
                "head"
                "critique"
                "fsgg.critique.report/3"
                "FS.GG.Critique.Validator"
                valid
            |> Result.isOk
        )

        let normalizedForgery =
            Encoding.UTF8.GetBytes
                $"""{{"schema":"fsgg.critique.report/3","provider":"critique","workId":"work","cycleId":"%s{target.Id}","sourceRevision":"base","candidateHead":"head","verdict":"pass","round":0,"playerJourney":null,"generator":{{"id":"FS.GG.Critique.Validator","version":"1.0.0"}}}}"""

        let forged =
            parseProviderReceipt "work" target "base" "head" "critique" normalizedForgery
    ...
    let ``player journey applicability is derived from the ledger unit`` () =
        ...
        let model =
            {
                SourceRevision = "base"
                Units = [ unit "work" [] false ]
            }

        let providerCycle = model.Units.Head.ProviderCycleId

        let implementation =
            receipt "work" target "base" "head" "fsgg-sdd" (sddBytes "work")

        let feedback =
            receipt providerCycle target "base" "head" "feedback" (feedbackBytes providerCycle)
        ...
        Assert.True(
            parseProviderReceipt
                ...
                "head"
                "critique"
                (critiqueBytes providerCycle "head" true false)
            |> Result.isError
        )

        let passingReview =
            receipt providerCycle target "base" "head" "critique" (critiqueBytes providerCycle "head" true true)

        Assert.Equal(
            Advance target,
            advance model target implementation passingReview feedback (evidence target "head")
            |> unwrap
        )
    ...
    let ``guarded update emits the exact receipt completion revalidates`` () =
        ...
        let guarded =
            match
                update model target (evidence target "head") "0123456789abcdef0123456789abcdef"
                |> unwrap
            with
        ...
        Assert.True(
            complete
                ...
                [
                    { guarded with
                        EvidenceDigest = "sha256:" + String.replicate 64 "a"
                    }
                ]
                [ target.Id ]
            |> Result.isError
    ...
    let ``parallel ready units require explicit operator scheduling and resume the selected live cycle`` () =
        let twoReady =
            { ledger with
                Units = [ unit "one" [] false; unit "two" [] false ]
            }

        Assert.True(
            register twoReady "worker" ".github" "base" None false false []
            |> Result.isError
        )
        ...
        Assert.Equal(
            Resume target,
            register twoReady "worker" ".github" "base" (Some "one") true true [ target ]
            |> unwrap
        )
        ...
        let cyclic =
            {
                ...
                Units = [ unit "a" [ "b" ] false; unit "b" [ "a" ] false ]
            }
