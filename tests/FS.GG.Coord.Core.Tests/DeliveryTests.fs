...
module DeliveryTests =
    ...
    let ``#2131 changed head after acceptance requires fresh review`` () =
        let changed =
            { snapshot "head-b" with
                Review = Some(review "head-a")
            }

        transition
            Delivery.ReviewActive
            (Delivery.RefreshReview "independent review is for a different head SHA")
            (Delivery.inspect changed)
    ...
    let ``#2207 malformed review evidence is refreshed rather than reported absent`` () =
        let malformed =
            { snapshot "head-a" with
                Review = None
                ReviewProblem = Some "comment 42 is missing the required 'verdict' field"
            }

        transition
            Delivery.ReviewActive
            (Delivery.RefreshReview "comment 42 is missing the required 'verdict' field")
            (Delivery.inspect malformed)
    ...
    let ``#3286 markerless landing exception fails closed for mutated two-phase authority`` () =
        ...
        let mutations =
            [
                ...
                { baseline with
                    Obligations = [ acceptanceReceipt "old-head" ]
                }
                { baseline with
                    Obligations =
                        [
                            { acceptanceReceipt "head-a" with
                                Evidence = Some "https://receipt"
                                Verified = true
                            }
                        ]
                }
                { baseline with
                    Obligations = [ acceptanceReceipt "head-a"; acceptanceReceipt "head-a" ]
                }
                { baseline with
                    Freshness =
                        { baseline.Freshness with
                            BoardState = "Ready"
                        }
                }
    ...
    let ``#2131 landing requires an explicit machine obligation declaration`` () =
        ...
        transition
            Delivery.ReviewReady
            (Delivery.RepairReviewHandoff "delivery obligations are undeclared")
            (Delivery.inspect undeclared)
    ...
    let ``#2905 a qualifying success admits completion while mixed diagnostics stay receipt-bound`` () =
        ...
        Assert.Equal(
            ...
            |> Delivery.tryDecodeCompletionReceipt
        )
        ...
        let mismatched =
            ...
            | Delivery.Verified receipt ->
                ...
                Delivery.Verified
                    { receipt with
                        Runs = { first with Sha = "other-sha" } :: receipt.Runs.Tail
                    }
    ...
    let ``#2905 red and unreadable post-merge observations remain visible and recoverable`` () =
        ...
        for verification, expected in
            [
                ...
                Delivery.Unreadable "Actions API timed out", "Actions API timed out"
            ] do
            match Delivery.inspectWithPostMergeVerification verification merged with
    ...
    let ``complete delivery decision owns obligations reachability projections and cleanup`` () =
        ...
        let decide facts =
            facts |> completionFactsVerified |> Delivery.decideCompletion
        ...
        Assert.Equal(Delivery.CompletionDecision.VerifyOutstandingObligation "nuget", decide pendingObligation)
                        ...
                        for claimReleased in [ false; true ] do
                            for pendingWrites in [ 0; 1 ] do
                                for cleanupEligible in [ false; true ] do
                                    ...
                                    let decision = candidate |> completionFactsVerified |> Delivery.decideCompletion
                                    ...
                                    let expected =
                                        ...
                                        | _ when
                                            not issueClosed || not boardDone || not claimReleased || pendingWrites <> 0
                                            ->
                                            "project"
    ...
    let ``delivery completion mutation controls kill obligation head evidence and projection forks`` () =
        ...
        Assert.NotEqual(
            ...
            decide
                { terminal with
                    Obligations = [ obligation "kit" "old-head" (Some "https://receipt") true ]
                }
        )
        ...
        Assert.NotEqual(
            ...
            decide
                { terminal with
                    Obligations = [ obligation "kit" "head-a" None true ]
                }
        )
    ...
    let ``delivery completion receipt binds merge obligations pending writes and transition identity`` () =
        ...
        let candidate =
            { snapshot "head-a" with
                ...
                PendingWrites = 0
                CleanupEligible = false
                Obligations = [ verified "kit" "https://receipt/kit"; verified "tool" "https://receipt/tool" ]
            }
        ...
        for tampered in
            [
                { receipt with
                    MergeSha = "different-merge"
                }
                { receipt with PendingBoardWrites = 1 }
                { receipt with
                    FreshnessToken = "different-transition"
                }
                { receipt with
                    ObligationReceipts =
                        { receipt.ObligationReceipts.Head with
                            Evidence = "different-evidence"
                        }
                        :: receipt.ObligationReceipts.Tail
                }
            ] do
    ...
    let ``#2233 an unread touch-set names the read as the failure`` () =
        let unread =
            { snapshot "head-a" with
                Freshness =
                    { freshness "head-a" with
                        DeclaredPaths = Delivery.Unread "issue body fetch timed out"
                    }
            }
    ...
    let ``#2233 the case is folded in, not just the text — an Unread reason cannot collide with the Known path it textually matches``
        ...
        let known =
            { freshness "head-a" with
                DeclaredPaths = Delivery.Known [ "shared text" ]
            }

        let unread =
            { freshness "head-a" with
                DeclaredPaths = Delivery.Unread "shared text"
            }
    ...
    let ``#2233 a freshness token distinguishes two different unread reasons`` () =
        ...
        let rateLimited =
            { freshness "head-a" with
                DeclaredPaths = Delivery.Unread "rate limited"
            }
        ...
        let read =
            terminal
                { snapshot "head-a" with
                    Freshness =
                        { freshness "head-a" with
                            DeclaredPaths = Delivery.Known [ "src" ]
                        }
                }
    ...
    let ``#2233 a freshness token distinguishes DeclaredNone from Undeclared from Known empty`` () =
        let declaredNone =
            { freshness "head-a" with
                DeclaredPaths = Delivery.DeclaredNone
            }

        let undeclared =
            { freshness "head-a" with
                DeclaredPaths = Delivery.Undeclared
            }
        ...
        let tokens =
            [
                Delivery.freshnessToken declaredNone
                Delivery.freshnessToken undeclared
                Delivery.freshnessToken known
            ]
    ...
    let ``#2175 fromReviewAcceptance folds a Review receipt into the same Driver.ReviewChain shape Delivery already reads``
        ...
        let staleFolded =
            Delivery.fromReviewAcceptance
                receipt
                { unfolded with
                    Freshness =
                        { unfolded.Freshness with
                            HeadSha = "head-b"
                        }
                }
    ...
    let ``#2575 a complete chain whose checks are not green is never reported as a review problem`` () =
        ...
        | Delivery.Next {
                            Action = Delivery.RefreshReview reason
    ...
    let ``#2773 one classifier enumerates every path admission with authority revisions`` () =
        ...
        let admission path =
            results |> List.find (fun result -> result.Path = path)
        ...
        Assert.True(
            (admission "work/2773-delivery-path-classifier/spec.md").AuthorityRevisions = [ "delivery-route:def" ]
        )

        Assert.Equal(Delivery.UndeclaredAuthoredPath, (admission "src/Undeclared.fs").Admission)

        let undeclaredRevisions =
            (admission "src/Undeclared.fs").AuthorityRevisions |> Set.ofList
        ...
        let results =
            Delivery.classifyPaths
                ...
                (Delivery.AuthorityKnown(
                    "delivery-route:def",
                    [ Types.Matchable "readiness/2773-delivery-path-classifier" ]
                ))
