...
module DriverTests =
    ...
    let saysThat fragment (errors: string list) =
        errors |> List.exists (fun e -> e.Contains(fragment: string))
    ...
    let ``#2162 triage content dispositions bind a durable consumer or evidenced one-off decision`` () =
        ...
        let disposition kind consumers rationale evidence =
            {
                ...
                ReceiptId =
                    contentDispositionReceiptId
                        "audit/2162: reusable failure boundary"
                        kind
                        consumers
                        "host-2162"
                        rationale
        ...
        let proseOnly =
            disposition
                ExampleFixture
                [ "docs/coordination/triage.md" ]
                "A document is not an executable consumer."
                None
        ...
        let oneOff =
            disposition
                NotReusable
                []
                "Measured as a repository-specific one-off with no reusable operator learning."
                (Some(EvidencePath "tests/FS.GG.Coord.Core.Tests/DriverTests.fs:130"))
        ...
        let unsupportedOneOff =
            ...
            { oneOff with
                ...
                ReceiptId =
                    contentDispositionReceiptId
                        oneOff.SourceFinding
                        oneOff.Disposition
                        oneOff.ConsumerPaths
                        oneOff.DecisionMaker
                        oneOff.Rationale
                        evidence
                        oneOff.ObservedAt
                        oneOff.SourceSha
