...
module TelemetryBudgetTests =
    ...
    let ``thresholds are exact and strict`` numerator denominator expected severe =
        ...
        | TelemetryBudget.Breach(_, _, actual) ->
            Assert.Equal("breach", expected)
            Assert.Equal(severe, actual)
    ...
    let ``known zero activity is not applicable and a missing denominator is unknown`` () =
        Assert.Equal(
            TelemetryBudget.NotApplicableVerdict "known-zero-activity",
            TelemetryBudget.assess (TelemetryBudget.Usable(0L, 0L))
        )

        Assert.Equal(
            TelemetryBudget.UnknownVerdict "missing-denominator",
            TelemetryBudget.assess (TelemetryBudget.Usable(1L, 0L))
        )

    [<Fact>]
    let ``parallel waits are unioned and witnessed work is excluded at nanosecond precision`` () =
        let interval startAt endAt : TelemetryBudget.Interval =
            {
        ...
        let waits =
            [ interval 0L 120_000_000_000L; interval 30_000_000_000L 90_000_000_000L ]
    ...
    let ``caller-authored budget verdicts are not facts`` () =
        let bytes =
            Encoding.UTF8.GetBytes
                $"""{{"schema":"{TelemetryStore.BatchSchema}","ingestId":"bad-budget","sourceIdentity":"worker","generation":"g","cursor":"1","eventCount":1,"events":[{{"kind":"budget-assessment","identity":"bad","itemId":"item","revision":0,"verdict":"pass","reset":true}}]}}"""
    ...
    let ``completion must come from the native item source`` () =
        let bytes =
            Encoding.UTF8.GetBytes
                $"""{{"schema":"{TelemetryStore.BatchSchema}","ingestId":"bad-population","sourceIdentity":"worker","generation":"g","cursor":"1","eventCount":1,"events":[{{"kind":"budget-population","identity":"population","itemId":"item","revision":0,"originalItemId":"item","state":"completed","sourceKind":"caller","sourceRef":"fake"}}]}}"""
    ...
    let ``machine native outcome is closed and does not carry an assessment verdict`` () =
        ...
        let withVerdict =
            Encoding.UTF8
                .GetString(valid)
                .Replace(
                    "\"sourceRef\":\"routine-delivery:item\"",
                    "\"sourceRef\":\"routine-delivery:item\",\"verdict\":\"pass\""
                )
            |> Encoding.UTF8.GetBytes
