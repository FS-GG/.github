...
module StructuredDecisionTests =
        ...
        let draft: StructuredDecision.ReviewRecord =
            {
                ...
                ClaimGeneration =
                    if kind = StructuredDecision.Acceptance then
                        Some "claim-1"
                    else
                        None
                BaseSha =
    ...
    let ``M4 route authorization is bound to structured scope dependencies touch-set policy and revision`` () =
        ...
        let missingDependencies =
            { missingDependencies with
                Digest = StructuredDecision.routeDigest missingDependencies
            }

        Assert.True(
            StructuredDecision.validateRouteLedger record.Subject [ missingDependencies ]
            |> Result.isError
        )
    ...
    let ``M4 append requires the exact previous digest and next revision`` () =
        ...
        Assert.True(
            StructuredDecision.validateRouteLedger first.Subject [ first; valid ]
            |> Result.isOk
        )
        ...
        Assert.True(
            StructuredDecision.validateRouteLedger first.Subject [ first; stale ]
            |> Result.isError
        )
        ...
        Assert.True(
            StructuredDecision.validateRouteLedger first.Subject [ first; gap ]
            |> Result.isError
        )
    ...
    let ``M4 checked-in active-item migration replay preserves authority across a body edit`` () =
        let fixturePath =
            Path.Combine(AppContext.BaseDirectory, "fixtures", "structured-decisions", "active-migration.json")
    ...
    let ``M4 review digest binds exact head critic verdict exceptions policy and revision`` () =
        let record =
            review 1 None StructuredDecision.Initial StructuredDecision.Pass 0 None None
    ...
    let ``#2360 acceptance digest binds claim generation and effective base`` () =
        ...
        let otherClaim =
            { accepted with
                ClaimGeneration = Some "claim-2"
            }

        let otherBase =
            { accepted with
                BaseSha = Some(String.replicate 40 "c")
            }
    ...
    let ``#2865 repair-phase receipt is digest-bound and projected from the structured ledger`` () =
        let initial =
            review 1 None StructuredDecision.Initial StructuredDecision.ChangesRequired 0 None None
    ...
    let ``M4 review ledger rejects tamper stale links and non exact SHAs`` () =
        ...
        Assert.True(
            StructuredDecision.validateReviewLedger first.Subject [ first; second ]
            |> Result.isOk
        )
        ...
        let shortHead =
            { shortHead with
                Digest = StructuredDecision.reviewDigest shortHead
            }

        Assert.True(
            StructuredDecision.validateReviewLedger first.Subject [ shortHead ]
            |> Result.isError
        )
    ...
    let private acceptedRoundFourSuccessorChain terminalVerdict =
        ...
        let round1 =
            next 2 initial.Digest 1 StructuredDecision.ChangesRequired "https://review/1"

        let round2 =
            next 3 round1.Digest 2 StructuredDecision.ChangesRequired "https://review/2"
    ...
    let ``2883 acceptance and delivery count repairs rather than passing successor confirmations`` () =
        ...
        let parsed =
            Driver.parseEffectiveReviewComments (String.replicate 40 "a") passing
            |> expectOk
        ...
        Assert.DoesNotContain(
            ...
            Driver.validateReviewChain Protocol.reviewPolicy.MaxAutomatedRepairRounds { parsed with ChecksGreen = true }
        )
        ...
        let legacy verdict : Driver.ReviewComment =
            {
                Id = 1L
                Url = "https://review/v1"
                Body = legacyBody verdict
            }

        let pass = Driver.liveReviewComments record.HeadSha [ legacy "pass"; structured ]

        let conflict =
            Driver.liveReviewComments record.HeadSha [ legacy "changes-required"; structured ]
    ...
    let ``M4 accepted structured generation is retired after head movement and fresh review`` () =
        let seal (record: StructuredDecision.ReviewRecord) =
            { record with
                Digest = StructuredDecision.reviewDigest { record with Digest = "" }
            }
        ...
        let acceptedB =
            { review
                  4
                  (Some initialB.Digest)
                  StructuredDecision.Acceptance
                  StructuredDecision.Accepted
                  0
                  (Some "https://review/3")
                  (Some "https://review/3") with
                HeadSha = initialB.HeadSha
                Critic = initialB.Critic
                Digest = ""
            }
            |> seal
        ...
        let unmoved =
            { initialB with
                HeadSha = initialA.HeadSha
                Digest = ""
            }
            |> seal

        Assert.True(
            StructuredDecision.validateReviewLedger initialA.Subject [ initialA; acceptedA; unmoved ]
            |> Result.isError
        )
    ...
    let m6_structured_chain_drives_effective_state () =
        ...
        let comments =
            [
                reviewComment 1L initial
                reviewComment 2L confirmation
                reviewComment 3L acceptance
            ]

        let parsed =
            Driver.parseEffectiveReviewComments confirmation.HeadSha comments |> expectOk
    ...
    let m6_v1_prose_chain_is_inert () =
        ...
        Assert.True(
            Driver.parseEffectiveReviewComments (String.replicate 40 "0") [ prose ]
            |> Result.isError
        )
    ...
    let m6_structured_fields_are_content_addressed () =
        ...
        let mutations =
            [
                { original with
                    Subject = "FS-GG/.github#42/pr/78"
                }
                { original with
                    RouteApplicability = "meaningful"
                }
                { original with
                    RouteEvidence = [ "changed" ]
                }
                { original with
                    Kind = StructuredDecision.Escalation
                }
                { original with Round = 1 }
                { original with
                    InitialReview = Some "https://review/0"
                }
                { original with
                    PrecedingReview = Some "https://review/0"
                }
                { original with
                    DiffAuditRequired = true
                }
                { original with
                    DiffAuditReceipts = [ "receipt" ]
                }
                { original with
        ...
        mutations
        |> List.iter (fun changed -> Assert.NotEqual<string>(original.Digest, StructuredDecision.reviewDigest changed))
    ...
    let m6_ledger_refuses_gaps_stale_links_subjects_and_critics () =
        ...
        let invalid =
            [
                ...
                { confirmation with
                    PreviousDigest = Some(String.replicate 64 "0")
                }
                |> reseal
                { confirmation with
                    Subject = "FS-GG/.github#42/pr/78"
        ...
        |> List.iter (fun record ->
            Assert.True(
                StructuredDecision.validateReviewLedger initial.Subject [ initial; record ]
                |> Result.isError
            ))
    ...
    let ``2662 a granted successor records under its own identity on confirmation escalation and repair-phase`` () =
        for kind in
            ...
            Assert.True(
                StructuredDecision.validateReviewLedger initial.Subject [ initial; appended ]
                |> Result.isOk
            )
    ...
    let ``2662 the succession is legible from the record alone and survives the wire`` () =
        ...
        let decoded =
            Driver.decodeStructuredReview (Driver.encodeStructuredReview appended)
            |> expectOk
    ...
    let ``2662 an absent grant contributes nothing to the digest`` () =
        ...
        for field in
            ...
            Assert.NotEqual<string>(
                StructuredDecision.reviewDigest
                    { ordinary with
                        Succession = Some(grant "tern-42")
                    },
                StructuredDecision.reviewDigest
                    { ordinary with
                        Succession = Some field
                    }
            )
    ...
    let ``2662 a succession record fails closed against an engine that predates the field`` () =
        let initial, appended =
            successionChain StructuredDecision.Confirmation "snipe-8934" id
        ...
        let asPreFieldEngineWouldStore =
            { appended with
                Digest = preFieldDigest
            }
    ...
    let ``2756 a critic change outside a repaired-head boundary is refused`` () =
        ...
        | Error errors -> Assert.Contains("every record in one review generation must bind the same critic", errors)
        ...
        let cases =
            [
                ...
                (fun (record: StructuredDecision.ReviewRecord) ->
                    { record with
                        Succession = Some(grant "somebody-else")
                    })
                ...
                (fun record ->
                    { record with
                        Succession = Some { grant "tern-42" with GrantUrl = "" }
                    })
                ...
                (fun record ->
                    { record with
                        Succession =
                            Some
                                { grant "tern-42" with
                                    GrantedBy = "fsgg-critic-best"
                                }
                    })
                "the record changes no critic at all", (fun record -> { record with Critic = "tern-42" })
            ]

        for label, mutate in cases do
            let initial, appended =
                successionChain StructuredDecision.Confirmation "snipe-8934" mutate
    ...
    let ``2662 a grant belongs to no initial and no acceptance record`` () =
        ...
        Assert.True(
            StructuredDecision.validateReviewLedger initial.Subject [ initial ]
            |> Result.isError
        )
        ...
        Assert.True(
            StructuredDecision.validateReviewLedger clean.Subject [ clean; acceptance ]
            |> Result.isError
        )

    [<Fact>]
    let ``2662 the seat changes hands so the host acceptance and a second grant bind the successor`` () =
        let initial, confirmation =
            successionChain StructuredDecision.Confirmation "snipe-8934" id
    ...
    let m6_generic_critic_and_wrong_acceptance_links_fail () =
        ...
        let genericAcceptance =
            { acceptance with
                PreviousDigest = Some generic.Digest
                Critic = generic.Critic
                Digest = ""
            }
            |> reseal

        Assert.True(
            Driver.parseReviewComments [ reviewComment 1L generic; reviewComment 2L genericAcceptance ]
            |> Result.isError
        )
        ...
        for changed in
            [
                ...
                { acceptance with
                    InitialReview = Some "https://review/wrong"
                }
                { acceptance with
                    PrecedingReview = Some "https://review/wrong"
                }
            ] do
            let changed = { changed with Digest = "" } |> reseal

            Assert.True(
                Driver.parseReviewComments [ reviewComment 1L initial; reviewComment 2L changed ]
                |> Result.isError
            )
    ...
    let m6_typed_escalation_and_repair_drive_state () =
        ...
        let comments =
            [
                reviewComment 1L initial
                reviewComment 2L escalation
                reviewComment 3L repair
                reviewComment 4L confirmation
                reviewComment 5L acceptance
            ]
    ...
    let m6_missing_malformed_and_misplaced_audit_fails () =
        ...
        let accepted =
            { acceptance with
                PreviousDigest = Some required.Digest
                Digest = ""
            }
            |> reseal

        Assert.True(
            Driver.parseReviewComments [ reviewComment 1L required; reviewComment 2L accepted ]
            |> Result.isError
        )

        let malformed =
            { accepted with
                DiffAuditReceipts = [ "not-base64" ]
                Digest = ""
            }
            |> reseal

        Assert.True(
            Driver.parseReviewComments [ reviewComment 1L required; reviewComment 2L malformed ]
            |> Result.isError
        )
        ...
        Assert.True(
            StructuredDecision.validateReviewLedger required.Subject [ misplaced ]
            |> Result.isError
        )

    /// One well-formed diff-audit receipt bound to `headSha`, honest about the pair it names.
    let private auditReceipt headSha =
        let occurrence: SemanticDiff.Occurrence =
            {
                Id = "occ-1"
                Path = "src/FS.GG.Coord.Core/Driver.fs"
                Line = 1
                Classification = SemanticDiff.StringLiteral
                Confidence = 100
                Before = "OldToken"
                After = "NewToken"
                Disposition = SemanticDiff.IntendedContractChange
            }
        ...
        SemanticDiff.receipt
            ...
            "OldToken"
            "NewToken"
            [ "src/FS.GG.Coord.Core/Driver.fs" ]
            true
            [ occurrence ]
        |> SemanticDiff.toBase64

    /// A generation whose INITIAL record sets `diffAuditRequired: true`, with the acceptance carrying a
    /// well-formed receipt bound to `receiptHead`.
    let private requiredAuditChain receiptHead =
        let initial, acceptance, _ = acceptedChain ()
        ...
        let accepted =
            { acceptance with
                PreviousDigest = Some required.Digest
                DiffAuditReceipts = receiptHead |> Option.map auditReceipt |> Option.toList
                Digest = ""
            }
            |> reseal

        required, [ reviewComment 1L required; reviewComment 2L accepted ]

    /// .github#2694. The facts-free parser used to answer "the live delivery facts needed to check a
    /// diff-audit receipt are absent" — a fact about the CALLER — as a refusal of the SUBJECT's receipt.
    /// `review record` seals a host acceptance through `parseEffectiveReviewComments`, which is that
    /// spelling, so a generation whose initial record set the flag could never be accepted, and the
    /// fresh-`initial` escape is closed until an acceptance exists. It cost PR #2682 a whole review chain.
    [<Fact>]
    let ``2694 a required diff audit with a correct receipt is acceptable on the facts-free path`` () =
        let required, comments = requiredAuditChain (Some(String.replicate 40 "a"))

        let chain = Driver.parseReviewComments comments |> expectOk
        Assert.True chain.HostAccepted
        Assert.True chain.DiffAuditRequired
        // Receipt-intrinsic and therefore still decided: which head the receipts bind.
        Assert.Equal(Some required.HeadSha, chain.DiffAuditHead)

        // The exact spelling `review record` validates a host-acceptance write with.
        Assert.True(Driver.parseEffectiveReviewComments required.HeadSha comments |> Result.isOk)

        // And the wedge does not merely MOVE into the delivery boundary's structural read.
        Assert.False(
            Driver.validateReviewChainStructure 3 { chain with ChecksGreen = true }
            |> List.exists (fun problem -> problem.Contains "diff-audit")
        )

    /// The companion non-vacuity leg: the repair withdraws only the verdict the parser could not support,
    /// never the refusals that are decidable from the receipts themselves. A corpus of REAL offenders,
    /// each still refused on the same facts-free path the test above now passes on.
    [<Fact>]
    let ``2694 the facts-free path still refuses every receipt defect it can decide`` () =
        let reviewedHead = String.replicate 40 "a"
        let strayHead = String.replicate 40 "c"

        // Absent, and malformed: both decidable without live facts, both still refused.
        Assert.True(requiredAuditChain None |> snd |> Driver.parseReviewComments |> Result.isError)

        let required, _ = requiredAuditChain None
        let _, wellFormed = requiredAuditChain (Some reviewedHead)
        Assert.Equal(required.HeadSha, reviewedHead)
        ...
        let malformedComments =
            ...
            let accepted =
                { acceptance with
                    PreviousDigest = Some req.Digest
                    DiffAuditReceipts = [ "not-base64" ]
                    Digest = ""
                }
                |> reseal

            [ reviewComment 1L req; reviewComment 2L accepted ]

        Assert.True(Driver.parseReviewComments malformedComments |> Result.isError)
        Assert.True(Driver.parseReviewComments wellFormed |> Result.isOk)

        // A receipt bound to a head this generation never reviewed is stale, and the structural read that
        // `Delivery.reviewProblem` and `Review.acceptanceOutcome` consume is where that is refused — the
        // parser resolves the head, `reviewChainProblems` compares it. Splitting it that way is why the
        // parser can stop rendering a verdict without any stale receipt becoming acceptable.
        let stray =
            requiredAuditChain (Some strayHead)
            |> snd
            |> Driver.parseReviewComments
            |> expectOk

        Assert.Equal(Some strayHead, stray.DiffAuditHead)
    ...
    let ``2694 no inventory renders no verdict but a supplied empty inventory refuses a stale receipt`` () =
        ...
        | Error errors ->
            Assert.Contains(
                errors,
                fun (problem: string) -> problem.Contains "stale or does not match live delivery facts"
            )
    ...
    let ``2694 the refused second initial names the successor pull request route`` () =
        ...
        | Error errors ->
            let refusal =
                errors
                |> List.tryFind (fun (problem: string) ->
                    problem.Contains "a new initial review is allowed only after host acceptance")
    ...
    let m6_moved_head_parses_only_new_live_generation () =
        ...
        let comments =
            [
                reviewComment 1L initialA
                reviewComment 2L acceptanceA
                reviewComment 3L initialB
                reviewComment 4L acceptanceB
            ]

        let parsed =
            Driver.parseEffectiveReviewComments initialB.HeadSha comments |> expectOk
