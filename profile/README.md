...
develop on. **FS-GG** is the split: the rendering product and the governance
tooling live in separate repositories, each using standard
...
governance tooling to build, test, document, package, or release. A contributor
should be able to clone a product repo, read its Spec Kit artifacts, run the
documented commands, and ship a change without learning a custom platform.
