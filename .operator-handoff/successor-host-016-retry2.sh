#!/usr/bin/env bash
# One bounded retry after a separate, complete 0.1.3 backup proved the repaired config.
set -euo pipefail
...
old_id=successor-013-to-016-retry-20260923-1
new_id=successor-013-to-016-retry-20260923-2
...
if not (pathlib.Path(journal['candidateDeployment']).is_file()
        ...
        and (state / 'prior-0.1.6-failed-rolled-back.json').is_file()):
    raise SystemExit('refused: retained candidate or older failure evidence missing')
config_dir = pathlib.Path(values['TELEMETRY_CONFIG_DIR'])
...
# Preserve the first retry's failed journal before this command replaces active.json.
...
target = state / 'prior-0.1.6-retry1-failed-rolled-back.json'
if target.exists():
...
state = root / 'update-state'
old = json.loads((state / 'prior-0.1.6-retry1-failed-rolled-back.json').read_text())
