...
| `rendering` | `fs-gg-ui-template` | `0.27.0` | `0.27.0` | `rendering:0.27.0` | dotnet new fs-gg-ui (template/base) + FS.GG.UI.* framework packages |
...
| `github` | `coord-engine` | `0.71.0` | `0.73.1` | `github:0.73.1` | the `fsgg-coord-engine` CLI verb surface (claim/take/batch/who/widen/set-paths/say/landable/done/release/flush/…) + its exit-code contract, emitted from src/FS.GG.Coord.Core/Protocol.fs; shipped as the FS.GG.Coord.Cli dotnet tool |
...
| `rendering` | `rendering-skills` | `0.1.0` | `0.1.0` | `rendering:0.1.0` | FS.GG.Rendering.Skills package — Rendering-owned product skills, materialized by SDD's production scaffold materializer for every provider |
