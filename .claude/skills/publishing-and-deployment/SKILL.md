...
| `rendering` | `fs-gg-ui-template` | `0.28.0` | `0.28.0` | `rendering:0.28.0` | dotnet new fs-gg-ui (template/base) + FS.GG.UI.* framework packages |
| `game` | `game-sim-core` | `0.14.0` | `0.14.0` | `game:0.14.0` | FS.GG.Game.Core package — BCL-only deterministic simulation primitives plus the packaged fs-gg-game-core-fable-lockstep-v1 bounded Fable source/profile and canonical oracle |
| `game` | `game-scene-adapter` | `0.14.0` | `0.14.0` | `game:0.14.0` | FS.GG.Game.Render package — Adapter (sim-state -> FS.GG.UI.Scene drawables) |
...
| `github` | `coord-engine` | `0.75.4` | `0.75.4` | `github:0.75.4` | the `fsgg-coord-engine` CLI verb surface (claim/take/batch/who/widen/set-paths/say/landable/done/release/flush/…) + its exit-code contract, emitted from src/FS.GG.Coord.Core/Protocol.fs; shipped as the FS.GG.Coord.Cli dotnet tool |
| `github` | `new-sdd-workspace` | `0.10.1` | `0.10.1` | `github:0.10.1` | the `new-sdd-workspace` scaffolder CLI (package FS.GG.NewSddWorkspace) — one-command full-stack SDD workspace creation, wrapping the FS.GG.Templates `rendering` provider (ADR-0016); shipped as a dotnet tool |
| `templates` | `fs-gg-workspace-template` | `0.10.0` | `0.10.0` | `templates:0.10.0` | FS.GG.Workspace.Template package, four `dotnet new` identities: `fs-gg-console` (provider `console` — minimal F# executable, no npm lane, ADR-0072 §2); `fs-gg-web` (provider `web` — F# ASP.NET Core + plain TypeScript/Vite website, ADR-0071 §3); `fs-gg-fable-game` (provider `fable-game` — F# ASP.NET Core + Fable/Elmish game client, SignalR for connection-oriented real-time traffic + plain HTTP endpoints with explicit versioned DTOs for typed request/response per ADR-0073, consuming the published `fs-gg-game-core-fable-lockstep-v1` profile per ADR-0069); `fs-gg-fable-bindings` (provider `fable-bindings` — Fable interop library over an exactly pinned npm package/declaration closure, ADR-0072 §3, first proved by the unreleased EHotwagner/babylonjsBindings reference). All four also carry `fs-gg-governance` (the pre-existing governance overlay template, folded into this package at the same rename).  |
