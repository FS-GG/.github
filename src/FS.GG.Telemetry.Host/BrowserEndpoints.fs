...
module BrowserEndpoints =
    ...
    let private headers (context: HttpContext) privateResponse =
        ...
        if privateResponse then
            context.Response.Headers[HeaderNames.CacheControl] <- "no-store"
        else
            context.Response.Headers[HeaderNames.CacheControl] <- "no-cache"

    let private json context status code =
        task {
            ...
            let bytes =
                JsonSerializer.SerializeToUtf8Bytes
                    {|
                        schema = "fsgg.telemetry.browser-error/1"
                        code = code
                    |}
    ...
    let private hostMatches (options: BrowserOptions) (context: HttpContext) =
        ...
        && String.Equals(context.Request.Host.Value, options.PublicOrigin.Authority, StringComparison.OrdinalIgnoreCase)
    ...
    let private readBounded (context: HttpContext) (token: CancellationToken) =
        task {
            if
                context.Request.ContentLength.HasValue
                && context.Request.ContentLength.Value > int64 MaxRequestBytes
            then
                return None
            else
    ...
    let private cookie (context: HttpContext) =
        ...
        | true, value when value.Length = 43 -> Some value
        | _ -> None
    ...
    let private sessionResponse (context: HttpContext) (identity: BrowserIdentity) =
        task {
            headers context true
            ...
            let bytes =
                JsonSerializer.SerializeToUtf8Bytes
                    {|
                        ...
                        workspaces = identity.WorkspaceIds |> Set.toArray
                    |}
    ...
    let private identity (security: BrowserSecurity.Service) context =
        cookie context
        |> Option.bind (fun value -> security.Validate(value, DateTimeOffset.UtcNow))
    ...
    let private exactObject (element: JsonElement) (names: string array) =
        if element.ValueKind <> JsonValueKind.Object then
            ...
            && actual |> Array.distinct |> Array.length = actual.Length
            && Set.ofArray actual = Set.ofArray names
    ...
    let private validOptionalId =
        function
        | None -> true
        | Some value -> TelemetryReceipt.validId value
    ...
    let private acceptsGzip (context: HttpContext) =
        ...
        |> Seq.exists (fun value ->
            String.Equals(value.Value.ToString(), "gzip", StringComparison.OrdinalIgnoreCase)
            && (not value.Quality.HasValue || value.Quality.Value > 0.))

    let map
        (endpoints: IEndpointRouteBuilder)
        (options: BrowserOptions)
        (security: BrowserSecurity.Service)
        (snapshot: BrowserSnapshotProvider)
        =
        let options =
            BrowserSecurity.validateOptions options
            |> Result.defaultWith (fun errors -> invalidArg "options" (String.concat ";" errors))

        let staticAsset (route: string) =
            Func<HttpContext, Task>(fun context ->
                task {
                    if not (hostMatches options context) then
                        do! json context 404 "not-found"
                    else
        ...
        endpoints.MapGet("/private/dashboard/", staticAsset "/private/dashboard/")
        |> ignore

        endpoints.MapGet("/private/dashboard/app.js", staticAsset "/private/dashboard/app.js")
        |> ignore

        endpoints.MapGet("/private/dashboard/styles.css", staticAsset "/private/dashboard/styles.css")
        |> ignore
        ...
        endpoints.MapPost(
            ...
            Func<HttpContext, Task>(fun context ->
                task {
                    if
                        not (
                            hostMatches options context
                            && originMatches options context
                            && jsonContent context
                        )
                    then
                        do! json context 400 "invalid-request"
                    else if not (security.TryAcquireLogin()) then
                        do! json context 429 "overload"
                    else
                        try
                            try
                                use deadline =
                                ...
                                | Some bytes ->
                                    try
                                        ...
                                        match
                                            exactObject root [| "principalId"; "accessKey" |],
                                            scalar root "principalId",
                                            scalar root "accessKey"
                                        with
                                        | true, Some principal, Some key ->
                                            match security.LoginAcquired(principal, key, DateTimeOffset.UtcNow) with
                                            | LoginAccepted(session, identity) ->
                                                setCookie options context session
                                                do! sessionResponse context identity
                            ...
                            with :? OperationCanceledException ->
                                if not context.Response.HasStarted then
                                    do! json context 408 "request-timeout"
                        finally
        ...
        endpoints.MapPost(
            "/private/dashboard/v1/logout",
            Func<HttpContext, Task>(fun context ->
                task {
                    if not (hostMatches options context && originMatches options context) then
                        do! json context 400 "invalid-request"
                    else
        ...
        endpoints.MapPost(
            "/private/dashboard/v1/session/refresh",
            Func<HttpContext, Task>(fun context ->
                task {
                    if not (hostMatches options context && originMatches options context) then
                        do! json context 400 "invalid-request"
                        ...
                        match
                            cookie context
                            |> Option.bind (fun value -> security.Rotate(value, DateTimeOffset.UtcNow))
                        with
                        | None -> do! json context 401 "authentication-required"
                        | Some(replacement, identity) ->
                            setCookie options context replacement
                            do! sessionResponse context identity
                })
        )
        |> ignore
            ...
            Func<HttpContext, Task>(fun context ->
                task {
                    ...
                    then
                        ...
                        | None -> do! json context 401 "authentication-required"
                        | Some browser ->
                            if not (security.TryAcquireQuery()) then
                                do! json context 429 "overload"
                            else
                                ...
                                try
                                    try
                                        ...
                                        | Some bytes ->
                                            try
                                                use document =
                                                    JsonDocument.Parse(bytes, JsonDocumentOptions(MaxDepth = 3))
                                                ...
                                                let shape =
                                                    ...
                                                    && names |> Array.distinct |> Array.length = names.Length
                                                    && Set.isSubset
                                                        (Set.ofArray names)
                                                        (set["workspaceId"
                                                             "itemId"])
                                                ...
                                                if
                                                    not shape
                                                    || workspace.IsNone
                                                    || not (TelemetryReceipt.validId workspace.Value)
                                                    || not (validOptionalId item)
                                                then
                                                    do! json context 400 "invalid-request"
                                                elif not (browser.WorkspaceIds.Contains workspace.Value) then
                                                    do! json context 404 "workspace-unavailable"
                                                else
                                                    transferred <- true

                                                    let work: Task<Result<byte array, string list>> =
                                                        Task.Run(fun () -> snapshot workspace.Value item)

                                                    work.ContinueWith(fun (_: Task<Result<byte array, string list>>) ->
                                                        security.ReleaseQuery())
                                                    |> ignore

                                                    let workTask: Task = work :> Task

                                                    let! completed =
                                                        Task.WhenAny(workTask, Task.Delay options.QueryTimeout)

                                                    if not (Object.ReferenceEquals(completed, workTask)) then
                                                        do! json context 503 "query-timeout"
                                                    else
                                                        let outcome =
                                                            if work.IsFaulted || work.IsCanceled then
                                                                None
                                                            else
                                                                Some(work.GetAwaiter().GetResult())

                                                        match outcome with
                                                        | None -> do! json context 503 "query-unavailable"
                                                        | Some(Error _) -> do! json context 404 "workspace-unavailable"
                                                        | Some(Ok response) when response.Length > MaxResponseBytes ->
                                                            do! json context 503 "response-too-large"
                                                        | Some(Ok response) ->
                                                            let compressed = acceptsGzip context
                                                            let body = if compressed then gzip response else response

                                                            if body.Length > MaxResponseBytes then
                                                                do! json context 503 "response-too-large"
                                                            else
                                                                headers context true
                                                                context.Response.StatusCode <- 200
                                                                ...
                                                                context.Response.Headers.ETag <-
                                                                    Microsoft.Extensions.Primitives.StringValues(
                                                                        "\""
                                                                        + Convert
                                                                            .ToHexString(SHA256.HashData response)
                                                                            .ToLowerInvariant()
                                                                        + "\""
                                                                    )
