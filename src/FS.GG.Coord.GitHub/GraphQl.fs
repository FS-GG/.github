...
module GraphQl =
    ...
    let decode (subject: string) (body: string) (decoder: JsonElement -> IoResult<'value>) : IoResult<'value> =
        ...
        | Ok data ->
            ...
            | :? InvalidOperationException as error ->
                Error(Malformed(subject, $"the GraphQL data had an invalid shape: %s{error.Message}"))
            | :? FormatException as error ->
                Error(Malformed(subject, $"the GraphQL data had an invalid value: %s{error.Message}"))
    ...
    let private pageCore
        ...
        if connection.ValueKind <> JsonValueKind.Object then
            Error(
                Malformed(subject, $"%s{what} is not a JSON object (%A{connection.ValueKind}) — this is a FAILED READ")
            )
        else
            match connection.TryGetProperty "nodes", connection.TryGetProperty "pageInfo" with
            | (true, nodes), (true, pageInfo) when
                ...
                | None ->
                    ...
                    if keys |> List.exists String.IsNullOrWhiteSpace then
                        ...
                        Error(
                            Malformed(
                                subject,
                                $"%s{what} repeated an item identity within one page — the connection mutated while it was read"
                            )
                        )
                    else
                        let totalCount =
                        ...
                        | true, hasNext when hasNext.ValueKind = JsonValueKind.True ->
                            ...
                            | true, cursor when
                                ...
                                && not (String.IsNullOrWhiteSpace(cursor.GetString()))
                                ->
                                if items.IsEmpty then
                                    Error(
                                        Malformed(
                                            subject,
                                            $"%s{what} returned an empty page while announcing another page"
                                        )
                                    )
                                else
                                    Ok
                                        {
                                            Items = items
                                            Keys = keys
                                            NextCursor = Some(cursor.GetString())
                                            TotalCount = totalCount
                                        }
                            | _ -> Error(Malformed(subject, $"%s{what} has another page but no usable cursor"))
                        | _ ->
                            Error(
                                Malformed(subject, $"%s{what}'s `pageInfo.hasNextPage` is missing or is not a Boolean")
                            )
    ...
    let page subject what key decodeNode connection =
        pageCore subject what None key decodeNode connection
                        ...
                        let total =
                            ...
                            | Some expected, Some actual when expected <> actual ->
                                Error(
                                    Malformed(
                                        subject,
                                        $"%s{what}'s totalCount changed from %d{expected} to %d{actual} while it was read"
                                    )
                                )
                        ...
                        | Ok stableTotal ->
                            ...
                            | Some duplicate ->
                                Error(
                                    Malformed(
                                        subject,
                                        $"%s{what} repeated item `%s{duplicate}` across pages — the connection mutated while it was read"
                                    )
                                )
                            | None ->
                                ...
                                if all.Length > limits.MaxItems then
                                    Error(
                                        Malformed(
                                            subject,
                                            $"%s{what} exceeded the explicit %d{limits.MaxItems}-item limit"
                                        )
                                    )
                                else
                                    ...
                                    | Some next ->
                                        loop
                                            ...
                                            (pages + 1)
                                            stableTotal
                                            (Set.add next seenCursors)
                                            (Set.union seenKeys (Set.ofList page.Keys))
                                            all
                                    | None ->
                                        ...
                                        | Some expected when expected <> all.Length ->
                                            Error(
                                                Malformed(
                                                    subject,
                                                    $"%s{what} ended with %d{all.Length} items but reported totalCount %d{expected}"
                                                )
                                            )
                                        | _ -> Ok all
    ...
    let partialMutation (subject: string) (body: string) : IoResult<string list * (string * string) list> =
        if String.IsNullOrWhiteSpace body then
            ...
            try
                ...
                if root.ValueKind <> JsonValueKind.Object then
                    Error(Malformed(subject, "the batch response was not an object"))
                else
                    let failures =
                        match root.TryGetProperty "errors" with
