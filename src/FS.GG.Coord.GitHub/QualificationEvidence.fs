...
module QualificationEvidence =
    ...
    let parseHostedSnapshot (bytes: byte array) =
        try
            ...
            let require (expected: string list) (element: JsonElement) (label: string) =
                ...
                let missing, unknown =
                    Set.difference (Set.ofList expected) (keys element),
                    Set.difference (keys element) (Set.ofList expected)

                if not missing.IsEmpty then
                    raise (FormatException($"%s{label} has missing fields"))

                if not unknown.IsEmpty then
                    raise (FormatException($"%s{label} has unknown fields"))

            require [ "schema"; "complete"; "items" ] root "hosted observation"

            if root.GetProperty("schema").GetString() <> HostedSchema then
                raise (FormatException($"hosted schema must be '%s{HostedSchema}'"))

            let itemsElement = root.GetProperty "items"

            if itemsElement.ValueKind <> JsonValueKind.Array then
                raise (FormatException("hosted items must be an array"))
            ...
            let items =
                ...
                |> Seq.mapi (fun index item ->
                    ...
                    let scope =
                        ...
                        | "job" -> Job
                        | "check" -> CheckRun
                        | other -> raise (FormatException($"unknown hosted scope '%s{other}'"))
                    ...
                    let state =
                        ...
                        | "completed", conclusion when
                            conclusion.ValueKind = JsonValueKind.String
                            && not (String.IsNullOrWhiteSpace(conclusion.GetString()))
                            ->
                            Completed(conclusion.GetString())
        ...
        with error ->
            Error [ $"invalid hosted observation JSON: %s{error.Message}" ]
...

        {
            ...
            Checks =
                ...
                |> List.map (fun item ->
                    ...
                    {
                        Qualification.HostedCheck.Scope = scope item.Scope
                        Id = item.Id
                        SubjectRevision = item.HeadSha
                        State = status
    ...
    let private strictObject expected (element: JsonElement) =
        if element.ValueKind <> JsonValueKind.Object then
            Error "obligation payload must be an object"
        else
            let expectedSet = Set.ofList expected
            let observed = element.EnumerateObject() |> Seq.map _.Name |> Set.ofSeq

            let missing, unknown =
                Set.difference expectedSet observed, Set.difference observed expectedSet

            let missingText, unknownText =
                String.concat "," (Set.toList missing), String.concat "," (Set.toList unknown)

            if not missing.IsEmpty then
                Error($"obligation payload is missing fields: %s{missingText}")
            elif not unknown.IsEmpty then
                Error($"obligation payload has unknown fields: %s{unknownText}")
            else
                Ok()
    ...
    let parseObligationReadback (bytes: byte array) =
        try
            ...
            |> Result.bind (fun () ->
                ...
                if schema <> ObligationReadbackSchema then
                    Error($"obligation readback schema must be '%s{ObligationReadbackSchema}'")
                elif commentId <= 0L then
                    Error "obligation readback commentId must be positive"
                elif not validUrl then
                    Error "obligation readback url must be an exact GitHub PR issuecomment URL for commentId"
                elif String.IsNullOrWhiteSpace author then
                    Error "obligation readback author must be non-empty"
                elif String.IsNullOrWhiteSpace body then
                    Error "obligation readback body must be non-empty"
                else
                    Ok
                        {
                            CommentId = commentId
        ...
        with error ->
            Error [ $"invalid obligation readback JSON: %s{error.Message}" ]
