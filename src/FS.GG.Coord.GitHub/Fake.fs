...
module Fake =
    ...
    let private varText (v: Var) =
        ...
        | VId s -> s
        | VDate s -> s
        | VNumber n -> if Double.IsInteger n then string (int64 n) else string n
    ...
    let describe (request: Request) =
        ...
        | _ ->
            ...
            | None ->
                if request.Path.StartsWith "rate_limit" then
                    "rate-limit"
                else
                    $"%s{method.ToLowerInvariant()} %s{request.Path}"
            ...
            | Some(nwo, rest) ->
                ...
                | "actions/runs" ->
                    let sha =
                        request.Query
                        |> List.tryFind (fun (k, _) -> k = "head_sha")
                        |> Option.map snd
                        |> Option.defaultValue ""
