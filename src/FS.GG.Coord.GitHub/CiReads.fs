...
module CiReads =
    ...
    let private int32 (node: JsonElement) (name: string) =
        int64Value node name
    ...
    let collect
        ...
        let send subject path query =
            if calls >= 20 then
                Error(Malformed(subject, "20-request collection budget exhausted"))
            elif timer.Elapsed > TimeSpan.FromSeconds 30.0 then
                Error(Transport "30-second collection deadline exceeded")
            else
                calls <- calls + 1
        ...
        let continuation (subject: string) (link: string) =
            try
                ...
                if
                    ...
                    || not (uri.AbsolutePath.StartsWith(prefix, StringComparison.Ordinal))
                then
                    ...
                    let query =
                        ...
                        |> Array.map (fun pair ->
                            let parts = pair.Split('=', 2) in

                            Uri.UnescapeDataString(parts[0]),
                            Uri.UnescapeDataString(if parts.Length = 2 then parts[1] else ""))
                        |> Array.toList

                    Ok(uri.AbsolutePath.TrimStart('/'), query)
            with :? UriFormatException ->
                Error(Malformed(subject, "pagination continuation is not a URI"))

        let readObject subject path query =
            send subject path query
            |> Result.bind (fun response -> parse subject response.Body |> Result.map (fun doc -> response, doc))
        ...
        | Ok(_, prDoc) ->
            ...
            let isMergeGroup =
                ...
                | true, value when value.ValueKind = JsonValueKind.String ->
                    value.GetString() = head && prHead <> Some head
                | _ -> false
            ...
            if prHead <> Some head || isMergeGroup then
                Ok
                    {
                        ...
                        Workflow = workflow
                        Binding =
                            if isMergeGroup then
                                "merge-group-unsupported"
                            else
                                "unresolved"
                        Pages = []
                        Runs = []
                        ...
                        AttemptCoverage = "unknown"
                        JobPageCoverage = "unknown"
                        TerminalCoverage = "unknown"
                        TimestampCoverage = "unknown"
                ...
                let rec readRunPage page path query =
                    ...
                    |> Result.bind (fun (response, document) ->
                        ...
                        | Some total, (true, array) when
                            array.ValueKind = JsonValueKind.Array
                            && array.GetArrayLength() <= 100
                            && total <= 1000
                            ->
                            pages.Add
                                {
                            ...
                            for item in array.EnumerateArray() do
                                match
                                    int64Value item "id",
                                    int32 item "run_attempt",
                                    str item "path",
                                    str item "event",
                                    str item "head_sha",
                                    str item "status"
                                with
                                | Some id, Some attempt, Some path, Some event, Some sha, Some status when
                                    sha = head
                                    && (event = "pull_request"
                                    ...
                                    let prs =
                                        ...
                                        | true, values when values.ValueKind = JsonValueKind.Array ->
                                            ...
                                            |> Seq.choose (fun value -> int32 value "number")
                                            |> Seq.toList
                                        | _ -> []
                                    ...
                                    if not prs.IsEmpty && not (List.contains pr prs) then
                                        ...
                                        | true, existing when existing <> row ->
                                            problem <- Some(Malformed(subject, "conflicting duplicate run identity"))
                                        | _ -> runs[key] <- row
                                | _ ->
                                    problem <-
                                        Some(Malformed(subject, "run row is malformed, wrong-head, or unsupported"))
                            ...
                            | None ->
                                ...
                                | None ->
                                    Error(
                                        Malformed(
                                            subject,
                                            $"run inventory reports %d{total} but collected %d{runs.Count}"
                                        )
                                    )
                                | Some link when not (seenLinks.Add link) ->
                                    Error(Malformed(subject, "pagination continuation loop"))
                                | Some link ->
                                    continuation subject link
                                    |> Result.bind (fun (nextPath, nextQuery) ->
                                        readRunPage (page + 1) nextPath nextQuery)
                        | _ -> Error(Malformed(subject, "run page count/shape is invalid or exceeds 1000")))

                let readRuns =
                    readRunPage
                        1
                        $"repos/%s{owner}/%s{repo}/actions/workflows/%s{Uri.EscapeDataString workflow}/runs"
                        [ "head_sha", head; "per_page", "100"; "page", "1" ]

                let rec readJobPage (run: Run) page path query =
                    let subject =
                        $"%s{repository} run %d{run.Id} attempt %d{run.Attempt} jobs page %d{page}"
                    ...
                    |> Result.bind (fun (response, document) ->
                        ...
                        | Some total, (true, array) when
                            ...
                            pages.Add
                                {
                                    Resource = $"jobs:%d{run.Id}:%d{run.Attempt}"
                                    Index = page
                                    Count = array.GetArrayLength()
                                    Total = total
                                }
                            ...
                            for item in array.EnumerateArray() do
                                ...
                                | Some id, Some name, Some status ->
                                    let steps =
                                        ...
                                        | true, values when values.ValueKind = JsonValueKind.Array ->
                                            ...
                                            |> Seq.choose (fun step ->
                                                ...
                                                | Some number, Some stepName, Some stepStatus ->
                                                    Some
                                                        {
                                                            Number = number
                                                            Name = stepName
                                    ...
                                    let checkRunId =
                                        ...
                                        |> Option.bind (fun value ->
                                            match Int64.TryParse(value.TrimEnd('/').Split('/') |> Array.last) with
                                            | true, id -> Some id
                                            | _ -> None)
                                    ...
                                    let row =
                                        {
                                            ...
                                            RunnerName = str item "runner_name"
                                    ...
                                    | true, existing when existing <> row ->
                                        problem <- Some(Malformed(subject, "conflicting duplicate job identity"))
                                    | _ -> jobs[key] <- row
                            ...
                            | None ->
                                let collected =
                                    jobs.Values
                                    |> Seq.filter (fun job -> job.RunId = run.Id && job.Attempt = run.Attempt)
                                    |> Seq.length
                                ...
                                | None ->
                                    Error(
                                        Malformed(
                                            subject,
                                            $"job inventory reports %d{total} but collected %d{collected}"
                                        )
                                    )
                                | Some link when not (seenLinks.Add link) ->
                                    Error(Malformed(subject, "pagination continuation loop"))
                                | Some link ->
                                    continuation subject link
                                    |> Result.bind (fun (nextPath, nextQuery) ->
                                        readJobPage run (page + 1) nextPath nextQuery)
                        | _ -> Error(Malformed(subject, "job page count/shape is invalid or exceeds 1000")))

                let finish complete diagnostic =
                    let runRows =
                        runs.Values |> Seq.sortBy (fun run -> run.Id, run.Attempt) |> Seq.toList

                    let jobRows =
                        jobs.Values
                        |> Seq.sortBy (fun job -> job.RunId, job.Attempt, job.Id)
                        |> Seq.toList

                    let terminal =
                        runRows
                        |> List.forall (fun run -> run.Status = "completed" && run.Conclusion.IsSome)

                    let timestamps =
                        jobRows
                        |> List.forall (fun job ->
                            job.Status <> "completed" || (job.StartedAt.IsSome && job.CompletedAt.IsSome))
                ...
                | Error(Malformed(_, detail) as error) when
                                ...
                                (fun state expected ->
                                    ...
                                    |> Result.bind (fun () ->
                                        ...
                                        |> Result.bind (fun (_, document) ->
                                            ...
                                            | Some _ ->
                                                Error(Malformed(subject, "run attempt increased during collection"))
                                            | None ->
                                                Error(Malformed(subject, "run attempt verification is malformed")))))
                                (Ok())
    ...
    let discoverPopulation
        ...
        let send subject path query =
            if calls >= callLimit then
                Error(Malformed(subject, "request-budget-exhausted"))
            elif timer.Elapsed > TimeSpan.FromSeconds 30.0 then
                Error(Transport "collection-deadline-exceeded")
            else
                calls <- calls + 1
        ...
        let read subject path query =
            ...
            |> Result.bind (fun response ->
                parse subject response.Body |> Result.map (fun document -> response, document))
            ...
            try
                ...
                then
                    ...
                    let query =
                        uri.Query.TrimStart('?').Split('&', StringSplitOptions.RemoveEmptyEntries)
                        |> Array.map (fun pair ->
                            let values = pair.Split('=', 2) in

                            Uri.UnescapeDataString(values[0]),
                            Uri.UnescapeDataString(if values.Length = 2 then values[1] else ""))
                        |> Array.toList
        ...
        let rec runPages page path query =
            ...
            read subject path query
            |> Result.bind (fun (response, document) ->
                use document = document

                match
                    int32 document.RootElement "total_count", document.RootElement.TryGetProperty "workflow_runs"
                with
                | Some total, (true, array) when
                    array.ValueKind = JsonValueKind.Array
                    && array.GetArrayLength() <= 100
                    ...
                    | None ->
                        ...
                        | Some link when not (seen.Add link) ->
                            Error(Malformed(subject, "pagination continuation loop"))
                        | Some link ->
                            continuation subject link
                            |> Result.bind (fun (nextPath, nextQuery) -> runPages (page + 1) nextPath nextQuery)
                | _ -> Error(Malformed(subject, "run page count/shape is invalid or exceeds 1000")))
            ...
            |> Result.bind (fun (response, document) ->
                ...
                | Some total, (true, array) when
                    ...
                    let results =
                        ...
                        |> Seq.map (parseJob subject run.Id run.Attempt)
                        |> Seq.toList
                    ...
                    | None ->
                        for row in
                            ...
                            |> List.choose (function
                                | Ok row -> Some row
                                | _ -> None) do
                            jobs[(row.RunId, row.Attempt, row.Id)] <- row

                        let count =
                            jobs.Values
                            |> Seq.filter (fun row -> row.RunId = run.Id && row.Attempt = run.Attempt)
                            |> Seq.length
                        ...
                        | Some link ->
                            continuation subject link
                            |> Result.bind (fun (nextPath, nextQuery) -> jobPages run (page + 1) nextPath nextQuery)
                | _ -> Error(Malformed(subject, "job page count/shape is invalid or exceeds 1000")))
            ...
            |> Result.bind (fun (response, document) ->
                ...
                | Some total, (true, array) when
                    ...
                    for item in array.EnumerateArray() do
                        ...
                        | Some id, Some name, Some status ->
                            let slug =
                                match item.TryGetProperty "app" with
                                | true, app when app.ValueKind = JsonValueKind.Object -> str app "slug"
                                | _ -> None
                            ...
                            match checks.TryGetValue id with
                            | true, existing when existing <> row -> gaps.Add("conflicting-check-identity")
                            | _ -> checks[id] <- row
                    ...
                    | Some link ->
                        continuation subject link
                        |> Result.bind (fun (nextPath, nextQuery) -> checkPages (page + 1) nextPath nextQuery)
        ...
        | Ok(_, document) ->
            ...
            let currentHead =
                match document.RootElement.TryGetProperty "head" with
                | true, value when value.ValueKind = JsonValueKind.Object -> str value "sha"
                | _ -> None
            ...
            let witnessed =
                currentHead = Some head
                && currentBaseRef = Some baseRef
                && currentBaseSha = Some baseSha

            if not witnessed && not admitted then
                let code =
                    if currentHead <> Some head then "head-not-admitted"
                    elif currentBaseRef <> Some baseRef then "base-ref-mismatch"
                    else "base-sha-mismatch"
                ...
                Ok
                    {
                        ...
                        Checks = []
                        CheckCoverage = "unknown"
                        ExternalChecks = 0
                        AdmissionWitness = false
                        Revision = 0L
                        Pending = []
                        Gaps = [ code ]
                    }
            else
                if admitted && (currentBaseRef <> Some baseRef || currentBaseSha <> Some baseSha) then
                    gaps.Add "admitted-base-moved"
                ...
                for latest in
                    listed
                    |> Seq.groupBy _.Id
                    |> Seq.map (snd >> Seq.maxBy _.Attempt)
                    |> Seq.sortBy _.Id do
                        ...
                        let run =
                            ...
                            | _ ->
                                ...
                                match
                                    read
                                        subject
                                        $"repos/%s{owner}/%s{repo}/actions/runs/%d{latest.Id}/attempts/%d{attempt}"
                                        []
                                with
                                | Ok(_, doc) ->
                                    use doc = doc in
                        ...
                        | Some row ->
                            match
                                jobPages
                                    row
                                    1
                                    $"repos/%s{owner}/%s{repo}/actions/runs/%d{row.Id}/attempts/%d{row.Attempt}/jobs"
                                    [ "per_page", "100"; "page", "1" ]
                            with
                            | Ok() -> ()
                            | Error _ ->
                                jobsComplete <- false
                                pending.Add($"jobs:%d{row.Id}:%d{row.Attempt}")
                        | None -> ()

                match
                    checkPages
                        1
                        $"repos/%s{owner}/%s{repo}/commits/%s{head}/check-runs"
                        [ "per_page", "100"; "page", "1" ]
                with
                | Ok() -> ()
                | Error _ ->
                    checksComplete <- false
                    pending.Add "check-runs"
                ...
                if inventoryComplete then
                    ...
                    | Ok(_, doc) ->
                        ...
                        match int32 doc.RootElement "total_count" with
                        | Some count when count = listed.Length -> ()
                        | _ ->
                            inventoryComplete <- false
                            pending.Add "actions-runs-changed"
                    | Error _ ->
                        inventoryComplete <- false
                        pending.Add "actions-runs-unverified"
                ...
                if checksComplete then
                    ...
                    | Ok(_, doc) ->
                        ...
                        match int32 doc.RootElement "total_count" with
                        | Some count when count = checks.Count -> ()
                        | _ ->
                            checksComplete <- false
                            pending.Add "check-runs-changed"
                    | Error _ ->
                        checksComplete <- false
                        pending.Add "check-runs-unverified"

                let actionCheckIds =
                    checks.Values
                    |> Seq.filter (fun row -> row.AppSlug = Some "github-actions")
                    |> Seq.map _.Id
                    |> Set.ofSeq
                ...
                if external > 0 then
                    gaps.Add "external-checks-not-attributed"

                if
                    checksComplete
                    && (actionCheckIds <> jobCheckIds || jobs.Values |> Seq.exists _.CheckRunId.IsNone)
                then
                    checksComplete <- false
                    pending.Add "workflow-check-inventory-mismatch"

                let runRows =
                    runs.Values |> Seq.sortBy (fun row -> row.Id, row.Attempt) |> Seq.toList

                let jobRows =
                    jobs.Values
                    |> Seq.sortBy (fun row -> row.RunId, row.Attempt, row.Id)
                    |> Seq.toList

                let terminal =
                    runRows
                    |> List.forall (fun row -> row.Status = "completed" && row.Conclusion.IsSome)

                let timestamps =
                    jobRows
                    |> List.forall (fun row ->
                        row.Status <> "completed" || (row.StartedAt.IsSome && row.CompletedAt.IsSome))
                ...
                let complete =
                    ...
                    && jobsComplete
                    && checksComplete
                    && (gaps |> Seq.forall (fun code -> code = "external-checks-not-attributed"))

                let latestTimestamp =
                    runRows
                    |> List.choose _.UpdatedAt
                ...
                let diagnostic =
                    if complete then
                        ...
                        pending |> Seq.tryHead |> Option.orElseWith (fun () -> gaps |> Seq.tryHead)
                ...
                let snapshot =
                    {
                        ...
                        Jobs = jobRows
                        InventoryCoverage =
                            (if inventoryComplete && checksComplete then
                                 "complete"
                             else
                                 "partial")
