...
module OperationalGraphQl =
    ...
    let private projectsDocument =
        "query($owner:String!,$cursor:String){organization(login:$owner){projectsV2(first:100,after:$cursor){totalCount pageInfo{hasNextPage endCursor} nodes{id title public number}}} rateLimit{cost remaining}}"
    ...
    let private projects (transport: IGitHubTransport) (owner: string) =
        ...
        let fetch cursor =
            let variables =
                [ "owner", VString owner ]
                @ (cursor
                   |> Option.map (fun c -> [ "cursor", VString c ])
                   |> Option.defaultValue [])
                                    ...
                                    Ok
                                        {
                                            ...
                                            Public = node.GetProperty("public").GetBoolean()
                                            Number = node.GetProperty("number").GetInt32()
                                        }
                                with _ ->
                                    malformed subject "a project node omitted id/title/public/number")
                            connection)
    ...
    let projectId (transport: IGitHubTransport) (owner: string) (number: int) =
        ...
        let document =
            "query($owner:String!,$number:Int!){organization(login:$owner){projectV2(number:$number){id title}} rateLimit{cost remaining}}"

        GraphQl.read
            transport
            (request subject document [ "owner", VString owner; "number", VNumber(float number) ])
            (fun data ->
                match requiredObject subject "organization" data with
                | Error error -> Error error
                | Ok org ->
                    ...
                    | Error error -> Error error
                    | Ok project ->
                        match stringOption "id" project with
                        | Some id when not (String.IsNullOrWhiteSpace id) -> Ok id
                        | _ -> malformed subject "project id was missing")

    let repositoryPolicy (transport: IGitHubTransport) (owner: string) (name: string) =
        let subject = $"%s{owner}/%s{name} repository policy"

        let document =
            "query($owner:String!,$name:String!){repository(owner:$owner,name:$name){id issueCreationPolicy hasIssuesEnabled mergeCommitAllowed squashMergeAllowed rebaseMergeAllowed} rateLimit{cost remaining}}"
        ...
        GraphQl.read transport (request subject document [ "owner", VString owner; "name", VString name ]) (fun data ->
            ...
            | Ok repo ->
                ...
                with _ ->
                    malformed subject "repository policy fields were incomplete")
    ...
    let issueIntakeIdentity (transport: IGitHubTransport) owner name number =
        ...
        let document =
            "query IntakeIdentity($owner:String!,$name:String!,$number:Int!){repository(owner:$owner,name:$name){issue(number:$number){id updatedAt author{login ... on Node{id}}}} rateLimit{cost remaining}}"
        ...
        GraphQl.read
            ...
            (fun data ->
                try
                    ...
                    let value =
                        {
                            IssueId = issue.GetProperty("id").GetString()
                            UpdatedAt = issue.GetProperty("updatedAt").GetString()
                            AuthorId = author.GetProperty("id").GetString()
                            AuthorLogin = author.GetProperty("login").GetString()
                        }

                    if
                        [ value.IssueId; value.UpdatedAt; value.AuthorId; value.AuthorLogin ]
                        |> List.exists String.IsNullOrWhiteSpace
                    then
                        malformed subject "issue identity, revision, or author was missing"
                    else
                        Ok value
                with _ ->
                    malformed subject "issue identity, revision, or author was incomplete")
    ...
    let meterRemaining (transport: IGitHubTransport) =
        ...
        GraphQl.read transport (request subject "query { rateLimit { remaining cost } }" []) (fun data ->
            ...
            | Ok meter ->
                try
                    Ok(meter.GetProperty("remaining").GetInt32())
                with _ ->
                    malformed subject "remaining was missing or not an integer")

    let private archiveDocument =
    ...
    let archiveScan (transport: IGitHubTransport) (projectId: string) =
                    ...
                    | Ok connection ->
                        GraphQl.page
                            ...
                            (fun raw ->
                                try
                                    let content =
                                        match raw.TryGetProperty "content" with
                                        | true, v when v.ValueKind = JsonValueKind.Object -> Some v
                                        | _ -> None

                                    let nested (name: string) (field: string) =
                                        match raw.TryGetProperty name with
                                        | true, v when v.ValueKind = JsonValueKind.Object -> stringOption field v
                                        | _ -> None

                                    Ok
                                        {
                                            ItemId = raw.GetProperty("id").GetString()
                                            Status = nested "status" "name"
                                            BlockedBy = nested "blockedBy" "text"
                                            Number =
                                                content
                                                |> Option.bind (fun c ->
                                                    ...
                                                    | true, v when v.ValueKind = JsonValueKind.Number ->
                                                        Some(v.GetInt32())
                                                    | _ -> None)
                                            State = content |> Option.bind (stringOption "state")
                                            ClosedAt = content |> Option.bind (stringOption "closedAt")
                                            Repo =
                                                content
                                                |> Option.bind (fun c ->
                                                    match c.TryGetProperty "repository" with
                                                    | true, r when r.ValueKind = JsonValueKind.Object ->
                                                        stringOption "nameWithOwner" r
                                                    | _ -> None)
                                        }
                                with _ ->
                                    malformed subject "an archive item node was malformed")
                            connection)
        ...
        | Ok items ->
            ...
            Ok
                {
                    Items = items
                    Pages = after.Calls - before.Calls
                    Spent = after.Points - before.Points
                }

    let archiveItems (transport: IGitHubTransport) (projectId: string) (itemIds: string list) =
        if List.isEmpty itemIds then
            malformed projectId "archive mutation requires at least one item id"
        else
            let subject = $"archive %d{itemIds.Length} project item(s)"

            let declarations =
                [ "$p:ID!" ] @ [ for i in 0 .. itemIds.Length - 1 -> $"$i%d{i}:ID!" ]
                |> String.concat ","

            let aliases =
                [
                    for i in 0 .. itemIds.Length - 1 ->
                        $"a%d{i}:archiveProjectV2Item(input:{{projectId:$p,itemId:$i%d{i}}}){{item{{id}}}}"
                ]
                |> String.concat " "
    ...
    let rosterBoard (transport: IGitHubTransport) (owner: string) (title: string) =
        ...
        | Ok values ->
            ...
            | [ project ] ->
                ...
                let document =
                    "query($owner:String!,$number:Int!,$cursor:String){organization(login:$owner){projectV2(number:$number){items(first:100,after:$cursor){totalCount pageInfo{hasNextPage endCursor} nodes{id status:fieldValueByName(name:\"Status\"){... on ProjectV2ItemFieldSingleSelectValue{name}} content{__typename ... on Issue{number repository{owner{login} name}} ... on PullRequest{number repository{owner{login} name}}}}}}} rateLimit{cost remaining}}"

                let fetch cursor =
                    let vars =
                        [ "owner", VString owner; "number", VNumber(float project.Number) ]
                        @ (cursor
                           |> Option.map (fun c -> [ "cursor", VString c ])
                           |> Option.defaultValue [])
                    ...
                    GraphQl.read transport (request subject document vars) (fun data ->
                        ...
                        | Ok org ->
                            ...
                            | Error error -> Error error
                            | Ok projectNode ->
                                match requiredObject subject "items" projectNode with
                                | Error error -> Error error
                                | Ok connection ->
                                    GraphQl.page
                                        subject
                                        ...
                                        (fun raw ->
                                            try
                                                ...
                                                if kind <> "Issue" && kind <> "PullRequest" then
                                                    ...
                                                    let status =
                                                        match raw.TryGetProperty "status" with
                                                        | true, s when s.ValueKind = JsonValueKind.Object ->
                                                            stringOption "name" s |> Option.defaultValue ""
                                                        | _ -> ""
                                                    ...
                                                    let row =
                                                        {
                                                            Owner =
                                                                repo
                                                                    ...
                                                                    .GetProperty("login")
                                                                    .GetString()
                                                            Repo = repo.GetProperty("name").GetString()
                                                            Number = number
                                                            Status = status
                                                        }
                                                    ...
                                                    Ok(id, Some row)
                                            with _ ->
                                                malformed subject "a roster board node was malformed")
                                        connection)

                GraphQl.drain subject "the roster board" { MaxPages = 100; MaxItems = 10000 } fetch
                |> Result.map (List.choose snd)
