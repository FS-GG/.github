...
module DeliveryRoute =
    ...
    let private nonEmptyList field values =
        if List.isEmpty values || values |> List.exists System.String.IsNullOrWhiteSpace then
    ...
    let private tokens field values =
        ...
        if values |> List.exists malformed then
            [ $"%s{field} must contain non-empty machine tokens" ]
        else
            []
    ...
    let validate expectedSubject expectedRevision receipt =
        let errors =
            [
                if receipt.Schema <> Schema then
                    yield $"schema must be '%s{Schema}'"
                if receipt.Subject <> expectedSubject then
                    yield "subject does not match the live item"
                if receipt.SubjectRevision <> expectedRevision then
                ...
                | Some Lightweight when
                    receipt.SddWorkId.IsSome
                    || receipt.SpecHome.IsSome
                    || not (List.isEmpty receipt.RequiredGates)
                    ->
