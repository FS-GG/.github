...
module FindingPacket =
    ...
    let private oneOf (field: string) (shape: string) (blame: string) =
        {
            Field = field
            Detail = $"%s{blame}; it %s{shape}"
        }

    // Decode a one-key tagged union. `cases` maps the JSON key to its constructor.
    let private decodeUnion
        (field: string)
        (shape: string)
        (cases: (string * (string -> 'a)) list)
        (element: JsonElement)
        =
    ...
    let private requiredUnion (root: JsonElement) (field: string) shape cases =
        ...
        | false, _ ->
            Error
                {
                    Field = field
                    Detail = $"is required and %s{shape}"
                }
            ...
            [
                ...
                if String.IsNullOrWhiteSpace causeValue then
                    yield
                        {
                            Field = "cause"
                            Detail = $"'%s{caseName}' must carry a non-blank string"
                        }

                for field, answer in
                    [
                        "redToday", packet.RedToday
                        "derivedBy", packet.DerivedBy
                        "classRow", packet.ClassRow
                    ] do
                ...
                if
                    not (String.IsNullOrWhiteSpace packet.Finder)
                    && not (workerId.IsMatch packet.Finder)
                then
                    yield
                        {
                            Field = "finder"
            ...
            if root.ValueKind <> JsonValueKind.Object then
                Error
                    [
                        {
                            Field = "document"
                            Detail = $"must be a JSON object carrying a {Schema} packet"
                        }
                    ]
            else
                ...
                if not (List.isEmpty unknown) then
                    ...
                    let cause =
                        requiredUnion
                            ...
                            "cause"
                            causeShape
                            [ "established", Established; "notEstablished", NotEstablished ]

                    let answerCases =
                        [
                            "found", Found
                            "searchedNotFound", SearchedNotFound
