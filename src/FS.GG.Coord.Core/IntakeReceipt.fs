...
module IntakeReceipt =
    ...
    let private digestWithSeverity severity (draft: Intake.Draft) =
        let parts =
            [
                ...
                draft.Acceptance
                draft.Verification
                String.concat "\u001f" draft.Paths
                draft.Class
                draft.Status
                string draft.Disposition
                string draft.Phase
                string severity
                string draft.BlockedBy
                string draft.BlockedOn
                string draft.BacklogReason
                string draft.JudgementQuestion
            ]

        SHA256.HashData(Encoding.UTF8.GetBytes(String.concat "\u001e" parts))
        ...
        |> List.collect (fun classVariant ->
            [
                ...
                | Some value when value.ToLowerInvariant() <> value ->
                    yield
                        { classVariant with
                            Severity = Some(value.ToLowerInvariant())
                        }
                | _ -> ()
            ])
        |> List.distinct
    ...
    let validate (draft: Intake.Draft) (receipt: Receipt) =
        if receipt.IssueNumber <= 0 then
            Error "receipt issueNumber must be positive"
        elif receipt.DraftId <> draft.Id then
            Error "receipt draft id does not match this draft"
        elif receipt.Owner <> draft.Owner || receipt.Repository <> draft.Repository then
            Error "receipt owner/repository does not match this draft"
        elif compatibleDigests draft |> List.contains receipt.DraftDigest |> not then
            Error "receipt content digest does not match this draft"
        else
            Ok receipt
