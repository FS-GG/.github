...
module ReviewWait =
    ...
    let private isPotentialOrdinaryExhaustionReceipt (receipt: WaitReceipt) =
        ...
        let separatorAt =
            receipt.ReviewGeneration.LastIndexOf(separator, StringComparison.Ordinal)
        ...
        if receipt.Kind <> RepairConfirmation || separatorAt <> 40 then
            ...
            | true, round -> round >= ordinaryExhaustionFloor && head |> Seq.forall Uri.IsHexDigit
            | _ -> false
            ...
            | Enter receipt ->
                box
                    {|
                        ...
                        kind = kindName receipt.Kind
                        enteredAt = instant receipt.EnteredAt
                        expiresAt = instant receipt.ExpiresAt
    ...
    let tryDecode (body: string) =
        if not (body.StartsWith(Marker, StringComparison.Ordinal)) then
            Ok None
        else
            try
                ...
                if requiredString "schema" root <> "fsgg.coord.review-wait/v1" then
                    invalidArg "schema" "must be fsgg.coord.review-wait/v1"
                ...
                let event =
                    ...
                    | "enter" ->
                        let kind =
                            ...
                            | other -> invalidArg "kind" $"unknown kind '%s{other}'"

                        Enter
                            {
                                Item = requiredString "item" root
                                ClaimGeneration = requiredString "claimGeneration" root
                                ReviewGeneration = generation ()
                                Kind = kind
                                EnteredAt = requiredInstant "enteredAt" root
                                ExpiresAt = requiredInstant "expiresAt" root
                                EvidenceRef = evidence ()
                            }
        ...
        if not (List.isEmpty errors) then
            ...
            let entries =
                ...
                |> List.choose (function
                    | Enter receipt when receipt.Item = item -> Some receipt
                    | _ -> None)
                |> List.fold
                    (fun (seen, ordered) receipt ->
                        if Set.contains receipt.ReviewGeneration seen then
                            seen, ordered
                        else
                            Set.add receipt.ReviewGeneration seen, ordered @ [ receipt ])
                    (Set.empty, [])
                |> snd
            ...
            | entries, active, _ ->
                let receipt =
                    active |> List.tryExactlyOne |> Option.defaultWith (fun () -> List.last entries)
                ...
                | generation, Some(0, at, evidence) when
                    generation <> Some receipt.ClaimGeneration
