...
module Qualification =
    ...
    let private obligationIdPattern =
        Regex("^[a-z0-9][a-z0-9._-]*$", RegexOptions.CultureInvariant)

    let private obligationKindPattern =
        Regex("^[a-z0-9][a-z0-9_-]*$", RegexOptions.CultureInvariant)
    ...
    let private duplicates kind (values: string list) =
        ...
        |> List.choose (fun (value, count) ->
            if String.IsNullOrWhiteSpace value || count > 1 then
                Some(DuplicateIdentity(kind, value))
            else
                None)

    let private digest field (value: string) =
        if shaPattern.IsMatch value then
            []
        else
            [ InvalidDigest(field, value) ]
    ...
    let private parseTool (label: string) (element: JsonElement) =
...

        {
            ...
            Version = text label "version" element
            Sha256 = text label "sha256" element
        }
    ...
    let parseInput (bytes: byte array) =
        try
            ...
            let tools =
                array "input" "toolManifest" root
                |> List.mapi (fun index item -> parseTool ($"toolManifest[%d{index}]") item)
                ...
                |> List.mapi (fun observationIndex observation ->
                    ...
                    let checks =
                        ...
                        |> List.mapi (fun checkIndex check ->
                            ...
                            {
                                Scope = text checkLabel "scope" check
                                Id = text checkLabel "id" check
                                SubjectRevision = text checkLabel "subjectRevision" check
                                State = text checkLabel "state" check
                                Conclusion = text checkLabel "conclusion" check
                            })

                    {
                        Complete = boolean label "complete" observation
                        Checks = checks
                    })
        ...
        try
            ...
            let counts =
                [
                    ...
                    accepted.ClaimCount
                    accepted.MutationCount
                    accepted.HostedCheckCount
                    accepted.ObligationCount
                ]

            if accepted.Schema <> ResultSchema then
                Error [ $"qualification result schema is not %s{ResultSchema}" ]
            elif not (subjectPattern.IsMatch accepted.Subject) then
                Error [ "qualification result subject is invalid" ]
            elif not (revisionPattern.IsMatch accepted.SubjectRevision) then
                Error [ "qualification result subject revision is invalid" ]
            elif counts |> List.exists (fun value -> value < 0) then
                Error [ "qualification result counts must be non-negative" ]
            elif not (shaPattern.IsMatch accepted.EvidenceDigest) then
                Error [ "qualification result evidence digest is invalid" ]
            elif not (shaPattern.IsMatch accepted.Digest) then
                Error [ "qualification result digest is invalid" ]
            else
                let actual =
                    canonicalAccepted { accepted with Digest = "" } false
                    |> Encoding.UTF8.GetBytes
                    |> CanonicalJson.sha256

                if actual <> accepted.Digest then
                    Error [ "qualification result digest does not bind its canonical content" ]
                else
                    Ok accepted
        with
    ...
    let validate (input: Input) =
        ...
        if input.Schema <> InputSchema then
            findings.Add(InvalidSchema(InputSchema, input.Schema))

        if not (subjectPattern.IsMatch input.Subject) then
            findings.Add(InvalidSubject input.Subject)

        if not (revisionPattern.IsMatch input.SubjectRevision) then
            findings.Add(InvalidDigest("subjectRevision", input.SubjectRevision))

        if not input.CheckoutClean then
            findings.Add DirtyCheckout
        ...
        let requiredObserved =
            observedOrder |> List.filter (fun kind -> List.contains kind requiredOrder)

        if requiredObserved <> requiredOrder then
            findings.Add(OperationOrderMismatch(requiredOrder, requiredObserved))

        for operation in input.Operations do
            if operation.SubjectRevision <> input.SubjectRevision then
                findings.Add(StaleSubject(operation.Id, operation.SubjectRevision))

            match tools.TryFind operation.Tool.Id with
            | None -> findings.Add(UndeclaredTool(operation.Id, operation.Tool.Id))
            | Some tool when tool <> operation.Tool ->
                findings.Add(ToolIdentityMismatch(operation.Id, operation.Tool.Id))
            | Some _ -> ()

            if operation.Executor <> input.Executor then
                findings.Add(WrongExecutor(operation.Id, operation.Executor.Id))
            ...
            |> List.iteri (fun index value ->
                findings.AddRange(digest ($"operation[%s{operation.Id}].artifactSha256[%d{index}]") value))

            match operation.Kind with
            | Mutation ->
                if operation.ExitCode = 0 then
                    findings.Add(FailedOperation(operation.Id, operation.ExitCode))

                if operation.Refusal |> Option.exists (String.IsNullOrWhiteSpace >> not) |> not then
                    findings.Add(UnexpectedRefusal operation.Id)
            | FixedPoint ->
                if operation.ExitCode <> 0 then
                    findings.Add(FailedOperation(operation.Id, operation.ExitCode))

                if operation.Refusal.IsSome then
                    findings.Add(UnexpectedRefusal operation.Id)

                if operation.ReplayResultSha256 <> Some operation.ResultSha256 then
                    findings.Add(FixedPointMismatch operation.Id)
            | _ ->
                if operation.ExitCode <> 0 then
                    findings.Add(FailedOperation(operation.Id, operation.ExitCode))
                ...
                if operation.ReplayResultSha256.IsSome then
                    findings.Add(FixedPointMismatch operation.Id)

        findings.AddRange(duplicates "claim" (input.Claims |> List.map _.Id))

        for claim in input.Claims do
            if claim.SubjectRevision <> input.SubjectRevision then
                findings.Add(StaleSubject(claim.Id, claim.SubjectRevision))
            ...
            for evidenceId in claim.EvidenceIds do
                ...
                | Some evidence when evidence.SubjectRevision <> claim.SubjectRevision ->
                    findings.Add(ClaimEvidenceMismatch(claim.Id, evidenceId))
                | Some _ -> ()

            for kind in claim.RequiredKinds do
                let adequate =
                    claim.EvidenceIds
                    |> List.exists (fun evidenceId ->
                        operations.TryFind evidenceId
                        |> Option.exists (fun evidence -> evidence.Kind = kind && evidence.ExitCode = 0))

                if not adequate then
                    findings.Add(InadequateClaimEvidence(claim.Id, kind))
        ...
        for mutation in input.Mutations do
            ...
            | Some operation when operation.Kind <> Mutation ->
                findings.Add(MutationOperationMissing(mutation.Id, mutation.OperationId))
            | Some operation ->
                if
                    operation.Refusal <> Some mutation.ObservedRefusal
                    || mutation.ExpectedRefusal <> mutation.ObservedRefusal
                then
                    findings.Add(MutationRefusalMismatch mutation.Id)

            if
                mutation.ProductionImplementationSha256 <> input.Executor.ImplementationSha256
                ...
                || mutation.FixtureExecutorRole = input.Executor.Role
            then
                findings.Add(MutationFixtureNotIndependent mutation.Id)

        if input.HostedObservations.Length < 2 then
            findings.Add HostedSetNotConverged

        input.HostedObservations
        |> List.iteri (fun index observation ->
            if not observation.Complete then
                findings.Add(HostedObservationIncomplete(index + 1))

            findings.AddRange(
                duplicates
                    ($"hosted-observation-%d{index + 1}")
                    (observation.Checks |> List.map (fun check -> $"%s{check.Scope}:%s{check.Id}"))
            )

            for check in observation.Checks do
                if check.SubjectRevision <> input.SubjectRevision then
                    findings.Add(HostedForeignSubject(check.Id, check.SubjectRevision))

                if check.State <> "completed" || String.IsNullOrWhiteSpace check.Conclusion then
                    findings.Add(HostedCheckPending check.Id))
        ...
        if input.HostedObservations.Length >= 2 then
            ...
            if normalized terminal[0] <> normalized terminal[1] then
                findings.Add HostedSetNotConverged

        if input.Obligations.HeadSha <> input.SubjectRevision then
            findings.Add(ObligationHeadMismatch input.Obligations.HeadSha)
        ...
        | declarations when
            ...
            let obligations =
                ...
                |> List.choose (function
                    | Obligation value -> Some value
                    | _ -> None)

            findings.AddRange(duplicates "obligation" (obligations |> List.map _.Id))

            for id, count in List.countBy id (obligations |> List.map _.Id) do
                if count > 1 then
                    findings.Add(ObligationIdDuplicate id)

            for obligation in obligations do
                if
                    not (obligationIdPattern.IsMatch obligation.Id)
                    || not (obligationKindPattern.IsMatch obligation.Kind)
                then
                    findings.Add(ObligationReadbackInvalid)
        | declarations -> findings.Add(ObligationDeclarationDuplicate declarations.Length)

        if input.Obligations.Readbacks.Length <> input.Obligations.Declarations.Length then
            findings.Add ObligationReadbackMissing
        ...
        for readback in input.Obligations.Readbacks do
            ...
            if
                readback.CommentId <= 0L
                || String.IsNullOrWhiteSpace readback.Author
                || not validUrl
            then
                findings.Add ObligationReadbackInvalid

        if
            not input.SemanticReview.Accepted
            || String.IsNullOrWhiteSpace input.SemanticReview.Evidence
        then
            findings.Add SemanticReviewMissing

        if input.SemanticReview.SubjectRevision <> input.SubjectRevision then
            findings.Add(SemanticReviewStale input.SemanticReview.SubjectRevision)
        ...
        if findings.Count > 0 then
            ...
            let executorValues (executor: ExecutorIdentity) =
                [ executor.Id; executor.Role; executor.ImplementationSha256 ]

            let operationValues (operation: OperationEvidence) =
                [ operation.Id; operationKindName operation.Kind; operation.SubjectRevision ]
                @ toolValues operation.Tool
                @ executorValues operation.Executor
                @ [ operation.CommandSha256 ]
                @ addList operation.ArtifactSha256
                @ [
                    operation.ResultSha256
                    operation.ReplayResultSha256 |> Option.defaultValue ""
                    string operation.ExitCode
                    operation.Refusal |> Option.defaultValue ""
                ]

            let claimValues (claim: Claim) =
                [ claim.Id; claim.SubjectRevision ]
                @ addList (claim.RequiredKinds |> List.map operationKindName)
                @ addList claim.EvidenceIds

            let mutationValues (mutation: MutationEvidence) =
                [
                    mutation.Id
                    mutation.OperationId
                    mutation.ExpectedRefusal
                    mutation.ObservedRefusal
                    mutation.ProductionImplementationSha256
                    mutation.FixtureImplementationSha256
                    mutation.FixtureExecutorId
                    mutation.FixtureExecutorRole
                ]
            ...
            let hostedValues (observation: HostedObservation) =
                ...
                @ (observation.Checks
                   |> List.collect (fun check ->
                       [ check.Scope; check.Id; check.SubjectRevision; check.State; check.Conclusion ]))

            let obligationValues =
                input.Obligations.Declarations
                |> List.collect (function
                    | NoObligations -> [ "none"; "" ]
                    | Obligation value -> [ value.Kind; value.Id ])
            ...
            let evidenceValues =
                ...
                @ [ input.Obligations.HeadSha ]
                @ addList obligationValues
                @ addList (
                    input.Obligations.Readbacks
                    |> List.collect (fun readback -> [ string readback.CommentId; readback.Url; readback.Author ])
                )
                @ [
                    input.SemanticReview.SubjectRevision
                    string input.SemanticReview.Accepted
                    input.SemanticReview.Evidence
                ]
            ...
            let draft =
                {
                    ...
                    ToolCount = input.ToolManifest.Length
                    ...
                    SemanticReviewEvidence = input.SemanticReview.Evidence
                    EvidenceDigest = evidenceDigest
                    Digest = ""
                }

            let digest =
                canonicalAccepted draft false |> Encoding.UTF8.GetBytes |> CanonicalJson.sha256
