...
module UsageReceiptStore =
    ...
    val defaultRoot: unit -> string
    val archive: root: string option -> bytes: byte array -> Result<ArchivedReceipt, string list>
    val resolve: root: string option -> source: string -> Result<byte array, string list>
    val tryResolve: root: string option -> source: string -> Result<byte array option, string list>

module LegacyReceiptProof =
    [<Literal>]
    val Schema: string = "fsgg.telemetry.legacy-receipt-proof/v1"
...
module LifecycleTelemetry =
    ...
    val sealSuccessor:
        ...
        draftJson: string ->
            Result<string, Finding list>

    val sealSuccessorWithEvidence:
        runId: string ->
        unitId: string ->
    ...
    val sealSuccessorWithEvidenceAndLegacy:
        runId: string ->
        unitId: string ->
        usageReports: (string * RuntimeUsage.UsageRow list) list ->
    ...
    val sealSuccessorWithEvidenceAndCheckpoints:
        runId: string ->
        unitId: string ->
        usageReports: (string * RuntimeUsage.UsageRow list) list ->
        legacyProofs: LegacyReceiptProof.Proof list ->
    ...
    val validateReconciledWithEvidence:
        runId: string ->
        unitId: string ->
        requireTerminal: bool ->
        requiredPhases: string list ->
    ...
    val validateWithEvidenceAndLegacy:
        runId: string ->
        unitId: string ->
        requireTerminal: bool ->
    ...
    val validateWithEvidenceAndCheckpoints:
        runId: string ->
        unitId: string ->
        requireTerminal: bool ->
        requireReconciled: bool ->
    ...
    val exportComments:
        runId: string -> unitId: string -> commentsJson: string -> Result<string * Finding list, Finding list>
...
module CritiqueReceipt =
    ...
    val validate:
        expectedCycle: string -> expectedHead: string option -> bytes: byte array -> Result<Receipt, string list>
...
module RoadmapProjection =
    ...
    val render:
        expectedSourceDigest: string -> roadmapBytes: byte array -> RoadmapClosure.Closed -> Result<string, string list>

    val verify:
        expectedSourceDigest: string ->
        sourceRoadmapBytes: byte array ->
        candidateRoadmapBytes: byte array ->
        RoadmapClosure.Closed ->
