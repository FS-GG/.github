...
module EpicBody =
    ...
    let childRefs (selfOwner: string) (selfRepo: string) (body: string) : string list =
        // A task line inside a fenced code block is a MENTION, not a declaration — and WHERE the fence
        // is, is `Markdown`'s question, not this module's (#972). This file used to carry its own fence
        // tracker; it was the third in the engine, and the three disagreed.
