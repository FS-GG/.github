...
module TelemetryRuntime =
    ...
    let parseAssignment (bytes: byte array) =
        try
            ...
            match
                ...
                text root "attemptId",
                parent,
                text root "producerStream"
            with
            | Some schema, Some feature, Some item, Some attempt, Some parentAttempt, Some producer when
                schema = AssignmentSchema
                && unknown.IsEmpty
                && List.forall safe [ feature; item; attempt; producer ]
                ->
                Ok
                    {
                        FeatureId = feature
            ...
            | _ ->
                Error
                    [
                        "assignment must be closed, schema-current, and contain safe feature/item/attempt/producer identifiers"
                    ]
    ...
    let parseInvocationContext (bytes: byte array) =
        try
            ...
            match
                ...
                parent,
                text root "producerStream",
                text root "activationId",
                text root "dispatchId",
                text root "invocationId",
                text root "rootInvocationId",
                optionalText root "storeRoot",
                lateAfter
            with
            | Some schema,
              Some feature,
              ...
              Ok storeRoot,
              Some late when
                schema = InvocationContextSchema
                && unknown.IsEmpty
                && List.forall
                    safe
                    [
                ...
                Ok
                    {
                        ...
                        StoreRoot = storeRoot |> Option.map IO.Path.GetFullPath
                        LateAfterSeconds = late
                    }
            | _ when not unknown.IsEmpty ->
                Error [ "invocation context contains unknown fields: " + String.concat "," unknown ]
            | _ ->
                Error
                    [
                        "invocation context must be closed, schema-current, private, and contain safe lineage identifiers"
                    ]
        with
    ...
    let projectLine (currentThreadId: string option) (turnSequence: int64) (raw: string) =
        try
            ...
            | Some "thread.started" ->
                match text root "thread_id" with
                | Some thread -> Some(ThreadStarted thread)
                | None -> Some(Gap "missing-thread-id")
            | Some "turn.started" ->
