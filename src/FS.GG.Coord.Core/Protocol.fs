    ...
    let takeExitCodes: ExitCodeDoc list =
        [ { Code = ExitCode.toInt ExitCode.Green
          ...
          { Code = ExitCode.toInt ExitCode.Rate
            Name = "EX_RATE"
            Meaning =
              "A rate limit is exhausted. The message names WHICH kind (#897/#1666): a primary REST limit takes `claim`/`take`/`who` with it, because the lock lives there (ADR-0034 §3); GraphQL takes the board reads. A secondary (abuse-detection) limit is different: it may name NO reset, never appears in `gh api rate_limit`, and is triggered by concurrent request bursts. A healthy `gh api rate_limit` reading therefore does not disprove this refusal. For a primary REST limit, the fleet standing down is designed behaviour (#976): answering \"is this item takeable?\" costs the very budget that is gone, and a lock you cannot verify is not a lock."
            Action =
