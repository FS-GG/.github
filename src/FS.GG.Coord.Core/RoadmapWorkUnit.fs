...
module RoadmapWorkUnit =
    ...
    let private unitId =
        Regex("^[A-Za-z0-9][A-Za-z0-9._-]*$", RegexOptions.CultureInvariant)

    let private issue =
        Regex("^https://github.com/[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+/issues/[1-9][0-9]*$", RegexOptions.CultureInvariant)

    let private issueRef =
        Regex("^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+#[1-9][0-9]*$", RegexOptions.CultureInvariant)
    ...
    let private canonical bytes =
        CanonicalJson.canonicalize bytes |> Result.defaultWith invalidOp
    ...
    let private draftDto (draft: Intake.Draft) =
        {|
            ...
            ``class`` = draft.Class
            status = draft.Status
            disposition = dispositionName draft.Disposition
            phase = draft.Phase
            severity = draft.Severity
            blockedBy = draft.BlockedBy
            blockedOn = draft.BlockedOn
            backlogReason = draft.BacklogReason
    ...
    let private rowDto (row: CatalogRow) =
        {|
            ...
            gates = row.Gates
            evidenceObligations = row.EvidenceObligations
    ...
    let private canonicalPlanPayload (plan: PreparationPlan) includeDigest =
        ...
        JsonSerializer.SerializeToUtf8Bytes
            {|
                schema = plan.Schema
                unit = rowDto plan.Unit
                acceptedPrerequisite = plan.AcceptedPrerequisite
                authority =
                    {|
                        roadmapRevision = plan.Authority.RoadmapRevision
                        roadmapDigest = plan.Authority.RoadmapDigest
                        catalogDigest = plan.Authority.CatalogDigest
                        issue = plan.Authority.Issue
                    |}
                sddWorkId = plan.SddWorkId
                registrations = registrations
                gateRegistrations = plan.GateRegistrations
    ...
    let canonicalPreparationApplication application =
        applicationPayload application true + "\n"
    ...
    let sealPreparationApplication (plan: PreparationPlan) (registrations: AppliedRegistration list) =
            ...
            | [ observed ] ->
                ...
                if not (issueRef.IsMatch observed.Issue) || not (issue.IsMatch observed.IssueUrl) then
                    ...
                    if observed.IssueUrl <> $"https://github.com/%s{repository}/issues/%s{number}" then
                        findings.Add(
                            RegistrationInvalid(expected.Id, "application issue URL differs from its canonical issue")
                        )
        ...
        if findings.Count > 0 then
            ...
            let unsigned =
                {
                    Schema = PreparationApplicationSchema
            ...
            Ok
                { unsigned with
                    Digest = digest (utf8 (applicationPayload unsigned false))
                }
    ...
    let sealRevisionBinding repository candidate merge candidateTree mergeTree exitCode =
        let commandSha256 =
            digest (utf8 (canonicalRevisionCommand repository candidate merge))

        let unsigned =
            {
                Schema = RevisionBindingSchema
        ...
        { unsigned with
            Digest = digest (utf8 (revisionBindingPayload unsigned false))
        }

    let canonicalRevisionBinding binding =
        revisionBindingPayload binding true + "\n"

    let private registrationDraft
        (owner: string)
        (repository: string)
        (paths: string list)
        (unit: CatalogRow)
        ...
        : Intake.Draft =
        {
            ...
            Acceptance =
                $"Registration identity matches roadmap unit %s{unit.UnitId} and replays through staged intake."
    ...
    let inspectPreparation (input: PreparationInput) =
        ...
        if input.Schema <> PreparationInputSchema then
            findings.Add(InvalidSchema(PreparationInputSchema, input.Schema))

        if not (revision.IsMatch input.RoadmapRevision) then
            findings.Add(InvalidIdentity("roadmapRevision", input.RoadmapRevision))

        if not (Regex.IsMatch(input.RoadmapSourceDigest, "^sha256:[0-9a-f]{64}$", RegexOptions.CultureInvariant)) then
            findings.Add(InvalidDigest("roadmapSourceDigest", input.RoadmapSourceDigest))

        if not (Regex.IsMatch(input.CatalogSourceDigest, "^sha256:[0-9a-f]{64}$", RegexOptions.CultureInvariant)) then
            findings.Add(InvalidDigest("catalogSourceDigest", input.CatalogSourceDigest))

        if not (issue.IsMatch input.AuthorityIssue) then
            findings.Add(InvalidIdentity("authorityIssue", input.AuthorityIssue))

        let authorityNumber =
            input.AuthorityIssue.Split('/') |> Array.tryLast |> Option.defaultValue ""

        if
            not (Regex.IsMatch(input.SddWorkId, "^[1-9][0-9]*-[a-z0-9][a-z0-9-]*$", RegexOptions.CultureInvariant))
            || not (input.SddWorkId.StartsWith(authorityNumber + "-", StringComparison.Ordinal))
        then
            findings.Add(InvalidIdentity("sddWorkId", input.SddWorkId))

        for duplicate in input.Catalog |> List.map _.UnitId |> duplicateValues do
            findings.Add(DuplicateCatalogUnit duplicate)

        let acceptedIds =
            input.Catalog
            |> List.choose (fun row -> if row.State = Accepted then Some row.UnitId else None)
            |> Set.ofList
        ...
        let firstUnchecked =
            input.Catalog |> List.tryFind (fun row -> row.State = Unchecked)

        for row in input.Catalog do
            if not (unitId.IsMatch row.UnitId) || String.IsNullOrWhiteSpace row.Title then
                findings.Add(InvalidIdentity("catalogRow", row.UnitId))

            if not (sha.IsMatch row.ContractSha256) then
                findings.Add(InvalidDigest($"%s{row.UnitId}.contractSha256", row.ContractSha256))

            for gate in duplicateValues row.Gates do
                findings.Add(DuplicateGate gate)

            match row.State, row.Prerequisite with
            | Unchecked, Some prerequisite when firstUnchecked = Some row && not (acceptedIds.Contains prerequisite) ->
                findings.Add(PrerequisiteNotAccepted(row.UnitId, prerequisite))
            | Unchecked, None when firstUnchecked = Some row ->
                findings.Add(PrerequisiteNotAccepted(row.UnitId, "<missing>"))
            | _ -> ()
        ...
        if findings.Count > 0 then
            ...
            if input.RoadmapRow.UnitId <> selected.UnitId then
                findings.Add(RoadmapIdentityMismatch "unitId")

            if input.RoadmapRow.Title <> selected.Title then
                findings.Add(RoadmapIdentityMismatch "title")

            if input.RoadmapRow.Prerequisite <> selected.Prerequisite then
                findings.Add(RoadmapIdentityMismatch "prerequisite")

            if input.RoadmapRow.Gates <> selected.Gates then
                findings.Add(RoadmapIdentityMismatch "gates")
            ...
            for obligation in required do
                if not (List.contains obligation selected.EvidenceObligations) then
                    findings.Add(EvidenceObligationMissing obligation)
            ...
            if findings.Count > 0 then
                    ...
                    [
                        {
                            ...
                            Draft =
                                registrationDraft
                                    input.RegistrationOwner
                                    input.RegistrationRepository
                                    input.RegistrationPaths
                                    selected
                                    "unit"
                                    registrationId
                                    $"[roadmap] %s{selected.UnitId} — %s{selected.Title}"
                        }
                    ]
                ...
                for registration in registrations do
                    ...
                    | Error errors ->
                        errors
                        |> List.iter (fun value ->
                            findings.Add(RegistrationInvalid(registration.Id, $"%s{value.Field} %s{value.Detail}")))
                    | Ok _ -> ()
                    ...
                    let draft =
                        {
                            ...
                            Unit = selected
                            AcceptedPrerequisite = prerequisite
                            Authority =
                                {
                                    RoadmapRevision = input.RoadmapRevision
                                    RoadmapDigest = input.RoadmapSourceDigest
                                    CatalogDigest = input.CatalogSourceDigest
                                    Issue = input.AuthorityIssue
                                }
                            SddWorkId = input.SddWorkId
                            Registrations = registrations
                            GateRegistrations = gateRegistrations
    ...
    let private marker (unit: string) =
        $"<!-- fsgg:roadmap-registration/%s{unit} -->"

    let private endMarker (unit: string) =
        $"<!-- /fsgg:roadmap-registration/%s{unit} -->"
    ...
    let private bounds (text: string) (unit: string) =
        ...
        if starts.Count <> 1 || ends.Count <> 1 || ends[0].Index <= starts[0].Index then
            Error [ BoundedPatchInvalid "registration marker authority is missing or ambiguous" ]
        else
            Ok(starts[0].Index, ends[0].Index + last.Length)

    let private renderBlock (plan: PreparationPlan) =
        let registrationLines =
            plan.Registrations
            |> List.map (fun value -> $"- `%s{value.Kind}` `%s{value.Id}` — staged intake `%s{value.Draft.Id}`")

        let gateLines =
            plan.GateRegistrations
            |> List.map (fun value -> $"- `gate` `%s{value}` — owned by `%s{plan.Registrations.Head.Id}`")

        String.concat
            "\n"
            ([
                marker plan.Unit.UnitId
                $"Authority: `%s{plan.Authority.RoadmapDigest}` via %s{plan.Authority.Issue}"
                $"Unit: `%s{plan.Unit.UnitId}` — %s{plan.Unit.Title}"
                $"Prerequisite: `%s{plan.AcceptedPrerequisite}`"
                "Owning registration:"
             ]
             @ registrationLines
             @ [ "Gate registrations:" ]
             @ gateLines
             @ [
                 "Evidence obligations: " + String.concat ", " plan.EvidenceObligations
                 $"Plan digest: `%s{plan.Digest}`"
                 endMarker plan.Unit.UnitId
             ])
    ...
    let verifyPreparation (source: byte array) (candidate: byte array) (plan: PreparationPlan) =
        let sourceText, candidateText =
            Encoding.UTF8.GetString source, Encoding.UTF8.GetString candidate
        ...
        | Ok(sf, sl), Ok(cf, cl) ->
            if
                sourceText.Substring(0, sf) <> candidateText.Substring(0, cf)
                || sourceText.Substring(sl) <> candidateText.Substring(cl)
            then
                Error [ BoundedPatchInvalid "content outside the registration marker was modified" ]
            elif candidateText.Substring(cf, cl - cf) <> renderBlock plan then
                Error
                    [
                        BoundedPatchInvalid "bounded registration block differs from the canonical plan"
                    ]
            else
                Ok()
    ...
    let private strict (label: string) (expected: string list) (element: JsonElement) =
        if element.ValueKind <> JsonValueKind.Object then
            raise (FormatException($"%s{label} must be an object"))

        let observed = element.EnumerateObject() |> Seq.map _.Name |> Set.ofSeq
        let expected = Set.ofList expected

        let missing, extra =
            Set.difference expected observed, Set.difference observed expected

        let missingText, extraText =
            String.concat "," (Set.toList missing), String.concat "," (Set.toList extra)

        if not missing.IsEmpty then
            raise (FormatException($"%s{label} is missing fields: %s{missingText}"))

        if not extra.IsEmpty then
            raise (FormatException($"%s{label} has unknown fields: %s{extraText}"))
    ...
    let private optionalText (label: string) (name: string) (value: JsonElement) =
        ...
        | JsonValueKind.String when not (String.IsNullOrWhiteSpace(item.GetString())) -> Some(item.GetString())
    ...
    let private strings (label: string) (name: string) (value: JsonElement) =
        ...
        if item.ValueKind <> JsonValueKind.Array then
            raise (FormatException($"%s{label}.%s{name} must be an array"))
    ...
    let private parseState (value: string) =
        ...
        | "accepted" -> Accepted
        | "unchecked" -> Unchecked
        | value -> raise (FormatException($"unknown unit state '%s{value}'"))
    ...
    let private parseRow (label: string) (value: JsonElement) : CatalogRow =
        ...
        {
            UnitId = text label "unitId" value
            Title = text label "title" value
            State = text label "state" value |> parseState
            Prerequisite = optionalText label "prerequisite" value
            Gates = strings label "gates" value
    ...
    let private array (name: string) (value: JsonElement) =
        ...
        if item.ValueKind <> JsonValueKind.Array then
            raise (FormatException($"%s{name} must be an array"))
    ...
    let parsePreparationApplication (bytes: byte array) =
        try
            ...
            let registrations =
                ...
                |> List.mapi (fun index item ->
                    ...
                    {
                        Id = text "registration" "id" item
                        Kind = text "registration" "kind" item
                        DraftSha256 = text "registration" "draftSha256" item
                        Issue = text "registration" "issue" item
                        IssueUrl = text "registration" "issueUrl" item
                    })
            ...
            let observed =
                {
                    ...
                    Registrations = registrations
                    Digest = text "preparationApplication" "digest" root
                }

            let errors = ResizeArray<string>()

            if observed.Schema <> PreparationApplicationSchema then
                errors.Add($"schema must be '%s{PreparationApplicationSchema}'")

            if not (unitId.IsMatch observed.UnitId) then
                errors.Add("unitId is invalid")

            if not (sha.IsMatch observed.PlanDigest) then
                errors.Add("planDigest is invalid")

            if not (sha.IsMatch observed.Digest) then
                errors.Add("digest is invalid")

            if digest (utf8 (applicationPayload observed false)) <> observed.Digest then
                errors.Add("digest does not match canonical application content")

            for registration in registrations do
                if not (sha.IsMatch registration.DraftSha256) then
                    errors.Add($"registration %s{registration.Id} draftSha256 is invalid")
        ...
        with error ->
            Error [ $"invalid preparation application: %s{error.Message}" ]
    ...
    let parseRevisionBinding (bytes: byte array) =
        try
            ...
            let binding =
                {
                    ...
                    Candidate = text "revisionBinding" "candidate" root
                    Merge = text "revisionBinding" "merge" root
                    CandidateTree = text "revisionBinding" "candidateTree" root
                    MergeTree = text "revisionBinding" "mergeTree" root
                    CommandSha256 = text "revisionBinding" "commandSha256" root
                    ExitCode = (prop "exitCode" root).GetInt32()
                    Digest = text "revisionBinding" "digest" root
                }

            let errors = ResizeArray<string>()

            if binding.Schema <> RevisionBindingSchema then
                errors.Add($"schema must be '%s{RevisionBindingSchema}'")
            ...
            for name, value in
                ...
                if not (revision.IsMatch value) then
                    errors.Add($"%s{name} must be an exact 40-hex commit SHA")

            if String.IsNullOrWhiteSpace binding.Repository then
                errors.Add("repository must not be blank")

            let expectedCommand =
                digest (utf8 (canonicalRevisionCommand binding.Repository binding.Candidate binding.Merge))

            if binding.CommandSha256 <> expectedCommand then
                errors.Add("commandSha256 does not bind the canonical Git observation commands")

            if
                not (sha.IsMatch binding.Digest)
                || digest (utf8 (revisionBindingPayload binding false)) <> binding.Digest
            then
                errors.Add("digest does not match canonical revision-binding content")
            ...
            if errors.Count = 0 then
                ...
                Error(List.ofSeq errors)
        with error ->
            Error [ $"invalid revision binding: %s{error.Message}" ]
    ...
    let parsePreparationInput (bytes: byte array) =
        try
            ...
            Ok
                {
                    Schema = text "preparationInput" "schema" root
                    RoadmapRevision = text "preparationInput" "roadmapRevision" root
                    RoadmapSourceDigest = text "preparationInput" "roadmapSourceDigest" root
                    CatalogSourceDigest = text "preparationInput" "catalogSourceDigest" root
                    Catalog =
                        array "catalog" root
                        |> List.mapi (fun index row -> parseRow $"catalog[%d{index}]" row)
                    RoadmapRow = parseRoadmapRow "roadmapRow" (prop "roadmapRow" root)
                    AuthorityIssue = text "preparationInput" "authorityIssue" root
                    SddWorkId = text "preparationInput" "sddWorkId" root
                    RegistrationOwner = text "preparationInput" "registrationOwner" root
                    RegistrationRepository = text "preparationInput" "registrationRepository" root
                    RegistrationPaths = strings "preparationInput" "registrationPaths" root
                }
        with error ->
            Error [ $"invalid roadmap preparation input: %s{error.Message}" ]
    ...
    let parsePreparationRequest (bytes: byte array) =
        try
            ...
            Ok
                {
                    Schema = text "preparationRequest" "schema" root
        ...
        with error ->
            Error [ $"invalid roadmap preparation request: %s{error.Message}" ]
    ...
    let compilePreparation (roadmapBytes: byte array) (catalogBytes: byte array) (request: PreparationRequest) =
        ...
        let roadmapRows =
            ...
            |> Seq.cast<Match>
            |> Seq.map (fun matched ->
                matched.Groups[2].Value, (matched.Groups[1].Value <> " ", matched.Groups[3].Value))
            |> List.ofSeq

        for duplicate in roadmapRows |> List.map fst |> duplicateValues do
            findings.Add(DuplicateCatalogUnit duplicate)
        ...
        try
            ...
            if text "catalog.roadmap" "repository" authority <> "FS-GG/.github" then
                findings.Add(RoadmapIdentityMismatch "repository")

            if text "catalog.roadmap" "path" authority <> "docs/github-substrate-v2-roadmap.md" then
                findings.Add(RoadmapIdentityMismatch "path")

            let pinnedRevision = text "catalog.roadmap" "revision" authority

            if not (revision.IsMatch pinnedRevision) then
                findings.Add(InvalidIdentity("catalog.roadmap.revision", pinnedRevision))
            elif pinnedRevision <> request.RoadmapRevision then
                findings.Add(RoadmapIdentityMismatch "catalog.roadmap.revision")

            let pinnedDigest = text "catalog.roadmap" "sha256" authority

            if pinnedDigest <> roadmapDigest then
                findings.Add(InvalidDigest("catalog.roadmap.sha256", pinnedDigest))
            ...
            for index, unit in array "units" root |> List.indexed do
                ...
                if unit.ValueKind <> JsonValueKind.Object then
                    raise (FormatException($"%s{label} must be an object"))
                ...
                if not unknown.IsEmpty then
                    raise (FormatException($"%s{label} has unknown fields: %s{unknownText}"))
                ...
                if not (sha.IsMatch contractDigest) then
                    findings.Add(InvalidDigest($"%s{id}.contractSha256", contractDigest))

                let calculatedContractDigest =
                    canonicalBytesOmitting "contractSha256" unit |> digest

                if calculatedContractDigest <> contractDigest then
                    findings.Add(InvalidDigest($"%s{id}.contractSha256", contractDigest))

                match Map.tryFind id roadmapById with
                | None -> findings.Add(RoadmapIdentityMismatch($"missing roadmap row %s{id}"))
                | Some(_, roadmapTitle) when roadmapTitle <> title ->
                    findings.Add(RoadmapIdentityMismatch($"title %s{id}"))
                | Some(accepted, _) ->
                    ...
                    catalogRows.Add
                        {
                            UnitId = id
                            Title = title
                            State = if accepted then Accepted else Unchecked
                            Prerequisite = prerequisite
                            Gates = qGates @ gateCommands
                            EvidenceObligations =
                                [
                                    "sdd:analyze"
                                    "sdd:verify"
        ...
        with error ->
            findings.Add(RoadmapIdentityMismatch($"catalog parse: %s{error.Message}"))

        let rows = List.ofSeq catalogRows

        let firstUncheckedRoadmap =
            roadmapRows
            |> List.tryFind (fun (_, (accepted, _)) -> not accepted)
            |> Option.map fst
        ...
        if findings.Count > 0 then
            ...
            | Some index when firstUncheckedRoadmap <> Some rows[index].UnitId ->
                Error
                    [
                        RoadmapIdentityMismatch "catalog omits or reorders the canonical first unchecked roadmap row"
                    ]
            | Some index when
                (rows |> List.take (index + 1) |> List.map _.UnitId)
                <> (roadmapRows
                    |> List.map fst
                    |> List.filter (Set.ofList (rows |> List.take (index + 1) |> List.map _.UnitId)).Contains)
                ->
                Error
                    [
                        RoadmapIdentityMismatch
            ...
            | Some index ->
                ...
                | Some prerequisite when not (accepted.Contains prerequisite) ->
                    Error [ PrerequisiteNotAccepted(selected.UnitId, prerequisite) ]
                | Some prerequisite when index = 0 || rows[index - 1].UnitId <> prerequisite ->
                    Error
                        [
                            RoadmapIdentityMismatch
                                "selected unit prerequisite is not the immediate preceding catalog row"
                        ]
                | Some _ ->
                    inspectPreparation
                        {
    ...
    let private parseDraft (label: string) (value: JsonElement) : Intake.Draft =
        ...
        let disposition =
            match text label "disposition" value with
            | "create" -> Some Intake.Create
            | "reuse" -> Some Intake.Reuse
            | other -> raise (FormatException($"unknown disposition '%s{other}'"))

        {
            Schema = text label "schema" value
            Id = text label "id" value
            Owner = text label "owner" value
            Repository = text label "repository" value
            Title = text label "title" value
            Observed = text label "observed" value
            RootCause = text label "rootCause" value
            Acceptance = text label "acceptance" value
            Verification = text label "verification" value
            Paths = strings label "paths" value
            Class = text label "class" value
            Status = text label "status" value
            Disposition = disposition
            Phase = optionalText label "phase" value
            Severity = optionalText label "severity" value
            BlockedBy = optionalText label "blockedBy" value
            BlockedOn = optionalText label "blockedOn" value
            BacklogReason = optionalText label "backlogReason" value
            JudgementQuestion = optionalText label "judgementQuestion" value
        }
    ...
    let parsePlan (bytes: byte array) =
        try
            ...
            let registrations =
                array "registrations" root
                |> List.mapi (fun index item ->
                    strict $"registrations[%d{index}]" [ "id"; "kind"; "draft" ] item

                    {
                        Id = text "registration" "id" item
                        Kind = text "registration" "kind" item
                        Draft = parseDraft "registration.draft" (prop "draft" item)
                    })

            let plan =
                {
                    Schema = text "plan" "schema" root
                    Unit = parseRow "unit" (prop "unit" root)
                    AcceptedPrerequisite = text "plan" "acceptedPrerequisite" root
                    Authority =
                        {
                            RoadmapRevision = text "authority" "roadmapRevision" authority
                            RoadmapDigest = text "authority" "roadmapDigest" authority
                            CatalogDigest = text "authority" "catalogDigest" authority
                            Issue = text "authority" "issue" authority
                        }
                    SddWorkId = text "plan" "sddWorkId" root
                    Registrations = registrations
                    GateRegistrations = strings "plan" "gateRegistrations" root
                    EvidenceObligations = strings "plan" "evidenceObligations" root
                    Digest = text "plan" "digest" root
                }

            let actual = canonicalPlanPayload { plan with Digest = "" } false |> utf8 |> digest

            if plan.Schema <> PreparationPlanSchema then
                Error [ $"plan schema is not %s{PreparationPlanSchema}" ]
            elif not (sha.IsMatch plan.Digest) || actual <> plan.Digest then
                Error [ "plan digest does not bind canonical content" ]
            else
                Ok plan
        with error ->
            Error [ $"invalid roadmap preparation plan: %s{error.Message}" ]
            ...
            yield
                {
                    ...
                    Sha256 = input.Plan.Digest
                    Source = input.Plan.Authority.Issue
                }
            yield
                {
                    Name = "preparation-application"
                    Sha256 = input.PreparationApplication.Digest
                    Source = input.PreparationApplication.UnitId
                }
            yield
                {
                    Name = "qualification"
                    Sha256 = input.Qualification.Digest
                    Source = input.Qualification.Subject
                }
            yield
                {
                    Name = "qualification-evidence"
                    Sha256 = input.Qualification.EvidenceDigest
                    Source = input.Qualification.SemanticReviewEvidence
                }
            yield
                {
                    Name = "lifecycle"
                    Sha256 = digest (utf8 input.LifecycleLog)
                    Source = $"%s{input.LifecycleRunId}/%s{input.LifecycleUnitId}"
                }
            yield
                {
                    Name = "lifecycle-history"
                    Sha256 = digest (utf8 input.LifecycleHistoryReport)
                    Source = input.LifecycleRunId
                }
            for index, receipt in input.LifecycleUsageReceipts |> List.indexed do
                yield
                    {
                        Name = $"lifecycle-usage-%d{index + 1}"
                        Sha256 = digest (utf8 receipt)
                        Source = input.LifecycleRunId
                    }
            yield
                {
                    Name = "review"
                    Sha256 = digest (utf8 input.ReviewReceipt)
                    Source = input.ReviewEvidence
                }
            yield
                {
                    Name = "structured-review"
                    Sha256 = digest (utf8 input.StructuredReviewEvidence)
                    Source = input.StructuredReviewEvidence
                }
            yield
                {
                    Name = "revision-binding-implementation"
                    Sha256 = input.ImplementationBinding.Digest
                    Source =
                        input.ImplementationBinding.Repository
                        + ":"
                        + input.ImplementationBinding.Candidate
                        + ".."
                        + input.ImplementationBinding.Merge
                }
            yield
                {
                    Name = "revision-binding-acceptance"
                    Sha256 = input.AcceptanceBinding.Digest
                    Source =
                        input.AcceptanceBinding.Repository
                        + ":"
                        + input.AcceptanceBinding.Candidate
                        + ".."
                        + input.AcceptanceBinding.Merge
                }
            for observation in input.SddObservations do
                yield
                    {
                        Name = "sdd-" + observation.Stage
                        Sha256 = digest (utf8 observation.ArtifactJson)
                        Source = input.SddWorkId
                    }
        ]
    ...
    let private buildAcceptance (input: AcceptanceInput) =
        ...
        input.Plan.EvidenceObligations
        |> List.iter (JsonValue.Create >> obligationNodes.Add)
...

        {
            ...
            EvidenceIndexJson = indexJson
            BundleJson = bundleJson
            Digest = receiptBase["digest"].GetValue<string>()
        }

    let inspectAcceptanceCandidate (input: AcceptanceInput) =
        let findings = ResizeArray<Finding>()

        if input.Schema <> AcceptanceInputSchema then
            findings.Add(InvalidSchema(AcceptanceInputSchema, input.Schema))
        ...
        | Ok expected when
            canonicalPreparationApplication expected
            <> canonicalPreparationApplication input.PreparationApplication
            ->
            findings.Add(
                RegistrationInvalid(
                    input.Plan.Unit.UnitId,
                    "preparation application does not match the canonical plan-bound receipt"
                )
            )
        | Ok _ -> ()
        ...
        if input.Qualification.SubjectRevision <> input.Identities.ImplementationCandidate then
            findings.Add(QualificationMismatch "subject revision is not the implementation candidate")

        if input.LifecycleUnitId <> input.Plan.Unit.UnitId then
            findings.Add(LifecycleInvalid "lifecycle unit id is not the selected roadmap unit")

        if
            input.LifecycleRunId
            <> "roadmap-unit-" + input.Plan.Unit.UnitId.ToLowerInvariant()
        then
            findings.Add(LifecycleInvalid "lifecycle run id is not the deterministic selected-unit run")

        if input.RequiredLifecyclePhases <> acceptanceLifecyclePhases then
            findings.Add(
                LifecycleInvalid
                    "required lifecycle phases are not the compiler-derived intake-through-acceptance sequence"
            )

        if input.ReviewCycleId <> input.Plan.Unit.UnitId then
            findings.Add(LifecycleInvalid "review cycle id is not the selected roadmap unit")
        ...
        if String.IsNullOrWhiteSpace input.SddWorkId then
            ...
            findings.Add(
                SddObservationInvalid("work", "unit SDD work id must be distinct from the compiler authority work id")
            )

        if
            String.IsNullOrWhiteSpace input.Qualification.SemanticReviewEvidence
            || String.IsNullOrWhiteSpace input.ReviewEvidence
            || String.IsNullOrWhiteSpace input.StructuredReviewEvidence
        then
            findings.Add ReviewEvidenceMissing

        if input.Qualification.SemanticReviewEvidence <> input.StructuredReviewEvidence then
            findings.Add(
                QualificationMismatch "semantic review evidence locator differs from the structured review authority"
            )
        ...
        let workCycleSuffix =
            let work =
                Regex.Match(input.SddWorkId, "^[1-9][0-9]*-(.+)$", RegexOptions.CultureInvariant)
        ...
        let reviewedHead =
            if receiptCycle = input.ReviewCycleId then
                Some input.Identities.ImplementationCandidate
            else
                None
        ...
        | Error errors ->
            errors
            |> List.iter (fun error -> findings.Add(LifecycleInvalid("review receipt: " + error)))
        | Ok receipt when
            receiptCycle <> input.ReviewCycleId
            && receipt.ReviewedCommit = input.Identities.ImplementationMerge
            ->
            findings.Add(
                LifecycleInvalid "review receipt cannot claim the generated implementation merge as its reviewed commit"
            )
        | Ok _ -> ()

        let usageResults =
            input.LifecycleUsageReceipts |> List.map (utf8 >> RuntimeUsage.parseCsvReceipt)
        ...
        | Ok history ->
            match
                LifecycleTelemetry.validateReconciledWithEvidence
                    input.LifecycleRunId
                    input.LifecycleUnitId
                    true
                    acceptanceLifecyclePhases
                    usageReports
                    history
                    input.LifecycleLog
            with
        ...
        | Some applied ->
            let rec expectedLifecycleRevision phase =
                match phase with
                | "merge"
                | "post-merge-obligations" -> input.Identities.ImplementationMerge
                | "acceptance-candidate" -> input.Identities.AcceptanceCandidate
                | "acceptance"
                | "protected-main-verification"
                | "receipt-projection"
                | "cleanup" -> input.Identities.AcceptanceMerge
            ...
            if not issueMatch.Success then
                findings.Add(LifecycleInvalid "applied unit issue identity is malformed")
            else
                ...
                |> Array.iteri (fun index line ->
                    try
                        ...
                        if
                            item.GetProperty("repo").GetString() <> expectedRepo
                            || item.GetProperty("number").GetInt32() <> expectedNumber
                            || item.GetProperty("url").GetString() <> expectedUrl
                            || authority.GetProperty("subject").GetString() <> applied.Issue
                        then
                            findings.Add(
                                LifecycleInvalid(
                                    $"lifecycle event %d{index + 1} is not bound to the applied unit issue"
                                )
                            )
                        ...
                        if
                            ...
                            <> input.ImplementationBinding.Repository
                            || source.GetProperty("revision").GetString() <> expectedRevision
                        then
                            findings.Add(
                                LifecycleInvalid(
                                    $"lifecycle event %d{index + 1} source revision is invalid for phase %s{phase}"
                                )
                            )
                    with error ->
                        findings.Add(LifecycleInvalid($"lifecycle event %d{index + 1} binding: %s{error.Message}")))

        let requiredStages =
            [
                "analyze", "implementationReady"
                "verify", "verificationReady"
                "ship", "shipReady"
            ]
            ...
            | [ observation ] ->
                try
                        ...
                        let hasWorkModelSource =
                            ...
                            && (sources.EnumerateArray()
                                |> Seq.exists (fun source ->
                                    ...
                                    && text "sddSource" "path" source =
                                        $"readiness/%s{input.SddWorkId}/work-model.json"))
                    ...
                    | Ok _ when
                        ...
                        && readiness = status
                        && observedWork = input.SddWorkId
                        && observation.SubjectRevision = input.Identities.ImplementationCandidate
                        ->
                        ()
                    | Ok _ ->
                        findings.Add(
                            SddObservationInvalid(
                                stage,
                                $"stage=%s{observedStage} status=%s{observedStatus} work=%s{observedWork} subject=%s{observation.SubjectRevision}"
                            )
                        )
                with error ->
                    findings.Add(SddObservationInvalid(stage, error.Message))
        ...
        for name, value in named do
            if not (revision.IsMatch value) then
                findings.Add(InvalidIdentity(name, value))

        if input.Identities.ImplementationPullRequest < 1 then
            findings.Add(
                InvalidIdentity("implementationPullRequest", string input.Identities.ImplementationPullRequest)
            )

        if input.Identities.AcceptancePullRequest < 1 then
            findings.Add(InvalidIdentity("acceptancePullRequest", string input.Identities.AcceptancePullRequest))

        let sharedPullRequest =
            input.Identities.ImplementationPullRequest = input.Identities.AcceptancePullRequest

        if sharedPullRequest then
            if input.Identities.ImplementationCandidate <> input.Identities.AcceptanceCandidate then
                findings.Add(
                    RevisionRelationInvalid
                        "a shared implementation and acceptance pull request must name one candidate"
                )

            if input.Identities.ImplementationMerge <> input.Identities.AcceptanceMerge then
                findings.Add(
                    RevisionRelationInvalid "a shared implementation and acceptance pull request must name one merge"
                )
        else
            ...
            for i in 0 .. distinct.Length - 1 do
                for j in i + 1 .. distinct.Length - 1 do
                    if snd distinct[i] = snd distinct[j] then
                        findings.Add(RevisionIdentityCollapse(fst distinct[i], fst distinct[j]))
        ...
        if input.Identities.ProtectedMain <> input.Identities.AcceptanceMerge then
            findings.Add(RevisionRelationInvalid "protected main must equal the observed acceptance merge")
        ...
        let verifyBinding label expectedRepository candidate merge (binding: RevisionBinding) =
            if binding.Candidate <> candidate || binding.Merge <> merge then
                findings.Add(RevisionRelationInvalid($"%s{label} binding does not match its named candidate and merge"))

            if
                binding.Schema <> RevisionBindingSchema
                || binding.Repository <> expectedRepository
            then
                findings.Add(RevisionRelationInvalid($"%s{label} binding does not name the authoritative repository"))

            if
                not (revision.IsMatch binding.CandidateTree)
                || not (revision.IsMatch binding.MergeTree)
            then
                findings.Add(RevisionRelationInvalid($"%s{label} binding tree identity is invalid"))

            if binding.CandidateTree <> binding.MergeTree then
                findings.Add(RevisionRelationInvalid($"%s{label} merge does not preserve the candidate tree"))

            let expectedCommand =
                digest (utf8 (canonicalRevisionCommand binding.Repository binding.Candidate binding.Merge))

            if binding.CommandSha256 <> expectedCommand || binding.ExitCode <> 0 then
                findings.Add(RevisionRelationInvalid($"%s{label} binding lacks a successful canonical Git observation"))

            if
                not (sha.IsMatch binding.Digest)
                || digest (utf8 (revisionBindingPayload binding false)) <> binding.Digest
            then
                findings.Add(RevisionRelationInvalid($"%s{label} binding digest is invalid"))
        ...
        match DateTimeOffset.TryParse input.AcceptedAt with
        | true, _ -> ()
        | _ -> findings.Add(InvalidIdentity("acceptedAt", input.AcceptedAt))

        if findings.Count = 0 then
            Ok(AcceptanceCandidate input)
        else
            Error(List.ofSeq findings)
    ...
    let private parseIdentities (value: JsonElement) : RevisionIdentities =
...

        {
            ...
            ImplementationCandidate = text "identities" "implementationCandidate" value
            ImplementationMerge = text "identities" "implementationMerge" value
            AcceptancePullRequest = (prop "acceptancePullRequest" value).GetInt32()
            AcceptanceCandidate = text "identities" "acceptanceCandidate" value
            AcceptanceMerge = text "identities" "acceptanceMerge" value
    ...
    let canonicalAcceptanceInput (input: AcceptanceInput) =
        ...
        JsonSerializer.SerializeToUtf8Bytes
            {|
                ...
                lifecycleLog = input.LifecycleLog
                requiredLifecyclePhases = input.RequiredLifecyclePhases
                lifecycleUsageReceipts = input.LifecycleUsageReceipts
                lifecycleHistoryReport = input.LifecycleHistoryReport
                reviewEvidence = input.ReviewEvidence
                structuredReviewEvidence = input.StructuredReviewEvidence
                reviewCycleId = input.ReviewCycleId
                reviewReceipt = input.ReviewReceipt
                sddWorkId = input.SddWorkId
                sddObservations = observations
                identities =
                    {|
                        implementationPullRequest = input.Identities.ImplementationPullRequest
                        implementationCandidate = input.Identities.ImplementationCandidate
                        implementationMerge = input.Identities.ImplementationMerge
                        acceptancePullRequest = input.Identities.AcceptancePullRequest
                        acceptanceCandidate = input.Identities.AcceptanceCandidate
                        acceptanceMerge = input.Identities.AcceptanceMerge
                        protectedMain = input.Identities.ProtectedMain
                    |}
    ...
    let parseAcceptanceInput (bytes: byte array) =
        try
            ...
            let plan =
                parsePlan (utf8 ((prop "plan" root).GetRawText()))
                |> Result.defaultWith (String.concat "; " >> FormatException >> raise)

            let preparationApplication =
                parsePreparationApplication (utf8 ((prop "preparationApplication" root).GetRawText()))
                |> Result.defaultWith (String.concat "; " >> FormatException >> raise)

            let qualification =
                Qualification.parseResult (utf8 ((prop "qualification" root).GetRawText()))
                |> Result.defaultWith (String.concat "; " >> FormatException >> raise)

            let observations =
                array "sddObservations" root
                |> List.mapi (fun index item ->
                    strict $"sddObservations[%d{index}]" [ "stage"; "subjectRevision"; "artifactJson" ] item

                    {
                        Stage = text "sddObservation" "stage" item
                        SubjectRevision = text "sddObservation" "subjectRevision" item
                        ArtifactJson = text "sddObservation" "artifactJson" item
                    })

            Ok
                {
                    Schema = text "acceptanceInput" "schema" root
                    Plan = plan
                    PreparationApplication = preparationApplication
                    Qualification = qualification
                    LifecycleRunId = text "acceptanceInput" "lifecycleRunId" root
                    LifecycleUnitId = text "acceptanceInput" "lifecycleUnitId" root
                    LifecycleLog = text "acceptanceInput" "lifecycleLog" root
                    RequiredLifecyclePhases = strings "acceptanceInput" "requiredLifecyclePhases" root
                    ...
                    SddWorkId = text "acceptanceInput" "sddWorkId" root
                    SddObservations = observations
        ...
        with error ->
            Error [ $"invalid roadmap acceptance input: %s{error.Message}" ]

    let candidateDigest (AcceptanceCandidate input) =
        canonicalAcceptanceInput input |> utf8 |> digest
    ...
    let verifyObservedAcceptance (ObservedAcceptance input) (bundle: byte array) =
        ...
        | Ok _ ->
            Error
                [
                    AcceptanceBundleInvalid "bundle bytes differ from the canonical sealed receipt and evidence index"
                ]
