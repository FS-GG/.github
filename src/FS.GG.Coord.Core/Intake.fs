        ...
        let findings =
            [
                ...
                for field, value in
                    ...
                    match required field value with
                    | Some finding -> yield finding
                    | None -> ()
                if List.isEmpty draft.Paths then
                    yield
                        {
                            Field = "paths"
                            Detail = "must declare at least one path"
                        }
                if draft.Paths |> List.exists System.String.IsNullOrWhiteSpace then
                    yield
                        {
                            Field = "paths"
                            Detail = "must not contain an empty path"
                        }
            ]
            @ [
                if draft.Owner.Contains "/" || draft.Owner.Contains " " then
                    yield
                        {
                            Field = "owner"
                            Detail = "must be an owner name, not a repository ref"
                        }
                if draft.Repository.Contains "/" || draft.Repository.Contains " " then
                    yield
                        {
                            Field = "repository"
                            Detail = "must be a repository name, not a repository ref"
                        }
                if
                    draft.Id
                    |> Seq.exists (fun c -> not (System.Char.IsLetterOrDigit c || c = '-' || c = '_' || c = '.'))
                then
                    yield
                        {
                            Field = "id"
                            Detail = "must contain only letters, digits, '-', '_' or '.'"
                        }
                ...
                | None ->
                    let known =
                        Class.legalClasses |> List.map Types.itemClassWireName |> String.concat ", "

                    yield
                        {
                            Field = "class"
                            Detail = $"must be one of %s{known}"
                        }
                ...
                | Some value ->
                    ...
                    | Ok canonical ->
                        yield
                            {
                                Field = "severity"
                                Detail = $"must use canonical board value '{canonical}'"
                            }
                ...
                | None ->
                    yield
                        {
                            Field = "disposition"
                            Detail = "must explicitly be create or reuse"
                        }
                | Some _ -> ()
                ...
                | "Backlog" ->
                    ...
                    | _ ->
                        yield
                            {
                                Field = "backlogReason"
                                Detail = "must be parked, awaiting-judgement, epic or not-yet-actionable for Backlog"
                            }
                | "Blocked" ->
                    match draft.BlockedBy, draft.BlockedOn with
                    | Some dependency, None when
                        Regex.IsMatch(dependency, @"^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+#[1-9][0-9]*$")
                        ->
                        ()
                        ...
                        yield
                            {
                                Field = "blockedBy"
                                Detail =
                                    "Blocked requires exactly one canonical dependency or human/action|human/decision park"
                            }
                | "Ready" ->
                    if
                        draft.BlockedBy.IsSome
                        || draft.BlockedOn.IsSome
                        || draft.JudgementQuestion.IsSome
                    then
                        yield
                            {
                                Field = "status"
                                Detail = "Ready cannot carry a dependency, human park or judgement question"
                            }
                | _ ->
                    yield
                        {
                            Field = "status"
                            Detail = "must be Backlog, Ready or Blocked"
                        }
            ]
