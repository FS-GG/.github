...
module Driver =
    ...
    let private parseReviewCommentsCore (trustedAudit: SemanticDiff.Receipt option) (comments: ReviewComment list) =
        ...
        let confirmations =
            ordered
            |> List.filter (fun c -> marker "independent-review-confirmation" c.Body)
        ...
        if List.length confirmations > 3 then
        ...
        let requireOne what values =
            ...
            | [ value ] -> Some value
            | _ ->
                errors.Add $"exactly one %s{what} marker is required"
                None

        match requireOne "independent-review" initial, requireOne "review-accepted" acceptances with
        | Some first, Some accepted ->
            match
                field "critic" first.Body,
                field "reviewed-head" first.Body,
                field "verdict" first.Body,
                field "accepted-head" accepted.Body,
                field "initial-review" accepted.Body,
                field "latest-confirmation" accepted.Body
            with
            | Ok critic, Ok initialHead, Ok initialVerdict, Ok acceptedHead, Ok acceptedInitialUrl, Ok acceptedLatestUrl ->
                ...
                if valid && errors.Count = 0 then
                    let rounds =
                        if List.isEmpty confirmations then
                            [ 1 ]
                        else
                            [ 1 .. List.length confirmations ]

                    Ok
                        { MarkerValid = true
                          CriticIdentity = Some critic
                          HeadSha = Some previousHead
                          Rounds = rounds
                          ChecksGreen = false
                          HostAccepted = true
    ...
    let receiptFresh now maxAgeSeconds (receipt: Receipt) =
        receipt.Complete
        && not (System.String.IsNullOrWhiteSpace receipt.SourceSha)
        && (receipt.Review |> Option.exists (validateReviewChain 3 >> List.isEmpty))
        && now >= receipt.ObservedAt
        && now - receipt.ObservedAt <= maxAgeSeconds
