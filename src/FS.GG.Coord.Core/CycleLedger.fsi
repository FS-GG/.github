...
module CycleLedger =
    ...
    val register:
        ...
        repository: string ->
        baseCommit: string ->
        selectedUnit: string option ->
        parallelAuthorized: bool ->
        disjointTouchSets: bool ->
        live: Cycle list ->
            Result<Action, string list>

    val parseProviderReceipt:
        ...
        artifactBytes: byte array ->
            Result<ProviderReceipt, string list>

    val validateProvider:
        expectedWorkId: string ->
        expectedCycle: Cycle ->
        expectedSourceRevision: string ->
        expectedHead: string ->
    ...
    val complete:
        Ledger ->
        accepted: Cycle list ->
        guardedUpdates: UpdateReceipt list ->
        rollupCycleIds: string list ->
            Result<Action, string list>
