...
module SelfHost =
    ...
    let private validate receipt =
        ...
        let errors =
            [
                ...
                if receipt.HostAcceptance.AcceptedAt = DateTimeOffset.MinValue then
                    ...
                    None
                if
                    receipt.CandidateBinarySha256.Length <> 64
                    || receipt.CandidateBinarySha256 |> Seq.exists (fun c -> not (Uri.IsHexDigit c))
                then
        ...
        let errors =
            if
                receipt.SnapshotSha256.Length <> 64
                || receipt.SnapshotSha256 |> Seq.exists (fun c -> not (Uri.IsHexDigit c))
            then
    ...
    let authorizeWrite receipt =
        ...
        | Ok() ->
            ...
            if String.Equals(expected, receipt.Digest, StringComparison.Ordinal) then
                Ok()
            else
                Error [ "self-host bootstrap receipt digest does not match its bound fields" ]
    ...
    let private validateReplayReceipt (receipt: SelfHostReplayReceipt) =
        [
            ...
            if String.IsNullOrWhiteSpace receipt.DecisionKey then
                yield "decisionKey is required"
            if String.IsNullOrWhiteSpace receipt.ActionKey then
                yield "actionKey is required"
            if receipt.ReplayedAt = DateTimeOffset.MinValue then
                yield "replayedAt is required"
        ]
        |> function
            | [] -> Ok()
            | errors -> Error errors
    ...
    let verifyReplayReceipt (bootstrap: SelfHostBootstrapReceipt) (receipt: SelfHostReplayReceipt) =
        ...
        |> Result.bind (fun () ->
            ...
            if not (String.Equals(expected, receipt.Digest, StringComparison.Ordinal)) then
                ...
                not (
                    String.Equals(bootstrap.SnapshotSha256, receipt.SnapshotSha256, StringComparison.OrdinalIgnoreCase)
                )
            then
    ...
    let tryDecodeReceipt (body: string) =
        if not (body.StartsWith(ReceiptMarker, StringComparison.Ordinal)) then
            Ok None
        else
            try
    ...
    let tryDecodeReplayReceipt (body: string) =
        if not (body.StartsWith(ReplayReceiptMarker, StringComparison.Ordinal)) then
            Ok None
        else
            try
                use document =
                    JsonDocument.Parse(body.Substring(ReplayReceiptMarker.Length).TrimStart())
