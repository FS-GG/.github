...
module RemoteClient =
    ...
    type Result =
        | Acknowledged of RemoteContract.Receipt
        | Unacknowledged of string
    ...
    let private verify scope batch digest bytes =
        ...
        | Ok receipt when
            receipt.Scope = scope
            && receipt.BatchId = batch
            && String.Equals(receipt.Digest, digest, StringComparison.Ordinal)
            ->
            Acknowledged receipt
        | _ -> Unacknowledged "receipt-unavailable"

    let private readBounded (response: HttpResponseMessage) (token: CancellationToken) =
        task {
    ...
    let private request
        (client: HttpClient)
        (config: RemoteContract.ClientConfig)
                ...
                | Some token ->
                    ...
                    while attempt < 5 && not finished && not cancellationToken.IsCancellationRequested do
                        ...
                        try
                            ...
                            use! response =
                                client.SendAsync(message, HttpCompletionOption.ResponseHeadersRead, attemptCts.Token)

                            if int response.StatusCode >= 300 && int response.StatusCode < 400 then
                                finished <- true
                                answer <- Unacknowledged "redirect-refused"
                            else
                                ...
                                | Some bytes when
                                    ...
                                    | Acknowledged _ ->
                                        finished <- true
                                        answer <- candidate
                                    | _ -> answer <- Unacknowledged "receipt-unavailable"
                                | Some bytes ->
                                    match RemoteContract.parseError bytes with
                                    | Some("identity-conflict" as code)
                                    | Some("unauthorized-scope" as code)
                                    | Some("unsupported-version" as code)
                                    | Some("invalid-request" as code)
                                    | Some("oversized-batch" as code) ->
                                        finished <- true
                                        answer <- Unacknowledged code
                                    | Some code -> answer <- Unacknowledged code
                                    | None -> answer <- Unacknowledged "receipt-unavailable"
                                | None -> answer <- Unacknowledged "receipt-unavailable"
                        with
                        | :? HttpRequestException -> answer <- Unacknowledged "receipt-unavailable"
                        | :? OperationCanceledException when not cancellationToken.IsCancellationRequested ->
                            answer <- Unacknowledged "receipt-unavailable"
    ...
    let submitWithClientForTesting client config resolve scope bytes cancellationToken =
        task {
    ...
    let lookupWithClientForTesting client config resolve scope (batchId: string) expectedDigest cancellationToken =
        request
            ...
            scope
            HttpMethod.Get
            ($"v1/receipts/{Uri.EscapeDataString(batchId)}")
            None
            batchId
            expectedDigest
            cancellationToken

    let submit config resolve scope bytes cancellationToken =
        task {
    ...
    let lookup config resolve scope batchId expectedDigest cancellationToken =
        task {
            use handler = createHandler ()
            use client = new HttpClient(handler)
