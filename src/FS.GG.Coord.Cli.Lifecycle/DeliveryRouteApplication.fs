...
module DeliveryRouteApplication =
    ...
    let private route (root: JsonElement) =
        ...
        | true, value when value.ValueKind = JsonValueKind.String && value.GetString() = "lightweight" ->
            Ok(Some DeliveryRoute.Lightweight)
        | true, value when value.ValueKind = JsonValueKind.String && value.GetString() = "sdd-required" ->
            Ok(Some DeliveryRoute.SddRequired)
        | true, value when value.ValueKind = JsonValueKind.Null -> Ok None
    ...
    let decodeStructured (raw: string) =
                ...
                | Ok schema,
                  ...
                  Ok digest ->
                    Ok(
                        ...
                        : StructuredDecision.RouteRecord
                    )
                | _ ->
                    let error =
                        function
                    ...
                    [
                        yield! error schema
                        yield! error subject
                        yield! error revision
                        yield! error previous
                        yield! error scope
                        yield! error dependencies
                        yield! error touchSet
                        yield! error policy
                        yield! error selectedRoute
                        yield! error agent
                        yield! error timestamp
                        yield! error reasons
                        yield! error rationale
                        yield! error work
                        yield! error spec
                        yield! error gates
                        yield! error digest
        ...
        with :? JsonException as error ->
            Error $"record is not valid JSON: {error.Message}"
    ...
    let run opts =
        ...
        | [ "validate"; subject; path ] ->
            ...
            | Ok record ->
                ...
                | Error errors ->
                    render "refusal" errors
                    ExitCode.toInt ExitCode.Error
        | [ "record"; _ ]
        | [ "show"; _ ] ->
            render "refusal" [ "route record/show require the live GitHub receipt boundary" ]
            ExitCode.toInt ExitCode.Error
        | _ ->
            render "refusal" [ "usage: route validate <subject> <record.json>" ]
            ExitCode.toInt ExitCode.Error
