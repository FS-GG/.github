...
module ReviewApplication =
    ...
    let private comments (element: JsonElement) : Driver.ReviewComment list =
        ...
        if value.ValueKind <> JsonValueKind.Array then
            invalidArg "comments" "must be an array"

        value.EnumerateArray()
        |> Seq.map (fun comment ->
            if comment.ValueKind <> JsonValueKind.Object then
                invalidArg "comments" "must contain objects"

            ({
                Id = readInt64 "id" comment
                Url = readString "url" comment
                Body = readString "body" comment
            }
    ...
    let private snapshot
        ...
        try
            ...
            if bindingElement.ValueKind <> JsonValueKind.Object then
                invalidArg "binding" "must be an object"

            let factsElement = required "facts" root

            if factsElement.ValueKind <> JsonValueKind.Object then
                invalidArg "facts" "must be an object"
    ...
    let private waitReceiptJson (receipt: ReviewWait.WaitReceipt) =
        {|
            ...
            kind =
                ...
                | ReviewWait.InitialReview -> "initial-review"
                | ReviewWait.RepairConfirmation -> "repair-confirmation"
    ...
    let private waitAuthority
        (binding: Review.Binding)
        (facts: Review.Facts)
        ordinaryExhaustionCompleted
        (state: Review.State)
        (action: Review.NextAction)
        (waitState: ReviewWait.State option)
        =
        let dispatchAuthority =
            match action with
            | Review.DispatchCritic ->
                Some(ReviewWait.InitialReview, ReviewWait.generationToken binding.HeadSha ReviewWait.InitialReview 0)
            | Review.DispatchSuccessor _ ->
                let round = stateRound state |> Option.defaultValue binding.Round

                Some(
                    ReviewWait.RepairConfirmation,
                    ReviewWait.generationToken binding.HeadSha ReviewWait.RepairConfirmation round
                )
            | _ -> None
        ...
        | Some(ReviewWait.Waiting receipt), Some(expectedKind, expectedGeneration) when
            receipt.Kind = expectedKind && receipt.ReviewGeneration = expectedGeneration
            ->
            Ok()
        | Some(ReviewWait.Waiting receipt), Some(_, expectedGeneration) ->
            Error
                [
                    $"the durable review wait does not authorize %s{actionName action}: expected generation '%s{expectedGeneration}', got '%s{receipt.ReviewGeneration}' / %A{receipt.Kind}"
                ]
        | Some(ReviewWait.Waiting receipt), None ->
            Error
                [
                    $"review generation '%s{receipt.ReviewGeneration}' remains unconsumed; record its completion, cancellation, or timeout before advancing"
                ]
        | Some ReviewWait.NoReceipt, Some _ ->
            Error
                [
                    $"%s{actionName action} requires a durable review-wait entry before dispatch"
                ]
        | Some(ReviewWait.Completed _), Some _
        | Some(ReviewWait.Cancelled _), Some _ ->
            Error
                [
                    $"%s{actionName action} requires a new durable review-wait entry for this generation"
                ]
    ...
    let private renderVerdict
        ...
        | Ok verdict ->
            ...
            let waitStatus =
                if hasRecordedRepairPhaseEntry facts ordinaryExhaustionCompleted then
                    Some "repairPhaseEntry"
                elif ordinaryExhaustionCompleted then
                    Some "ordinaryExhaustion"
                else
                    waitStatus

            match
                waitAuthority binding facts ordinaryExhaustionCompleted verdict.State verdict.NextAction waitState,
                opts.Render
            with
            | Error reasons, Json ->
                printfn
            ...
            | Ok(), Text ->
                ...
                | Some reason -> printfn "%s — %s: %s" (stateName verdict.State) (actionName verdict.NextAction) reason
