...
module IntakeApplication =
    let private error message =
        printfn
            "{\"schema\":\"fsgg.coord.intake-result/v1\",\"kind\":\"refusal\",\"reason\":%s}"
        ...
        try
            ...
            if root.ValueKind <> JsonValueKind.Object then
                ...
                let unknown =
                    ...
                    |> Seq.tryFind (fun property -> not (known.Contains property.Name))
                    ...
                    let paths =
                        ...
                        | true, value when
                            ...
                            && (value.EnumerateArray()
                                |> Seq.forall (fun item -> item.ValueKind = JsonValueKind.String))
                            ->
                            Ok(value.EnumerateArray() |> Seq.map (fun item -> item.GetString()) |> List.ofSeq)
                        | true, _ -> Error "paths must be an array of strings"
                        | false, _ -> Error "paths is required"

                    let disposition =
                        match root.TryGetProperty "disposition" with
                        | true, value when value.ValueKind = JsonValueKind.String && value.GetString() = "create" ->
                            Ok Intake.Create
                        | true, value when value.ValueKind = JsonValueKind.String && value.GetString() = "reuse" ->
                            Ok Intake.Reuse
                                ...
                                let draft: Intake.Draft =
                                    {
                                        ...
                                        Paths = paths
                                        Class = Types.itemClassWireName parsed
                                        Status = status
    ...
    let validateLivePaths (draft: Intake.Draft) =
        ...
        let rec gitRoot (cursor: DirectoryInfo) =
            ...
            then
                Some cursor.FullName
            else
                gitRoot cursor.Parent
        ...
        let originMatches root =
            try
                ...
                let url =
                    if rawUrl.EndsWith(".git", StringComparison.OrdinalIgnoreCase) then
                        rawUrl.Substring(0, rawUrl.Length - 4)
                    else
                        rawUrl
                ...
                child.ExitCode = 0
                && (url.EndsWith("/" + expected, StringComparison.OrdinalIgnoreCase)
                    || url.EndsWith(":" + expected, StringComparison.OrdinalIgnoreCase))
            with _ ->
                false
        ...
        | Some currentRoot ->
            ...
            let rec ancestors (directory: DirectoryInfo) =
                if isNull directory then
                    ...
                    directory.FullName :: ancestors directory.Parent
            ...
            match
                candidates
                |> List.tryFind (fun path -> Directory.Exists path && originMatches path)
            with
            | None ->
                Error
                    $"cannot locate a live checkout of target repository %s{draft.Owner}/%s{draft.Repository} for path validation"
            | Some root ->
                let missing =
                    draft.Paths
                ...
                if List.isEmpty missing then
                    ...
                    Error $"paths do not exist in target repository %s{draft.Owner}/%s{draft.Repository}: {rendered}"
        ...
        | [ action; path ] ->
            ...
            | Ok draft ->
                ...
                | "validate", Ok _ ->
                    ...
                    | Ok() ->
                        printfn
                            "{\"schema\":\"fsgg.coord.intake-result/v1\",\"kind\":\"validated\",\"draftId\":%s,\"writes\":0}"
                            (JsonSerializer.Serialize draft.Id)

                        ExitCode.toInt ExitCode.Green
                | "validate", Error findings ->
                    error (
                        findings
                        |> List.map (fun finding -> $"{finding.Field} {finding.Detail}")
                        |> String.concat "; "
                    )
                | "apply", Ok _ -> error "live intake apply is not wired; validation performed zero writes"
                | "apply", Error findings ->
                    error (
                        findings
                        |> List.map (fun finding -> $"{finding.Field} {finding.Detail}")
                        |> String.concat "; "
