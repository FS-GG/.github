...
module WorkspaceTelemetryApplication =
    ...
    let private option name args =
        ...
        |> List.indexed
        |> List.rev
        |> List.tryPick (fun (i, v) -> if v = name then List.tryItem (i + 1) args else None)

    let private options name args =
        args
        |> List.indexed
        |> List.choose (fun (i, v) -> if v = name then List.tryItem (i + 1) args else None)
    ...
    let private configPath explicitPath =
        explicitPath |> Option.defaultWith defaultConfig |> Path.GetFullPath

    let private safeAncestors path =
        let rec loop (directory: DirectoryInfo) =
            if isNull directory then
                true
            elif directory.Exists && not (isNull directory.LinkTarget) then
                false
            else
    ...
    let private checkPrivateFile path =
        ...
        if not info.Exists then
            ...
            not (safeAncestors path)
            || not (isNull info.LinkTarget)
            || not (ownedByCaller path)
            || info.Length > 65536L
        then
    ...
    let private text (node: JsonObject) (name: string) =
        ...
        | null -> None
        | value -> value.GetValue<string>() |> Some

    let private parseAssociation (node: JsonObject) =
        try
            if not (exactObject node [ "workspaceId"; "producerId"; "streamId"; "repositories"; "destination" ]) then
                ...
                let repositories =
                    node["repositories"].AsArray() |> Seq.map _.GetValue<string>() |> Seq.toList

                let destination = node["destination"].AsObject()

                if
                    [ workspace; producer; stream ] |> List.exists (TelemetryReceipt.validId >> not)
                    || repositories.IsEmpty
                    || repositories |> List.exists (validRepo >> not)
                    || repositories.Length <> (repositories |> List.distinct |> List.length)
                then
                    Error "configuration-identity"
                else
                    ...
                    | Some "local" when exactObject destination [ "kind"; "storeRoot" ] ->
                        ...
                        if not (Path.IsPathFullyQualified root) then
                            ...
                            Ok
                                {
                                    ...
                                    Destination = Local(Path.GetFullPath root)
                                }
                    | Some "remote" when
                        exactObject destination [ "kind"; "endpoint"; "credentialReference"; "spoolRoot" ]
                        ->
                        ...
                        let candidate: RemoteContract.ClientConfig =
                            {
                                Endpoint = endpoint
                            ...
                            Ok
                                {
                                    ...
                                    Stream = stream
                                    Repositories = repositories
                                    Destination = Remote(endpoint, credential, Path.GetFullPath spool)
                                }
    ...
    let private load explicitPath =
        ...
        | Ok() ->
            try
                ...
                if not (noDuplicateProperties document.RootElement) then
                    Error [ "configuration-duplicate-property" ]
                else
                    let root = JsonNode.Parse(bytes).AsObject()

                    if
                        not (exactObject root [ "schema"; "engine"; "associations"; "retiredAssociations" ])
                        || text root "schema" <> Some Schema
                    then
                        ...
                        if
                            ...
                            || engine.Contains('\\')
                        then
                            Error [ "configuration-engine" ]
                        else
                            let parsed =
                                root["associations"].AsArray()
                                |> Seq.map (fun x -> parseAssociation (x.AsObject()))
                                |> Seq.toList
                            ...
                            | None ->
                                ...
                                let retiredParsed =
                                    root["retiredAssociations"].AsArray()
                                    |> Seq.map (fun x -> parseAssociation (x.AsObject()))
                                    |> Seq.toList
                                ...
                                | None ->
                                    ...
                                    let repos =
                                        associations |> List.collect _.Repositories |> List.map _.ToLowerInvariant()
                                    ...
                                    if
                                        repos.Length <> (repos |> List.distinct |> List.length)
                                        || scopes.Length <> (scopes |> List.distinct |> List.length)
                                        || workspaces.Length <> (workspaces |> List.distinct |> List.length)
                                        || producers.Length <> (producers |> List.distinct |> List.length)
                                    then
    ...
    let private select supplied config =
        ...
        | Some repo ->
            match
                ...
                |> List.filter (fun a ->
                    a.Repositories
                    |> List.exists (fun value -> String.Equals(value, repo, StringComparison.OrdinalIgnoreCase)))
            with
            | [ one ] -> Ok one
            | [] -> Error [ "workspace-unassociated" ]
            | _ -> Error [ "workspace-ambiguous" ]

    let private scope a : TelemetryReceipt.Scope =
        {
    ...
    let private associationDigest association =
        let destination =
            ...
            | Remote(endpoint, reference, spool) -> [ "remote"; endpoint.AbsoluteUri; reference; spool ]

        use bytes = new MemoryStream()
        use writer = new BinaryWriter(bytes, Encoding.UTF8, true)

        for value in
            [ association.Workspace; association.Producer; association.Stream ]
            @ destination
            @ association.Repositories do
    ...
    let private envelope (a: Association) (payload: byte array) =
        try
            ...
            let batchId =
                if TelemetryReceipt.validId originalIngestId then
                    originalIngestId
                else
                    "batch-"
                    + Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes originalIngestId)).ToLowerInvariant()

            payloadNode["ingestId"] <- batchId

            let normalizedPayload =
                Encoding.UTF8.GetBytes(payloadNode.ToJsonString(JsonSerializerOptions(WriteIndented = false)))

            TelemetryStore.parseBatch normalizedPayload
            |> Result.defaultWith (fun _ -> invalidArg "payload" "invalid ingest")
            |> ignore

            let root = JsonObject()
            root["schema"] <- "fsgg.telemetry.envelope/1"
            root["workspaceId"] <- a.Workspace
            root["producerId"] <- a.Producer
            root["streamId"] <- a.Stream
            ...
            TelemetryReceipt.parse bytes
            |> Result.map (fun value -> Encoding.UTF8.GetBytes value.Canonical, value)
        with _ ->
    ...
    let private ensurePrivateDirectory (path: string) =
        try
            if not (Path.IsPathFullyQualified path) then
                Error [ "spool-path-invalid" ]
            else
                let parent = DirectoryInfo(Path.GetDirectoryName path)

                if
                    not parent.Exists
                    || not (safeAncestors parent.FullName)
                    || not (isNull parent.LinkTarget)
                    || not (ownedByCaller parent.FullName)
                    || (not (OperatingSystem.IsWindows())
                    ...
                    if
                        ...
                        || (not (OperatingSystem.IsWindows())
                            && File.GetUnixFileMode(path)
                               <> (UnixFileMode.UserRead ||| UnixFileMode.UserWrite ||| UnixFileMode.UserExecute))
                    then
                        Error [ "spool-path-unsafe" ]
                    else
                        Ok path
        with _ ->
            Error [ "spool-unavailable" ]
    ...
    let private remoteOne
        ...
        | Ok parsed ->
            ...
            let result =
                (RemoteClient.lookup config resolve (scope a) parsed.BatchId parsed.Digest token)
                    .GetAwaiter()
                    .GetResult()

            let result =
                match result with
                | RemoteClient.Unacknowledged "receipt-unavailable" ->
                    (RemoteClient.submit config resolve (scope a) bytes token).GetAwaiter().GetResult()
                | other -> other

            match result with
            | RemoteClient.Acknowledged receipt when
                    ...
                    let outcome =
                        Encoding.UTF8.GetBytes(
                            JsonSerializer.Serialize
                                {|
                                    schema = "fsgg.telemetry.workspace-outcome/1"
    ...
    let private withLock config action =
        ...
        try
            if OperatingSystem.IsLinux() then
                let descriptor =
                    Native.openPrivateFile (lockPath, 0x2 ||| 0x40 ||| 0x20000 ||| 0x80000, 0x180u)

                if descriptor < 0 then
                    Error [ "configuration-unsafe" ]
                else
                    try
                        if Native.flock (descriptor, Native.LockExclusive ||| Native.LockNonBlocking) <> 0 then
                ...
                action ()
        with
        | :? IOException -> Error [ "configuration-busy" ]
                ...
                Ok
                    {
                        ...
                        Digest = associationDigest association
                    }
    ...
    let tryPublishBound
        configArg
        repositoryArg
        (expectedProducer: string option)
        (expectedBinding: string option)
        payload
        =
            ...
            try
                withLock path (fun () ->
                    ...
                    | Ok config ->
                        ...
                        | Error errors -> Error errors
                        | Ok association when expectedProducer.IsSome && expectedProducer <> Some association.Producer ->
                            Error [ "association-stale" ]
                        | Ok association when
                            expectedBinding.IsSome && expectedBinding <> Some(associationDigest association)
                            ->
                            Error [ "association-stale" ]
                        | Ok association ->
                                    ...
                                    | Ok directory ->
                                        ...
                                        let outcomeFile =
                                            Path.Combine(
                                                directory,
                                                "outcomes",
                                                TelemetryReceipt.key association.Producer parsed.BatchId + ".json"
                                            )

                                        let outcomeConflict =
                                            if not (File.Exists outcomeFile) then
                                                false
                                            else
                                                let info = FileInfo outcomeFile

                                                if info.Length > 4096L || not (isNull info.LinkTarget) then
                                                    true
                                                else
                                                    use outcome = JsonDocument.Parse(File.ReadAllBytes outcomeFile)

                                                    outcome.RootElement.GetProperty("digest").GetString()
                                                    <> parsed.Digest

                                        if outcomeConflict then
                                            ...
                                            then
                                                ...
                                                | Ok prior when prior.Digest = parsed.Digest ->
                                                    remoteOne
                                                        association
                                                        endpoint
                                                        reference
                                            ...
                                            if
                                                files.Length >= 128 || total + int64 bytes.Length > 8L * 1024L * 1024L
                                            then
                                                Error [ "spool-overload" ]
                                            else
    ...
    let tryPublishExpected configArg repositoryArg expectedProducer payload =
        tryPublishBound configArg repositoryArg expectedProducer None payload

    let tryPublish configArg repositoryArg payload =
        tryPublishExpected configArg repositoryArg None payload
                        ...
                        | Ok association ->
                            ...
                            | Remote(endpoint, reference, spool) ->
                                ...
                                | Ok directory ->
                                    ...
                                    let files =
                                        Directory.EnumerateFiles(directory, "*.ready") |> Seq.truncate 16 |> Seq.toList

                                    let results =
                                        files |> List.map (remoteOne association endpoint reference deadline.Token)
    ...
    let tryDrain configArg repositoryArg =
        tryDrainBound configArg repositoryArg None

    let tryDrainExpected configArg repositoryArg expectedBinding =
        tryDrainBound configArg repositoryArg expectedBinding

    let tryPublishBinding (binding: Binding) payload =
        tryPublishBound
            (Some binding.ConfigPath)
            (Some binding.Repository)
            (Some binding.Producer)
            (Some binding.Digest)
            payload

    let tryDrainBinding (binding: Binding) =
        tryDrainBound (Some binding.ConfigPath) (Some binding.Repository) (Some binding.Digest)
    ...
    let tryLocalStoreRootBound (binding: Binding) =
        ...
        |> Result.bind (fun association ->
            if associationDigest association <> binding.Digest then
                Error [ "association-stale" ]
            else
                Ok(
                    match association.Destination with
    ...
    let private serialize (path: string) (associations: Association list) (retired: Association list) =
        ...
        for a in associations do
            ...
            | Local store ->
                ...
                destination["storeRoot"] <- store
            | Remote(endpoint, reference, spool) ->
                destination["kind"] <- "remote"
                destination["endpoint"] <- endpoint.AbsoluteUri
                destination["credentialReference"] <- reference
                destination["spoolRoot"] <- spool
        ...
        if bytes.Length > 65536 then
            invalidOp "configuration-capacity"

        let directory = Path.GetDirectoryName path
        let directoryInfo = DirectoryInfo directory

        if
            not directoryInfo.Exists
            || not (safeAncestors directory)
            || not (isNull directoryInfo.LinkTarget)
            || (not (OperatingSystem.IsWindows())
    ...
    let private activate kind args =
        try
            ...
            then
                ...
                match
                    required "--workspace", required "--producer", required "--stream", options "--repository" args
                with
                | Ok workspace, Ok producer, Ok stream, repositories when
                    [ workspace; producer; stream ] |> List.forall TelemetryReceipt.validId
                    && not repositories.IsEmpty
                    && repositories |> List.forall validRepo
                    && repositories.Length =
                        (repositories |> List.map _.ToLowerInvariant() |> List.distinct |> List.length)
                    ->
                    let destination =
                        match
                            kind,
                            option "--store-root" args,
                            option "--endpoint" args,
                            option "--credential-reference" args,
                            option "--spool-root" args
                        with
                        | "local", Some root, _, _, _ when Path.IsPathFullyQualified root ->
                            Some(Local(Path.GetFullPath root))
                        | "remote", _, Some endpoint, Some reference, Some spool when
                            Path.IsPathFullyQualified spool && TelemetryReceipt.validId reference
                            ->
                            ...
                            | Ok _ -> Some(Remote(uri, reference, Path.GetFullPath spool))
                            | Error _ -> None
                        | _ -> None
                    ...
                    | Some(Local root as selected) ->
                        ...
                        TelemetryStoreApplication.initialize root assessment
                        |> Result.bind (fun _ ->
                    ...
                    | Some(Remote(_, reference, spool) as selected) ->
                        let available =
                            (credential reference CancellationToken.None).GetAwaiter().GetResult().IsSome

                        if not available then
                            Error [ "credential-unavailable" ]
                        else
                            ensurePrivateDirectory spool
                            |> Result.map (fun _ ->
                                {
                                    Workspace = workspace
                                    Producer = producer
                    ...
                    |> Result.bind (fun association ->
                        ...
                        if not (Directory.Exists directory) then
                            ...
                            if
                                ...
                                || not (isNull parent.LinkTarget)
                            then
                                invalidOp "configuration-parent-unsafe"
                        ...
                        withLock path (fun () ->
                            let existing, retired =
                                match load (Some path) with
                                | Ok c -> c.Associations, c.Retired
                                | Error [ "unconfigured" ] -> [], []
                                | Error errors -> raise (InvalidOperationException(String.concat ";" errors))

                            let repositories =
                                association.Repositories |> List.map _.ToLowerInvariant() |> Set.ofList

                            if
                                (existing @ retired) |> List.exists (fun a -> a.Producer = association.Producer)
                                || existing
                                   |> List.exists (fun a ->
                                       a.Workspace = association.Workspace
                                       || a.Repositories
                                          |> List.exists (fun repo ->
                                              Set.contains (repo.ToLowerInvariant()) repositories))
                            then
                                Error [ "association-conflict" ]
                            else
                                serialize path (existing @ [ association ]) retired
                                Ok "activated"))
    ...
    let private status args =
        ...
        | Error [ "unconfigured" ] ->
            Ok "{\"schema\":\"fsgg.telemetry.workspace-status/1\",\"status\":\"unconfigured\"}\n"
        | Error errors -> Error errors
        | Ok config ->
            ...
            | Ok a ->
                let kind, pending, lossy, census =
                    ...
                    | Local root ->
                        match
                            TelemetryStoreApplication.receiptCapacity
                                root
                                (TelemetryStoreApplication.assessProductionRoot root)
                        with
                ...
                Ok(
                    JsonSerializer.Serialize
                        {|
                            schema = "fsgg.telemetry.workspace-status/1"
                            status = "configured"
                            workspaceId = a.Workspace
                            producerId = a.Producer
                            streamId = a.Stream
                            destination = kind
                            pending = pending
                ...
                Ok(
                    JsonSerializer.Serialize
                        {|
                            ...
                            configPath = config.Path
                            repository = repo
                            producerId = association.Producer
                            bindingDigest = associationDigest association
                            destination = kind
    ...
    let private associate args =
        ...
        if not (File.Exists path) then
            Error [ "unconfigured" ]
        elif workspace.IsNone || (additions @ removals |> List.exists (validRepo >> not)) then
                    ...
                    | Some index ->
                        ...
                        let repositories =
                            (current.Repositories
                             |> List.filter (fun r ->
                                 removals
                                 |> List.exists (fun x -> String.Equals(x, r, StringComparison.OrdinalIgnoreCase))
                                 |> not))
                            @ additions
                            |> List.distinctBy _.ToLowerInvariant()

                        if repositories.IsEmpty then
                            Error [ "workspace-requires-repository" ]
                        elif
                            config.Associations
                        ...
                        then
                            ...
                            let updated =
                                ...
                                |> List.mapi (fun i a ->
                                    if i = index then
                                        { a with Repositories = repositories }
                                    else
                                        a)

                            serialize path updated config.Retired
                            Ok "associated")

    let private pending association =
        match association.Destination with
        | Local root ->
            TelemetryStoreApplication.recoverReceiptCapacity root (TelemetryStoreApplication.assessProductionRoot root)
            |> Result.map (fun (_, count, _) -> count)
        | Remote(_, _, spool) ->
            try
                Ok(
                    if Directory.Exists spool then
                        Directory.EnumerateFiles(spool, "*.ready")
                        |> Seq.truncate 1
                        |> Seq.length
                        |> int64
                    else
    ...
    let private cutover args =
        ...
        if not (File.Exists path) then
            ...
            withLock path (fun () ->
                match
                    load (Some path),
                    option "--workspace" args,
                    option "--producer" args,
                    option "--stream" args,
                    option "--to" args
                with
                | Ok config, Some workspace, Some producer, Some stream, Some destinationKind when
                    [ workspace; producer; stream ] |> List.forall TelemetryReceipt.validId
                    ->
                    match config.Associations |> List.tryFind (fun a -> a.Workspace = workspace) with
                    | None -> Error [ "workspace-unassociated" ]
                    | Some old when
                        (config.Associations @ config.Retired)
                        |> List.exists (fun a -> a.Producer = producer)
                        ->
                        Error [ "identity-preserving-import-required" ]
                    | Some old ->
                        ...
                        | Ok _ ->
                            let selected =
                                ...
                                | "remote", _, Some endpoint, Some reference, Some spool when
                                    ...
                                    let remote: RemoteContract.ClientConfig =
                                        {
                                            Endpoint = uri
                                            CredentialReference = reference
                                        }

                                    match RemoteContract.validateClientConfig remote with
                                    | Ok _ -> Some(Remote(uri, reference, Path.GetFullPath spool))
                                    | Error _ -> None
                                | _ -> None
                            ...
                            let selectedResult =
                                ...
                                | Some(Local root as destination) ->
                                    ...
                                    |> Result.map (fun _ -> destination)
                                | Some(Remote(_, reference, spool) as destination) ->
                                    if
                                        not (
                                            (credential reference CancellationToken.None)
                                                .GetAwaiter()
                                                .GetResult()
                                                .IsSome
                                        )
                                    then
                                        ...
                                        ensurePrivateDirectory spool |> Result.map (fun _ -> destination)
                            ...
                            |> Result.map (fun destination ->
                                ...
                                let active =
                                    config.Associations
                                    |> List.map (fun a -> if a.Workspace = workspace then replacement else a)

                                serialize path active (config.Retired @ [ old ])
                                "cutover")
    ...
    let run action args =
            ...
            | "submit" ->
                ...
                | Some path ->
                    tryPublishBound
                        ...
                        (option "--producer" args)
                        (option "--binding-digest" args)
                        (File.ReadAllBytes path)
                | None -> Error [ "--input is required" ]
            | "drain" ->
                tryDrainExpected (option "--config" args) (option "--repository" args) (option "--binding-digest" args)
        ...
        | Error errors ->
            errors
            |> List.iter (fun e -> Console.Error.WriteLine("fsgg-coord-engine: telemetry workspace: " + e))
