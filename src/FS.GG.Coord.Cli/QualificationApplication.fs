...
module QualificationApplication =
    ...
    type private OperationResolver =
        {
            Id: string
    ...
    type private Execution =
        {
            ...
            Tools: ToolResolver list
            Operations: OperationResolver list
            Fixtures: FixtureResolver list
            HostedObservationPaths: string list
            ObligationCommentPaths: string list
        }
        ...
        try
            ...
            strictObject
                ...
                [
                    ...
                    "operations"
                    "fixtures"
                    "hostedObservationPaths"
                    "obligationCommentPaths"
                ]
                root

            if text "execution" "schema" root <> ExecutionSchema then
                raise (FormatException($"schema must be '%s{ExecutionSchema}'"))

            let timeout = root.GetProperty("timeoutSeconds").GetInt32()

            if timeout < 1 || timeout > 3600 then
                raise (FormatException("timeoutSeconds must be between 1 and 3600"))
            ...
            let tools =
                ...
                |> List.mapi (fun index item ->
...

                    {
                        ...
                        Path = text label "path" item
                        VersionArguments = strings label "versionArguments" item
                    })
            ...
            let operations =
                ...
                |> List.mapi (fun index item ->
                    ...
                    {
                        Id = text label "id" item
                        Arguments = strings label "arguments" item
                        Artifacts = strings label "artifacts" item
                    })
            ...
            let fixtures =
                ...
                |> List.mapi (fun index item ->
                    ...
                    {
                        MutationId = text label "mutationId" item
                        ExecutorId = text label "executorId" item
            ...
            Ok
                {
                    Checkout = text "execution" "checkout" root
    ...
    let private duplicates (label: string) (values: string list) =
        ...
        |> List.choose (fun (value, count) ->
            if count > 1 then
                Some($"duplicate %s{label} identity '%s{value}'")
            else
                None)
    ...
    let private digestText (values: string list) =
        values
        |> List.map frame
        |> String.concat "|"
    ...
    let private digestFile (path: string) =
        File.ReadAllBytes path |> CanonicalJson.sha256
    ...
    let private under (root: string) (path: string) =
        ...
        if candidate = rootPath || candidate.StartsWith(prefix, StringComparison.Ordinal) then
            Ok candidate
        else
            Error $"artifact path escapes checkout: %s{path}"
    ...
    let run inputPath executionPath =
        ...
        | Ok input, Ok execution ->
            ...
            if not (Directory.Exists checkout) then
                errors.Add($"checkout does not exist: %s{checkout}")

            if checkout = fullPath (Directory.GetCurrentDirectory()) then
                errors.Add "qualification checkout must be isolated from the invoking checkout"
            ...
            if manifestToolIds <> resolverToolIds then
                errors.Add "execution tools do not exactly match the closed tool manifest"
            ...
            if inputOperationIds <> resolverOperationIds then
                errors.Add "execution operations do not exactly match the qualification input"

            for operation in input.Operations do
                if not (Set.contains operation.Tool.Id manifestToolIds) then
                    errors.Add($"operation '%s{operation.Id}' names undeclared tool '%s{operation.Tool.Id}'")

                if
                    operation.Executor.Id <> input.Executor.Id
                    || operation.Executor.Role <> input.Executor.Role
                then
            ...
            if inputMutationIds <> resolverMutationIds then
                errors.Add "execution fixtures do not exactly match the qualification mutations"

            if execution.HostedObservationPaths.Length < 2 then
                errors.Add "at least two hosted observation paths are required"

            if
                execution.Executor.Id <> input.Executor.Id
                || execution.Executor.Role <> input.Executor.Role
            then
            ...
            if String.IsNullOrWhiteSpace executorPath || not (File.Exists executorPath) then
                errors.Add "executing qualification assembly identity is unavailable"
            ...
            if errors.Count > 0 then
                ...
                let resolvedExecutor =
                    { input.Executor with
                        ImplementationSha256 = digestFile executorPath
                    }

                let resolvers =
                    execution.Tools |> List.map (fun tool -> tool.Id, tool) |> Map.ofList

                let operationResolvers =
                    execution.Operations
                    |> List.map (fun operation -> operation.Id, operation)
                    |> Map.ofList

                let fixtureResolvers =
                    execution.Fixtures
                    |> List.map (fun fixture -> fixture.MutationId, fixture)
                    |> Map.ofList
                ...
                for declared in input.ToolManifest do
                    ...
                    if not (File.Exists path) then
                        errors.Add($"tool '%s{declared.Id}' does not exist")
                    else
                ...
                if errors.Count > 0 then
                    ...
                    let toolMap =
                        resolvedTools
                        |> Seq.map (fun (id, identity, path) -> id, (identity, path))
                        |> Map.ofSeq
                    ...
                    let checkoutFacts =
                        ...
                        | Some(_, git) ->
                            ...
                            with
                            | Ok head, Ok status when head.ExitCode = 0 && status.ExitCode = 0 ->
                                Ok(head.Stdout.Trim(), String.IsNullOrEmpty status.Stdout)
                    ...
                    | Ok(head, _) when head <> input.SubjectRevision ->
                        Error
                            [
                                $"checkout HEAD '%s{head}' does not match subject revision '%s{input.SubjectRevision}'"
                            ]
                    | Ok(_, initiallyClean) ->
                        ...
                        for template in input.Operations do
                            ...
                            let artifactPaths =
                                ...
                                |> List.choose (fun artifact ->
                                    ...
                                    | Error reason ->
                                        errors.Add reason
                                        None
                                    | Ok path when not (usedArtifacts.Add path) ->
                                        errors.Add($"artifact is reused by unrelated operations: %s{artifact}")
                                        None
                                    | Ok path when File.Exists path ->
                                        errors.Add(
                                            $"operation '%s{template.Id}' artifact exists before execution: %s{artifact}"
                                        )
                            ...
                            let executionResult =
                                if errors.Count <> operationErrorCount then
                                    Error "artifact preflight failed"
                                else
                                    execute ()
                            ...
                            | Ok result ->
                                ...
                                for artifact, path in artifactPaths do
                                    if not (File.Exists path) then
                                        errors.Add($"operation '%s{template.Id}' artifact does not exist: %s{artifact}")
                                    else
                                        artifactDigests.Add(digestFile path)

                                let resultDigest =
                                    digestText [ string result.ExitCode; result.Stdout; result.Stderr ]

                                let replay =
                                    if template.Kind <> Qualification.FixedPoint then
                                        None
                                    else
                                        match execute () with
                                        | Error reason ->
                                            errors.Add(
                                                $"fixed-point replay '%s{template.Id}' failed to execute: %s{reason}"
                                            )
                                        ...
                                        | Ok replayResult ->
                                            ...
                                            if replayArtifactDigests <> List.ofSeq artifactDigests then
                                                errors.Add(
                                                    $"fixed-point replay '%s{template.Id}' changed artifact bytes"
                                                )

                                            Some(
                                                digestText
                                                    [
                                                        string replayResult.ExitCode
                                                        replayResult.Stdout
                                                        replayResult.Stderr
                                                    ]
                                            )
                        ...
                        if errors.Count > 0 then
                            ...
                            let observedById =
                                observed |> Seq.map (fun operation -> operation.Id, operation) |> Map.ofSeq
                            ...
                            let mutations =
                                ...
                                |> List.map (fun mutation ->
                                    ...
                                    if fixture.ExecutorId = resolvedExecutor.Id then
                                        errors.Add(
                                            $"mutation fixture '%s{mutation.Id}' reuses the production executor identity"
                                        )

                                    if fixture.ExecutorRole = resolvedExecutor.Role then
                                        errors.Add(
                                            $"mutation fixture '%s{mutation.Id}' reuses the production executor role"
                                        )

                                    if not (File.Exists fixturePath) then
                                        errors.Add($"mutation fixture '%s{mutation.Id}' implementation does not exist")

                                    let fixtureDigest =
                                        if File.Exists fixturePath then
                                            digestFile fixturePath
                                        else
                                            mutation.FixtureImplementationSha256

                                    if fixtureDigest = resolvedExecutor.ImplementationSha256 then
                                        errors.Add(
                                            $"mutation fixture '%s{mutation.Id}' reuses the production implementation"
                                        )
                                    ...
                                    with
                                    | Error reason ->
                                        errors.Add($"mutation fixture '%s{mutation.Id}' failed to execute: %s{reason}")
                                    | Ok result when result.ExitCode = 0 ->
                                        errors.Add($"mutation fixture '%s{mutation.Id}' did not refuse")
                                    | Ok result when result.Stderr.Trim() <> mutation.ExpectedRefusal ->
                                        errors.Add($"mutation fixture '%s{mutation.Id}' refusal did not match exactly")
                                    | Ok _ -> ()

                                    match observedById.TryFind mutation.OperationId with
                                    | Some operation ->
                                        { mutation with
                                            ObservedRefusal = operation.Refusal |> Option.defaultValue ""
                            ...
                            if errors.Count > 0 then
                                ...
                                let hosted =
                                    ...
                                    |> List.map (fun path ->
                                        ...
                                        | Error reason ->
                                            errors.Add reason
                                            None
                                        | Ok resolved when not (Set.contains resolved hostedArtifacts) ->
                                            errors.Add(
                                                $"hosted observation is not an artifact of a hosted operation: %s{path}"
                                            )

                                            None
                                        | Ok resolved when not (File.Exists resolved) ->
                                            errors.Add($"hosted observation does not exist: %s{path}")
                                            None
                                        | Ok resolved ->
                                            match
                                                QualificationEvidence.parseHostedSnapshot (File.ReadAllBytes resolved)
                                            with
                                            | Error reasons ->
                                                errors.AddRange reasons
                                                None
                                ...
                                let obligationComments =
                                    ...
                                    |> List.choose (fun path ->
                                        ...
                                        | Ok resolved when not (Set.contains resolved hostedArtifacts) ->
                                            errors.Add(
                                                $"obligation comment is not an artifact of a hosted operation: %s{path}"
                                            )

                                            None
                                        | Ok resolved when not (File.Exists resolved) ->
                                            errors.Add($"obligation comment does not exist: %s{path}")
                                            None
                                        | Ok resolved ->
                                            match
                                                QualificationEvidence.parseObligationReadback (
                                        ...
                                        ({
                                            ...
                                            Declarations =
                                                (if declarations.IsEmpty then
                                                     ...
                                                     declarations
                                                     |> List.map (fun item ->
                                                         Qualification.Obligation { Id = item.Id; Kind = item.Kind }))
                                            Readbacks =
                                                obligationComments
                                ...
                                | Error reasons -> errors.AddRange reasons
                                | Ok _ -> ()
                                ...
                                if errors.Count > 0 then
                                    ...
                                    let qualified =
                                        { input with
                                            CheckoutClean = initiallyClean
                                            Executor = resolvedExecutor
                                            Operations = List.ofSeq observed
                                            Mutations = mutations
    ...
    let runBoundToTree expectedTree inputPath executionPath =
        ...
        | Ok input, Ok execution ->
            ...
            if
                execution.Environment
                |> List.exists (fun (name, _) -> name.StartsWith("GIT_", StringComparison.OrdinalIgnoreCase))
            then
            ...
            for fixture in execution.Fixtures do
                if under checkout fixture.Path |> Result.isError then
                    errors.Add(
                        $"production qualification fixture '%s{fixture.MutationId}' escapes the candidate checkout"
                    )

            if not (File.Exists "/usr/bin/git") then
                errors.Add "pinned system git /usr/bin/git is unavailable"
                ...
                match
                    ...
                    runProcess
                        ...
                        "/usr/bin/git"
                        [ "status"; "--porcelain"; "--untracked-files=all" ],
                    runProcess execution.TimeoutSeconds checkout trustedEnvironment "/usr/bin/git" [ "ls-files"; "-v" ]
                with
                | Ok tree, Ok status, Ok index when tree.ExitCode = 0 && status.ExitCode = 0 && index.ExitCode = 0 ->
                    if tree.Stdout.Trim() <> expectedTree then
                        errors.Add(
                            $"trusted checkout tree '%s{tree.Stdout.Trim()}' does not match expected candidate tree '%s{expectedTree}'"
                        )

                    if not (String.IsNullOrEmpty status.Stdout) then
                        errors.Add "trusted candidate checkout is not clean"
                    ...
                    if hidden.Length > 0 then
                        errors.Add "trusted candidate checkout contains skip-worktree or assume-unchanged index entries"
            ...
            if errors.Count > 0 then
                ...
                let isolated =
                    Path.Combine(Path.GetTempPath(), "fsgg-qualification-" + Guid.NewGuid().ToString("n"))
                ...
                try
                    let clone =
                        runProcess
                            ...
                            trustedEnvironment
                            "/usr/bin/git"
                            [ "clone"; "--quiet"; "--no-hardlinks"; "--no-checkout"; checkout; isolated ]
                    ...
                    | Ok result when result.ExitCode <> 0 ->
                        Error
                            [
                                $"could not create isolated qualification checkout: %s{result.Stderr.Trim()}"
                            ]
                    | Ok _ ->
                        let isolatedEnvironment =
                            trustedEnvironment
                            |> List.map (fun (name, value) -> if name = "HOME" then name, isolated else name, value)
                        ...
                        match
                            runProcess
                                ...
                                isolatedEnvironment
                                "/usr/bin/git"
                                [ "checkout"; "--quiet"; "--detach"; input.SubjectRevision ]
                        with
                        | Error reason ->
                            Error [ $"could not materialize isolated qualification candidate: %s{reason}" ]
                        | Ok result when result.ExitCode <> 0 ->
                            Error
                                [
                                    $"could not materialize isolated qualification candidate: %s{result.Stderr.Trim()}"
                                ]
                        | Ok _ ->
                                    ...
                                    | Ok tracked when tracked.ExitCode = 0 ->
                                        match
                                            runProcess
                                                ...
                                                isolatedEnvironment
                                                "/usr/bin/git"
                                                [ "update-index"; "--skip-worktree"; "--"; artifact ]
                                        with
                                        | Ok marked when marked.ExitCode = 0 ->
                                            if File.Exists path then
                                                File.Delete path
                                        | value ->
                                            preparationErrors.Add(
                                                $"could not isolate tracked qualification artifact '%s{artifact}': %A{value}"
                                            )
                                    | _ ->
                                        if File.Exists path then
                                            preparationErrors.Add(
                                                $"untracked qualification artifact exists in fresh isolated checkout: %s{artifact}"
                                            )

                            if preparationErrors.Count > 0 then
                                Error(List.ofSeq preparationErrors)
                            else
                                ...
                                for environment in executionNode["environment"].AsArray() do
                                    ...
                                    if entry["name"].GetValue<string>() = "HOME" then
                                        entry["value"] <- JsonValue.Create isolated
                                ...
                                let remapPath (container: JsonNode) =
                                    ...
                                    if Path.IsPathFullyQualified path && under checkout path |> Result.isOk then
                                        entry["path"] <-
                                            JsonValue.Create(
                                                Path.Combine(isolated, Path.GetRelativePath(checkout, path))
                                            )

                                for tool in executionNode["tools"].AsArray() do
                                    remapPath tool

                                for fixture in executionNode["fixtures"].AsArray() do
                                    remapPath fixture

                                let isolatedExecution =
                                    Path.Combine(
                                        Path.GetTempPath(),
                                        "fsgg-qualification-execution-" + Guid.NewGuid().ToString("n") + ".json"
                                    )

                                try
                                    File.WriteAllText(
                                        isolatedExecution,
                                        executionNode.ToJsonString(),
                                        UTF8Encoding(false)
                                    )
                                    ...
                                    if unchanged then
                                        ...
                                        Error
                                            [
                                                "trusted candidate checkout changed during isolated production qualification"
                                            ]
                                finally
                                    if File.Exists isolatedExecution then
                                        File.Delete isolatedExecution
                finally
                    if Directory.Exists isolated then
