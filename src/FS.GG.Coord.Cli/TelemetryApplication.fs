...
module TelemetryApplication =
    ...
    let private emitUsage (args: string list) (format: string) (rows: RuntimeUsage.UsageRow list) =
        ...
        | _, Some path when format = "json" ->
            ...
            File.AppendAllText(path, rendered, UTF8Encoding(false))
            Ok()
        | _, Some path ->
            Path.GetDirectoryName(Path.GetFullPath path)
            |> Directory.CreateDirectory
            ...
            | Ok runId, Ok unitId ->
                ...
                | "validate" ->
                    ...
                    | Ok path ->
                        ...
                        | _ ->
                            match
                                LifecycleTelemetry.validateWithEvidence
                                    runId
                                    unitId
                            ...
                            | Ok result ->
                                printfn
                                    "{\"schema\":\"fsgg.telemetry.lifecycle-validation/1\",\"events\":%d,\"completedPhases\":%s,\"activePhases\":%s,\"blockedPhases\":%s}"
                                    result.EventCount
                ...
                | "seal-successor" ->
                    ...
                    | Ok draft ->
                        ...
                        if not (usageErrors @ historyErrors).IsEmpty then
                            fail "telemetry lifecycle" (usageErrors @ historyErrors)
                        else
                            match
        ...
        try
            ...
            | Ok evidencePath ->
                ...
                | Ok closed ->
                    ...
                    | "inspect" ->
                        printfn
                            "{\"schema\":\"fsgg.roadmap-closure/1\",\"verdict\":\"accepted\",\"unitId\":%s,\"externalObligations\":%d}"
                        ...
                        | Ok path, Ok digest ->
                            if action = "render" then
                                ...
                                | Ok sourcePath ->
                                    ...
                                    | Error reasons -> fail "roadmap close" reasons
                                    | Ok() ->
                                        printfn "FSGG-ROADMAP-VERIFIED %s" closed.Evidence.UnitId
                                        green
                        | Error reason, _
