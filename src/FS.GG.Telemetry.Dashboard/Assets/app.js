(() => {
  ...
  function render(data) {
    ...
    data.items.forEach((item)=>{const card=document.createElement("article");card.setAttribute("role","listitem");card.append(element("h3",item.id));card.append(element("p",relationSummary(item),"relation"));card.append(element("p",`${item.state.outcome} · ${item.state.population}`,"state"));card.append(element("p",`${tokenSummary(item)} · ${count(item.runtime.terminal)}/${count(item.runtime.admitted)} terminal`,"metric"));card.append(element("p",`coverage ${item.coverage.populationCoverage} · CI ${item.coverage.ciInventory}`,"coverage"));target.append(card);});
