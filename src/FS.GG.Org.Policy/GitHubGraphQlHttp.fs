...
module GitHubGraphQlHttp =
    ...
    type Reader private (token: string, handler: HttpMessageHandler) =
        ...
        let execute (request: GitHubCommitMembership.ExactRequest) =
            if not (GitHubCommitMembership.isExactReadRequest request) then
                Error ()
            else
                try
                    let payload =
                        JsonSerializer.Serialize(
                            {| query = request.Document
                               variables = {| owner = request.Owner; name = request.Name; commit = request.CommitId |} |})
        ...
        interface GitHubCommitMembership.IReadOnlyGraphQlReader with
            member _.ExecuteExact request = execute request
