...
module HandlerRegistration =
    ...
    let commands =
        ...
        |> List.choose (fun descriptor ->
            if descriptor.HandlerOwner = Options.BoardOps then
                Some descriptor.Command
            else
                None)

    let validate<'handler> (allCommands: Options.Command list) (registrations: (Options.Command * 'handler) list) =
        let grouped = registrations |> List.groupBy fst

        let duplicates =
            grouped
            |> List.choose (fun (command, entries) -> if entries.Length = 1 then None else Some command)
        ...
        let errors =
            [
                if not duplicates.IsEmpty then
                    yield $"duplicate handlers: %A{duplicates}"
                if not missing.IsEmpty then
                    yield $"missing handlers: %A{missing}"
                if not unexpected.IsEmpty then
                    yield $"unexpected handlers: %A{unexpected}"
            ]

        if errors.IsEmpty then
            Ok(Map.ofList registrations)
        else
            Error errors
