...
module RefParsing =
    let parse (owner: string) (defaultRepo: string option) (raw: string) : Result<Ref, string> =
        let forms =
            "use a URL, owner/repo#n, repo#n, or a bare n inside the repo's checkout"

        let numbered ownr repo (digits: string) =
            match
                Int32.TryParse(digits, Globalization.NumberStyles.None, Globalization.CultureInfo.InvariantCulture)
            with
            | true, n ->
                Ok
                    {
