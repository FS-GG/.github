...
O2-I4 and O2 are complete. The standalone roadmap is now **11/12** complete, with O3 controlled adoption
as its sole remaining milestone. No workspace or lifecycle default changed.
...
| **E0–E1 / F0–F5 — Execution and cooperation** | **Selected single-host foundation active; wider scope conditional** | The [standalone O0–O3 roadmap](roadmaps/2026-09-09-190726-standalone-telemetry-host-and-orchestration.md) owns the selected Akka.NET actor, bounded execution and durable Main-host work. Source, immutable Host/runner publication, installed operation, one physical-reboot recovery and native delivery are accepted through O2. This is partial E1 foundation delivery, not completion of E0's comparative measurements or F0–F5 federation. | Reuse the selected contracts and qualified implementation; do not schedule a second actor/executor build or repeat runtime selection for the same scope. O3 controlled adoption, additional planners, federation and broader defaults retain sections 8–9's conditions. O2/O3 does not complete or gate V0/V1. |
| **Selected standalone O2 — Provider-session execution** | **Complete — bounded Main pilot and reboot recovery accepted** | The provider-neutral core and immutable bundle culminated in one generation-bound Codex subscription attempt. It survived one physical reboot, recovered without another model call, and natively merged [`.github` #3515](https://github.com/FS-GG/.github/pull/3515) as `cf840ff9f760d2fdebcdca84caf0ae24f75b8dfa`; its issue, claim and branch are closed and all units are stopped. | [O3 controlled adoption](roadmaps/2026-09-09-190726-standalone-telemetry-host-and-orchestration.md#o3--controlled-adoption-and-later-options) is the remaining standalone milestone. Claude, OpenCode and DeepSeek remain intended adapters. Stronger credential isolation, wider scope and defaults remain separately conditional. |
