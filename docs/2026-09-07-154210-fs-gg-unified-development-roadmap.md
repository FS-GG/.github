...
Last reconciled: **2026-09-09 12:24 UTC**. Current frontier: **V0 and V1 in parallel**.
...
| **Public dashboard source** | **Source delivered and host-adopted** | Bounded Actions/merged-delivery views and privacy filtering exist. [#3381](https://github.com/FS-GG/.github/pull/3381) delivered the schema-8 process-detail display; UTEL-DASH-08/09/10 add one engine-owned coherent `/2` snapshot, an engine-only adapter with a separate public revision, complete-token verdicts only for a fully joined gap-free native root/child/follow-up population under one compatible accounting basis, and one safe preview/install/verify entry point. The approved host now resolves the exact merged engine. | Complete UTEL-DASH-04's recurrence and browser-verification boundaries under their separate operational authority. |
| **Public dashboard operation** | **One-shot feed verified; recurrence unavailable** | An exact empty alias allowlist was approved by digest. Exact-head activation published [commit `b92610f…`](https://github.com/FS-GG/.github/commit/b92610fd6df47708573384378807e5d66adfc690), then verified immutable file bytes, public payload revision and the current `telemetry-data` branch ref. Marker-owned inert user units are installed without embedded credentials. | This host exposes no user systemd bus, so recurrence is not enabled or claimed. Run the same digest-bound activation on a suitable user-systemd host, verify a recurring refresh, and browser-read the deployed page before closing UTEL-DASH-04. |
