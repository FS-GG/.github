...
Last reconciled: **2026-09-09 11:27 UTC**. Current frontier: **V0 and V1 in parallel**.
...
| **Public dashboard source** | **Source delivered, host feed incomplete** | Bounded Actions/merged-delivery views and privacy filtering exist. [#3381](https://github.com/FS-GG/.github/pull/3381) delivered the schema-8 process-detail display; UTEL-DASH-08/09/10 now add one engine-owned coherent `/2` snapshot, an engine-only adapter with a separate public revision, complete-token verdicts only for a fully joined gap-free native root/child/follow-up population under one compatible accounting basis, and one safe preview/install/verify entry point. | Adopt the source on the approved host and complete UTEL-DASH-04 under the separate content and operational authorities. |
