...
Last reconciled: **2026-09-09 12:45 UTC**. Current frontier: **V0 and V1 in parallel**.
...
|---|---|---|---|
| **V0 — Simplified baseline and v2 bindings** | **In progress** | Routine delivery is the canonical default for eligible work; protected operations retain explicit authority. The private telemetry engine/store, prospective runtime and CI observation, canonical budget reducer, post-completion activity/review model, coherent dashboard snapshot source, runtime-coverage repair, safe publisher setup and explicitly authorized event-refresh source are delivered. The approved host uses the exact merged engine and empty privacy-conservative alias allowlist; one public feed commit and the page are verified. | Finish the R2/R4/R5 evidence denominators; adopt the coverage repair, activate the private event receipt, observe an event-triggered verified publication and browser-read the refreshed page. Close remaining automatic parent/child, CI-selection and unsupported native-usage coverage gaps without making telemetry block delivery. |
...
| **Public dashboard source** | **Source delivered; host adoption pending** | Bounded Actions/merged-delivery views and privacy filtering exist. [#3381](https://github.com/FS-GG/.github/pull/3381) delivered the schema-8 process-detail display; UTEL-DASH-08–13 add one engine-owned coherent `/2` snapshot, an engine-only adapter with a separate public revision, strict mixed-runtime coverage and one-basis total proof, one safe preview/install/verify entry point, and an explicitly activated advisory event-refresh path with semantic no-op suppression. The approved host resolves the exact merged engine; its currently verified feed predates the coverage and event-refresh source. | Adopt the merged coverage adapter, activate the private event receipt, observe one event-triggered verified refresh and browser-read the refreshed page under UTEL-DASH-04's separate operational authority. |
| **Public dashboard operation** | **One-shot feed and page verified; event activation pending** | An exact empty alias allowlist was approved by digest. Exact-head activation published [commit `b92610f…`](https://github.com/FS-GG/.github/commit/b92610fd6df47708573384378807e5d66adfc690), then verified immutable file bytes, public payload revision and the current `telemetry-data` branch ref. The deployed page was browser-verified against that immutable feed on desktop and mobile. It reports 24 admitted invocations, 7 with observed usage and 17 missing usage, plus zero eligible completed items and one dirty item; those aggregates do not establish private native outcomes. Marker-owned inert user units are installed without embedded credentials, and the source can request an advisory refresh without a daemon after explicit receipt activation. | This host exposes no user systemd bus, and no event receipt or event-triggered publication is yet activated or claimed. Explicitly activate the digest-bound event receipt, observe a verified event refresh, and browser-read the refreshed page before closing UTEL-DASH-04. |
...
2. **UTEL-DASH operational completion.** The coherent source, safe publisher setup and event-refresh windows are
   merged, exact-host adoption is complete and one public feed commit is immutably verified. Explicitly activate
   the private event receipt, observe an event-triggered verified publication, then browser-read the deployed page
   to close UTEL-DASH-04. Do not introduce an unowned background-loop substitute or claim activation from source.
...
|---|---|---|
| [SVG game engine and Fable template](2026-09-07-064259-svg-game-engine-template-design-roadmap.md), with [revision rationale](2026-09-07-121207-svg-game-engine-roadmap-revision-proposal.md) | Product-owned incremental previews, modeled game/protocol semantics where relevant, published packages and generated-workspace journeys | Preserve Game/Rendering/Net/Audio/Template/S.I.R. ownership, capability scope and default-player distinction; preview work is not automatically a cutover blocker; later defaults need their own decision |
