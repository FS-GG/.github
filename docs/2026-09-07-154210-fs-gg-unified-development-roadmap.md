...
Last reconciled: **2026-09-11 16:38 UTC**. Current frontier: **V0 and V1 in parallel; the accepted SVG
workspace programme completed SVG-FOUND-01 and selects SVG-QUAL-01 installed model qualification next**.
...
| **SVG-QUAL-01 — Installed model qualification** | **Accepted; .1 ready** | The SVG-FOUND-01 audit reproduced installed profile-2 empty-cache refusal and identified the exact accepted Quint/lmt identities. The complete engine programme is accepted for execution with S.I.R. strictly read-only. | SDD first qualifies a supported exact-tool provisioning route; Rendering correspondence and Templates clean/retained receiver qualification follow. Release A remains pending. |
...
3. **SVG-QUAL-01.1 installed profile-2 provisioning.** Reproduce the exact empty-cache refusal in SDD, try the
   existing supported provisioning route, and implement only a demonstrated producer-owned gap. Qualify exact
   installed tool/cache identities plus offline author/inspect and missing, modified and wrong-profile refusal.
   S.I.R. remains strictly read-only.

The independent SVG-FOUND-01 ready window completed at its reviewable local candidate boundary. A later
authorized release window may publish and qualify the exact producer/template set; no publication, registry
activation or default change is implied by SVG-QUAL-01.
