...
Last reconciled: **2026-09-11 17:26 UTC**. Current frontier: **V0 and V1 in parallel; the accepted SVG
workspace programme completed SVG-QUAL-01.1 and selects SVG-QUAL-01.2 reducer correspondence next**.
...
| **Selected standalone O2 — Provider-session execution** | **O2-S1 and Codex adapter source delivered; integration/pilot pending** | [Coordination PR #368](https://github.com/FS-GG/FS.GG.Coordination/pull/368), merge `f9cadece4305bfbad583cd0d996bc1fcaf250808`, delivers the provider-neutral session core and thin Akka supervisor with 15 focused execution tests. [Coordination PR #369](https://github.com/FS-GG/FS.GG.Coordination/pull/369), merge `86559e971122dcb02c4c122dbddf0454d7af1774`, adds the Codex subscription adapter with 12 executable fixtures and green required qualification; broader optimistic formal coverage was still pending at merge. The canonical Quint job now assembles parallel semantic and performance evidence; required qualification and native merge were confirmed. Broader optimistic validation subsequently passed in [run 34621712027](https://github.com/FS-GG/FS.GG.Coordination/actions/runs/34621712027). The operator contract now matches the accepted subscription trust/accounting decision; SystemAdmin reports its source-only validation repair [#52](https://github.com/EHotwagner/SystemAdmin/pull/52) merged and green, not installed. | [O2-S2](roadmaps/2026-09-09-190726-standalone-telemetry-host-and-orchestration.md#o2-source-window-after-the-provider-session-correction) integrates the existing Codex subscription CLI session; Claude, OpenCode and DeepSeek remain intended adapter implementations. Installed adoption, representative pilot delivery and recovery, and O3 controlled adoption remain due. Missing dispatch telemetry is explicit. |
...
| **SVG-QUAL-01 — Installed model qualification** | **.1 complete; .2 ready** | SDD source [PR #981](https://github.com/FS-GG/FS.GG.SDD/pull/981) and release [PR #982](https://github.com/FS-GG/FS.GG.SDD/pull/982) delivered and published the additive coherent SDD 1.7.0 toolchain. Exact [candidate run 34623117701](https://github.com/FS-GG/FS.GG.SDD/actions/runs/34623117701) and [dual-feed readback 34623411200](https://github.com/FS-GG/FS.GG.SDD/actions/runs/34623411200) passed; public-package Q2 passed 13/13 and Q3 passed 22/22 with exact Quint 0.32.0, lmt and Fable 5.13.0 identities. S.I.R. remained strictly read-only. | Rendering now binds the canonical retained reducer model to real .NET and Fable reducers; Templates clean/retained receiver qualification follows. Rendering/Templates publication and Release A remain pending. |
...
3. **SVG-QUAL-01.2 retained reducer correspondence.** Bind Rendering's canonical literate profile-2 model to
   its real .NET and Fable reducers using the installed SDD 1.7.0 toolchain qualified by .1. Cover stale
   replacement/selection, focus, camera and pointer capture/release with bounded witnesses, shared runtime
   traces and targeted failing mutants. S.I.R. remains strictly read-only.
...
| **SVG game engine and Fable workspace completion** | Section 15 independent producer/product track: complete C01–C20, M0–M11, section 13 and Releases A–D through the accepted ordered feature sequence; no V0–V6 completion prerequisite for independent source/qualification work | `.github` planning owner with SDD, Rendering, Game, Audio, Net and Templates implementation owners. S.I.R. is strictly read-only and supplies only an audited disclosed compatibility baseline. SVG-FOUND-01 and SVG-QUAL-01.1 are complete; SVG-QUAL-01.2 is next. Protected publication, installed qualification and later defaults keep their own authority | [accepted complete programme](2026-09-07-064259-svg-game-engine-template-design-roadmap.md), [SVG-FOUND-01 foundation](roadmaps/svg-game-engine-foundation.md), [SVG-QUAL-01 installed model qualification](roadmaps/svg-game-engine-installed-model-qualification.md) and [revision rationale](2026-09-07-121207-svg-game-engine-roadmap-revision-proposal.md) |
...
| **SVG installed model qualification — SVG-QUAL-01** | Explicit typed-SDD/profile-2 SVG fixtures can receive exact-tool provisioning and model/reducer correspondence; existing provider/lifecycle defaults remain unchanged | .1–.2 change SDD/Rendering producer qualification only. .3 first changes candidate clean and retained Templates fixtures. Installed public behavior begins only after compatible SDD, Rendering and Templates publication plus actual receiver consumption; Release A and any default activation remain later boundaries |
