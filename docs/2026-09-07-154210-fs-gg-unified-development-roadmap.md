...
Last reconciled: **2026-09-09 12:32 UTC**. Current frontier: **V0 and V1 in parallel**.
...
| **Public dashboard source** | **Source delivered; coverage repair pending host adoption** | Bounded Actions/merged-delivery views and privacy filtering exist. [#3381](https://github.com/FS-GG/.github/pull/3381) delivered the schema-8 process-detail display; UTEL-DASH-08/09/10 add one engine-owned coherent `/2` snapshot, an engine-only adapter with a separate public revision and one safe preview/install/verify entry point. UTEL-DASH-11 retains every expected runtime type in item coverage, distinguishes complete, partial and unknown native usage, and preserves the strict gap-free one-basis proof for a complete token total. The approved host resolves the exact merged engine; its currently verified feed predates this coverage-adapter repair. | Adopt the merged coverage adapter on the approved host, populate explicitly approved completed-item aliases when available, and complete UTEL-DASH-04's recurring publication boundary under its separate operational authority. |
| **Public dashboard operation** | **One-shot feed and page verified; recurrence unavailable** | An exact empty alias allowlist was approved by digest. Exact-head activation published [commit `b92610f…`](https://github.com/FS-GG/.github/commit/b92610fd6df47708573384378807e5d66adfc690), then verified immutable file bytes, public payload revision and the current `telemetry-data` branch ref. The deployed page was browser-verified against that immutable feed on desktop and mobile. It reports 24 admitted invocations, 7 with observed usage and 17 missing usage, plus zero eligible completed items and one dirty item; those public aggregates do not establish the presence or absence of private native outcomes. Marker-owned inert user units are installed without embedded credentials. | This host exposes no user systemd bus, so recurrence is not enabled or claimed. Run the same digest-bound activation on a suitable user-systemd host and verify a recurring refresh before closing UTEL-DASH-04. |
...
2. **UTEL-DASH operational completion.** The coherent source window and safe publisher setup are merged, exact-host
   adoption is complete and one public feed commit is immutably verified. Move the installed inert units to a
   suitable user-systemd host, verify recurring publication, then browser-read the deployed page to close
   UTEL-DASH-04. Do not introduce an unowned background-loop substitute.
