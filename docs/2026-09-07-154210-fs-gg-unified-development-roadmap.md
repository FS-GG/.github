...
| **Selected standalone O2 — Provider-session execution** | **S1/S2 source and executable Main composition delivered; publication/pilot pending** | [Coordination #368](https://github.com/FS-GG/FS.GG.Coordination/pull/368), [#369](https://github.com/FS-GG/FS.GG.Coordination/pull/369), [#370](https://github.com/FS-GG/FS.GG.Coordination/pull/370) and [#371](https://github.com/FS-GG/FS.GG.Coordination/pull/371) deliver the neutral core, Codex subscription adapter, durable transport/admission and runnable executor/candidate pipeline; candidate validation and S2b postmerge runs passed. [#372](https://github.com/FS-GG/FS.GG.Coordination/pull/372), merge `59e7521d324fe2c7af2aa10a77c2861238f7bedd`, delivers Main's production actor graph, authenticated admission/relay and seven-effect driver. Exact-head bootstrap/coherent validation and native merge readback passed. A real PostgreSQL + Release packaged-runner journey recovers a fresh actor graph after claim settlement and completes all seven effects against deterministic GitHub transport. This is source qualification, not installed/live acceptance. | [The existing O2 window](roadmaps/2026-09-09-190726-standalone-telemetry-host-and-orchestration.md#o2-source-window-after-the-provider-session-correction) continues with immutable Host/runner publication and the SystemAdmin launcher, installed adoption, live pilot/recovery and O3. Claude, OpenCode and DeepSeek remain intended adapters. These are the selected E1 foundations, not a future duplicate actor project. Prospective telemetry works; earlier gaps and unsupported native usage remain explicit. |
...
Reconciled SVG feature-milestone states: **16 completed, 1 current frontier and 3 pending**. SVG-PREVIEW-A.2
is the next boundary; it is a protected publication operation and is not authorized by .1's source merge.
...
| **SVG-PREVIEW-A.2** | **Current frontier** | Defined in the [Preview-A subroadmap](roadmaps/svg-preview-a.md) | Obtain protected-operation authority, publish the retained coherent Rendering 0.29.0 archives to both feeds and verify payload identity. |
...
3. **Continue SVG-PREVIEW-A at .2.** The accepted [Preview-A subroadmap](roadmaps/svg-preview-a.md) has
   completed recoverable source preparation. Obtain independent protected-operation authority, publish the
