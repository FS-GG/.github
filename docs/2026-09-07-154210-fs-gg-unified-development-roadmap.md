...
| **Public dashboard source** | **Source delivered, host feed incomplete** | Bounded Actions/merged-delivery views, privacy filtering and an optional host publisher exist. [#3381](https://github.com/FS-GG/.github/pull/3381) adds the schema-8 item-detail consumer for activity spans, attribution, typed complications and process-review metadata. | Host publication remains pending. A complete coherent engine snapshot, removal of the remaining bounded Python SQLite reads and one safe publisher preview/install/verify command remain future work. |
