...
SVG-PREVIEW-B is selected next**.
...
Reconciled SVG feature-milestone states: **42 completed; SVG-PREVIEW-B selected next**.
Rendering 0.29.0 and Templates 0.11.0 remain the current public set. Authoring, input, runtime and
presentation are qualified as exact local generated candidates without changing provider, lifecycle or general
template defaults; Preview B owns their publication and installed public proof.
...
| **SVG Preview A — SVG-PREVIEW-A** | The public release remains opt-in: all 17 `FS.GG.UI.*` libraries plus BOM and `FS.GG.UI.Template` at Rendering 0.29.0, then `FS.GG.Workspace.Template` 0.11.0. The wizard remains 0.11.1 and uses its two-step `fable-game` then SVG-adopter route; `svg: false` and lifecycle `sdd` remain omitted | Release A is complete. Fresh direct, SDD, typed and wizard receivers consume the public packages; existing projects change only through the bounded adopter from public Templates 0.10.0. No provider activation or default changed. SVG-PREVIEW-B is selected after completed authoring/input/runtime/presentation candidates |
| **SVG Preview B — SVG-PREVIEW-B** | The opt-in public path will carry Rendering 0.30.0, Game 0.15.0, Audio 0.6.0 and Templates 0.12.0 with authoring, local-play and presentation composition | The selected [Preview-B plan](roadmaps/svg-preview-b.md) first prepares and reads back producer releases, then adopts exact public pins and qualifies clean, SDD, typed, wizard and retained receivers. Source candidates alone do not change installed workspaces or defaults |
