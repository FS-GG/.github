...
| GS2-10.8; GS2-12.10; GS2-14.4 | One accountable owner records architecture, security, operations, migration and receiver findings in fresh critique passes, using separate black-box controls and provider readback. | Exact candidate/manifest acceptance and each required technical gate. If a verifier requires a native identity, satisfy its real contract; role naming or a subagent cannot manufacture that identity. |
