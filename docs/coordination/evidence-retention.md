...
byte count, SHA-256, immutable run/artifact URL, and expiry. The manifest also binds the exact source SHA and
...
or timezone-free timestamp also fails closed. Consumers download the named artifact and compare its SHA-256;
...
the same bytes. CI policy subject discovery lives in `policy/subjects.json` and is executed by one runner and
one workflow. Adding a subject changes the inventory instead of adding another entry-point workflow.
