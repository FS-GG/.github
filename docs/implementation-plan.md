...
categoryindex: 6
index: 8
description: Coordination page for the separate FS.GG rendering and governance implementation plans.
---

# FS.GG implementation plans

The split is implemented through separate plans. The rendering plan comes first
and starts from a fresh standard Spec Kit repository. The governance plan comes
later and starts from its own fresh standard Spec Kit repository. This repository
is source inventory, provenance, and archive material; it is not the base that is
gradually transformed into either destination.
...
3. Implement the [governance repository plan](governance-implementation-plan.md).
4. Add lightweight cross-repo contracts only after both repositories exist.
5. Decide any rebrand as a separate package/template/docs release decision.
...
- keep the governance repository free to experiment without affecting product
  work.
...
- this repository is documented as bridge/archive and no longer receives normal
  product features;
- the governance repository exists independently and can evolve as optional
  tooling;
