...
## 2026-08-27 protected post-state and independent approval action

**Observed:** 2026-08-27T10:08:36+02:00

The organization configuration action below is complete. Configuration 17 is attached to repository id
`1346720714`; its association is `attached`; CodeQL default setup is configured for Actions; and private
vulnerability reporting is enabled. The organization's current entitlement leaves the repository's effective
validity-check and non-provider-pattern projections disabled even though configuration 17 requests them. They
are recorded as license-unsupported, not operational. Configuration 17's distinct generic-secrets value remains
`not_set`.

The reviewed repository rulesets are also active:
...
This intentionally exposes one final administrator action. Team `coordination-maintainers` currently has only
`EHotwagner`, the author of the protected receipt PR, so no different CODEOWNER can approve it. Add existing FS-GG
member and repository Admin `nuklearwanze` at
...
[FS-GG/FS.GG.Coordination#23](https://github.com/FS-GG/FS.GG.Coordination/pull/23). Do not weaken the ruleset,
add a bypass, dismiss the required checks, or use the author account for this approval. The approval must target
the current PR head after the independent receipt critic passes.
