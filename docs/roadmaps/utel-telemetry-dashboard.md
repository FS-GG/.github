...
- [x] **UTEL-DASH-08 — Consume engine-owned detail safely.** On schema 8, the host adapter calls the configured engine's versioned `item-detail` command for selected mapped member items after closing its bounded SQLite transaction. It namespaces private member joins, publishes only closed activity categories, counters, typed complication metadata and safe review metadata, and preserves independent activity, attribution, complication and review truncation. Schema 7 remains compatible through the isolated unchanged-table reader.
- [x] **UTEL-DASH-09 — Present activity, complications and process reviews.** The completed-item drilldown shows overlapping activity spans, direct/mixed/unclassified token attribution, missing-attribution coverage, typed complication triggers and causes, and review scope, revision, confidence, declared coverage, mapped reviewer categories and private-list counts. It keeps review prose private and distinguishes attempt review, item review, activity time, runtime time, review duration and CI time.
...
The schema-7/8 SQLite reader remains isolated inside the source-local host adapter for canonical completion, native outcome, dirty-state, invocation, scoped usage and CI facts that the item-detail export does not supply. It verifies the engine-owned status first, opens the canonical database with `mode=ro`, requires WAL, bounds rows and VM work, and never initializes, drains, migrates or writes. On schema 8, the adapter closes that transaction before obtaining independently observed activity, attribution, complication and review detail from the configured engine. These reads are not presented as one atomic snapshot.

<a id="producer-handoff-missing-activity-and-repair-evidence"></a>

### Producer detail contract and remaining handoff

The engine now provides `fsgg.telemetry.item-detail/1`, capped independently at 256 activities, 256 usage attributions, 256 complications and 128 reviews. The private result also contains identifiers, summaries, review prose, evidence and digests, so the host adapter validates it and selects only the public fields named above. Accounting totals cover all attribution rows even when returned detail is truncated; `missingAttribution` is a usage-row count. Native total zero does not establish zero cost or complete capture. The export has no provider/model/accounting-scope columns for attribution, no original-item completion decision and no overall snapshot revision.

A later coherent engine export could remove the remaining unchanged-table reads by supplying completion, invocation spans, scoped native usage and a snapshot revision together. Activity stage stays orthogonal to bureaucracy classification: repair or review does not automatically mean administrative overhead, useful validation remains excluded, and no model turn inherits a whole activity or cost category from its parent role. This dashboard change adds no schema migration or duplicate completion reducer.
