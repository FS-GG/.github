...
- [x] **UTEL-DASH-04 — Deployed dashboard and host feed activation.** The operator installed the immutable publisher package and native dependencies on a persistent host mount, with no container-name dependency or credential relocation, then enabled the private timer and event receipt. The timer independently completed a one-shot publication, and a real root attempt's terminal drain automatically published [event commit `2edf246…`](https://github.com/FS-GG/.github/commit/2edf24678840c47a7a8a1306cf9947755007d07d); a separate explicit refresh was a semantic no-op. Independent readback validated the 5,491-byte host/3 payload and revision `851ab857…` at the current branch ref. [Pages run 34355973095](https://github.com/FS-GG/.github/actions/runs/34355973095) deployed source `209410f…`, and desktop/mobile browsers loaded exactly that immutable host object without errors or overflow. The allowlist remains empty; the canonical projection separately reports zero eligible and published completed items plus one dirty item, and native collaboration token capture remains unsupported rather than zero.
