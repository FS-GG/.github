...
- [x] **LEARN-01.1 — Freeze the baseline and one reviewable experiment contract — route: routine.**
  ...
  Evidence: [`policy/learn-01-current-focused-v1.json`](../../policy/learn-01-current-focused-v1.json), the [review contract](../research/learn-01-current-focused-contract.md) and `tests/learn-01-analysis` freeze the source-only comparison and exercise its corpus. Native usage is currently `not-configured`; enrollment remains ineligible until the existing private join covers every expected invocation. Publication, installation, activation, observation, .2–.5 and feature exit remain pending.
