...
- [ ] H3 — Main activation and container durability proof. [Partial Main evidence](../reports/evidence/2026-09-10-standalone-telemetry-h3.md) records restored-receipt verification and credential rotation; the milestone remains open. Main installation is the user's manual step, with direct private-LAN HTTPS connectivity as the first target. Follow the [SystemAdmin Main quickstart](https://github.com/EHotwagner/SystemAdmin/blob/main/Services/telemetry-host-podman/MAIN-QUICKSTART.md) with the immutable Containerfile and operator wrapper. First verify scoped ingestion and dashboard access from the development container; then complete the restart, recreation and recovery evidence below. Preparing this handoff does not establish Main readiness, producer connectivity or durability; record those results after the manual run.
