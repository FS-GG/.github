...
- [x] H2 — Immutable host packaging and deployment rehearsal: [release and deployment evidence](../reports/evidence/2026-09-10-standalone-telemetry-h2.md). Host 0.1.1 is immutable and verified on both feeds; the accepted SystemAdmin consumer ran the real rootless Podman installation, authentication, crash/restart, backup/restore and refusal journey on isolated eligible storage. Main activation was later accepted under H3; cross-version rollback and physical power-loss recovery remain unqualified limitations rather than claims of this roadmap.
- [x] H3 — Main activation and container durability proof. [Accepted Main evidence](../reports/evidence/2026-09-10-standalone-telemetry-h3.md) records scoped ingestion and browser access, restart/restore, credential rotation, visible missing ingestion, duplicate-safe fact replay, updater activation, and the stopped-development-container independence proof. Main kept the exact applied reference receipt and unchanged private-history revision/counts while the only running development container was stopped, then the same container recovered healthy without duplication. Main OS reboot, physical power-loss recovery and measured RPO/RTO remain explicitly unclaimed.
...
- [x] O2-S2 — Codex subscription CLI adapter and runner integration — route: routine.
  ...
  This records the old topology's completed source evidence only. The September 14 topology decision
  supersedes it for installation: the Host and runner now ship in one orchestration-container bundle,
  and the Host supervises the runner as a local child. The old host relay and `podman exec` launcher
  are not prerequisites for the remaining live pilot.

The next installed window adopts the combined immutable application bundle into the selected two-container
deployment, qualifies actual authentication/egress and execution behavior, and produces a fresh deployment preview.
...
**Use the release-specific instructions for executable commands.** The [SystemAdmin Main quickstart](https://github.com/EHotwagner/SystemAdmin/blob/main/Services/telemetry-host-podman/MAIN-QUICKSTART.md) and its linked operator reference cover the released telemetry container. The user performs Main installation and activation manually. The outline below explains the capability boundaries; orchestration operation remains pending. Follow only steps for the selected profile and retain the distinction between isolated qualification and observed Main behavior.
...
The telemetry container runs one verified release. Its service does not update its own binaries from NuGet, and automatic package upgrades remain outside this implementation scope. The Main quickstart documents explicit image replacement. A future separately selected updater could discover new stable NuGet releases, verify their accepted manifests, build immutable images, and perform backup, compatibility checks and readiness verification before replacement. Podman's image-update mechanism alone does not implement those package and state checks.
