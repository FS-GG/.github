...
- **Why:** The game-logic primitives shipped recently into Rendering (`FS.GG.UI.Canvas.SpatialGrid`,
  `FS.GG.UI.Canvas.Pathfinding`, `FS.GG.UI.Scene.Geometry`, and the #132 collision / grids /
...
- **Home:** all five live in `FS.GG.Rendering` under `FS.GG.UI.Canvas.*` / `FS.GG.UI.Scene.*`,
