...
- **Scope:** The **2D collision detection & resolution** game-logic skill — what
  `FS.GG.UI.Scene.Geometry` (today: an AABB type) and the `collision` fragment should grow
...
type Shape =
    | Aabb    of min: Vec2 * max: Vec2                   // grows FS.GG.UI.Scene.Geometry
...
- **Home:** `FS.GG.Rendering`, growing `FS.GG.UI.Scene.Geometry` into a
