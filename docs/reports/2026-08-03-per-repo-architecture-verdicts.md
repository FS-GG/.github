...
```json
{
  ...
  "verdict": "clean | findings | no-verdict",
...
The leg returns exactly the shared outcomes: exit `0` only after every declared surface was read and no architecture finding remains; exit `1` for a readable, actionable architecture finding; and exit **`3`** for a permanent no-verdict when the review cannot establish its subject. A malformed corpus, missing review revision, absent required surface, unparseable architecture declaration, or a missing built-artifact/journey where the repository claims reachable functionality are all exit-3 conditions. A transport/rate-limit failure remains the shared retryable no-verdict path.
...
1. A complete repository corpus with all declared surfaces observed and a passing built-route comparison returns `0`. This is the contrasting clean control, not an empty corpus.
2. A corpus whose runtime entry point has no reachable test/player journey returns `3`, even if source files and unit tests are present. This prevents a source-only review from certifying an unreachable feature.
3. A corpus with an observed contradiction — for example a delivery declaration naming a package or workflow no runtime/release surface owns — returns `1` with the stable subject and both evidence locators.
4. A malformed or incomplete surface declaration returns `3`, never `0` and never a fabricated finding.
...
4. Emit the per-repository record and its shared exit. A finding is routed to the repository owning its established root cause; a distinct unestablished cause remains no-verdict rather than being guessed into a board item.
