...
> **Separable clause, and a reviewer may reject it without disturbing the rest of this section.** Beyond adding the new function, `winner` *could* be re-expressed in terms of it, so that "lowest id wins" has provably one implementation rather than two that currently happen to agree. The argument for: that is what `#485` actually asks for, and the two are today related only by inspection. The argument against: it enlarges a slice that was otherwise narrow, and edits a safety-critical read path for a property no observed defect has yet violated — which is ADR-0041's Option A objection in a different file. **The new function and the `Paths:` widening do not depend on this clause**; if it is rejected, slice 2 adds the function beside `winner` and leaves `winner` alone.
