...
**Is this a second CAS?** No, and the distinction is the one §12.5 rests on. A CAS here means *lock* semantics: lease, release, eviction, twin detection, restore-previous-column. The election has none of them; it is `sortBy id |> tryHead` over a filtered list. **That ordering rule must not be written twice** (`#485`). The engine already *computes* exactly it, without the lease filter — but **not anywhere a caller can reach**, and that distinction is load-bearing rather than pedantic. The expression lives inline in one branch of `reserver`, and the module exports no lease-free ordering function at all.
*Verification:* `src/FS.GG.Coord.GitHub/Reads.fs:394` — `markers |> List.sortBy (fun m -> m.Id) |> List.tryHead`, inside `reserver` (`:391-394`); `winner` at `:375-379` is the same expression with `List.filter (isStale leaseMinutes >> not)` applied. `Reads.fsi` exports `markerScan` (`:87`), `requireCompleteMarkerScan` (`:98`), `winner` (`:116`) and `reserver` (`:126`) — and nothing else.

So the election needs **one new exported ordering function** in `Reads.fs`/`Reads.fsi`, and slice 2's `Paths:` in §11.2 declares those two files for that reason. The alternative — implementing `sortBy id |> tryHead` in the CLI layer — is precisely the second copy this paragraph and §12.5 forbid, so it is not available. Note the boundary: this is the **read** side. §4.1's "the CAS gains no code" is a claim about the *write* path (`claimScoped`), and that remains true and untouched.

> **Separable clause — a reviewer may reject all of this without disturbing the new function or the `Paths:` widening above.**
>
> "Lowest id wins, lease-free" is **not** written once today. It is written **four** times, and **three of them decide locks**:
>
> | site | what it decides |
> |---|---|
> | `src/FS.GG.Coord.GitHub/Reads.fs:394` | `reserver`'s lapsed-claim fallback |
> | `src/FS.GG.Coord.Cli/Client.fs:2352` | `who`'s Held/Stale classification |
> | `src/FS.GG.Coord.Cli/Client.fs:2756` | **`reap`'s** stale-lock candidate — *"reap's one candidate per item"* |
> | `src/FS.GG.Coord.Cli/Client.fs:3461` | **`adopt`'s** expired-claim selection — its own comment calls it *"(`reap`'s rule)"* |
>
> *Verification:* `grep -rn "List.sortBy (fun m -> m.Id) |> List.tryHead" src/` returns exactly these four.
>
> All four hand-roll `reserver`'s semantics, and `reserver`'s own doc comment **names `who`** as making the same choice (`Reads.fs:389-390`) — while `who` does not call it. That is precisely what `winner`'s doc comment says it exists to prevent: *"It is a function, and there is one of it, because #485 is what happens otherwise: 'who holds this?' computed in five places and agreeing in none"* (`Reads.fs:365-368`). The defect has reappeared for the lease-free variant, under `reap` and `adopt` — the highest-stakes lock paths in the engine.
>
> So this clause is not scope bought for the election's benefit; the election merely makes a fifth copy unless something is done. **The argument against remains real**: it edits a safety-critical read path for a property no *observed* defect has yet violated, which is ADR-0041's Option A objection in another file.
>
> **Two constraints if it is adopted, and the first governs what may be claimed:**
>
> 1. **The strong claim is earned only by converting all four.** Re-expressing `winner` alone leaves one exported rule with two callers *plus three unconverted copies* — a real improvement, but not "provably one implementation". Slice 2's paths already include `Client.fs`, so it can convert all three; **either it does, and the strong claim holds, or the claim is scoped to `Reads.fs` and the three `Client.fs` survivors are named.** Asserting the strong version while copies remain is the overstatement class repair rounds 1 and 2 each corrected, and this would be the third.
> 2. **These are not drop-in `reserver` calls, and an implementer must not treat them as such.** The shared thing is the lease-free **ordering**, not `reserver` itself. Each site wraps it in its own arm logic: `who` needs the Held-versus-Stale distinction; `reap` and `adopt` act *only when there is no live winner* (`Client.fs:2756` does nothing on `Some _`). `reserver` returns `winner` when a live winner exists, so mechanically substituting it would hand **`reap` the live winner** — wrong, and dangerous in the one path that breaks locks.
>
> If the clause is rejected, slice 2 adds the new ordering function beside `winner`, leaves `winner` alone, and the four copies above survive — recorded here so a later reader knows they were seen rather than missed.
>
> **Disposition: absorbed into slice 2, deliberately not filed as a separate row.** No open issue carries this cause, and `#485` is CLOSED with a different subject — scheduler startability, not lock ordering.
> *Verification:* `gh issue view 485 --repo FS-GG/.github --json title,state` → *"is this item startable?" is computed in five places and agrees in none*, `CLOSED`.
...
| 2 | **Two mechanisms, one ordering rule.** (a) the per-receiver dispatch lock: a new `opLockRef` table beside `choreLockRef` (**including the missing `FS.GG.Net` row**) plus acquire/release calling `claimScoped` with that ref — the CAS's **write** path gains no code, no prefix, no field, no parameter (§4.1); (b) the merge election: append-only marker, no lease, read through **one new exported lease-free ordering function** in `Reads`, not a second copy of "lowest id wins" in the CLI layer (§4.2). The `Reads.fs`/`Reads.fsi` paths are declared for that function alone; re-expressing `winner` in terms of it is the separately rejectable clause in §4.2 | `src/FS.GG.Coord.Cli/Options.fs src/FS.GG.Coord.Cli/Client.fs src/FS.GG.Coord.GitHub/Reads.fs src/FS.GG.Coord.GitHub/Reads.fsi tests/FS.GG.Coord.Cli.Tests tests/FS.GG.Coord.GitHub.Tests` | hardening | after 1 |
