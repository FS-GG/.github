...
- **Date:** 2026-07-05
- **Owner:** `.github` (cross-repo coordination); **implementation home:** `FS.GG.Rendering`
...
- **Scope:** The **grid coordinate systems + spatial-partitioning** game-logic skill — what
  `FS.GG.UI.Canvas.SpatialGrid` (today: a uniform grid with exact range/radius queries) and
...
- **Home:** `FS.GG.Rendering` as `FS.GG.UI.Canvas.Grid` (coordinate algebra + storage) and an
  extended `FS.GG.UI.Canvas.SpatialGrid` (index/broad-phase); depends *down* only. Never Governance.
