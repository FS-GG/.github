...
**P5 is the live front (#219): publish.** Release `FS.GG.Game.Core` + `.Render` to the org feed + nuget.org (ADR-0012/0013), execute the staged Canvas + Scene SemVer majors in Rendering (the physical removal of the moved surfaces + the frozen fragments' re-home), finalize the per-package `contracts:` rows (`game-sim-core`, `game-scene-adapter`) + the `Game.Render → FS.GG.UI.Scene` edge against live versions, and flip `game-extraction` coherent:true — publish-before-flip (FR-007) throughout.
