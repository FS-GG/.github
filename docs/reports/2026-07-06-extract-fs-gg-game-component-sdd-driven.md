...
3. ~~**Design-corpus location**~~ — **CLOSED (2026-07-06, post-P5): relocated.** The game-skill design reports moved `.github/docs/reports/` → **`FS.GG.Game/docs/reports/`** — the 6 game-logic designs ([FS.GG.Game#8](https://github.com/FS-GG/FS.GG.Game/pull/8), consolidated into `docs/reports` by [FS.GG.Game#10](https://github.com/FS-GG/FS.GG.Game/pull/10)) + the `game-audio` library design (its `fs-gg-audio` skill is game-owned; the `Audio` surface stays in `FS.GG.UI.Canvas`), each with a provenance banner. The game TestSpecs stay in `.github` (cross-repo tests reference them there); the StarCraft2 unit-symbology design stays too (`fs-gg-symbology` is a Rendering skill).
4. ~~**Governance dogfood**~~ — **CLOSED (2026-07-06, post-P5): SDD-only in P3, governance adopted as the fast-follow.** `FS.GG.Game` dropped in the light `.fsgg/` governance overlay (the four slots — `governance`/`capabilities`/`policy`/`tooling`, `light` profile, adapted to the `FS.GG.Game.slnx` / stock-dotnet shape) + a **non-blocking** `governance.yml` CI job (`fsgg-governance route --mode gate`, `continue-on-error`). Advisory only — governance inspects, never gates the merge (`gate.yml` stays the merge gate). FS.GG.Game is the first FS-GG framework repo to dogfood governance on its own code ([FS.GG.Game#9](https://github.com/FS-GG/FS.GG.Game/pull/9)).
