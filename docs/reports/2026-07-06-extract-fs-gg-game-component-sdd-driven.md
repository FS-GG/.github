...
- **Progress (2026-07-06):** **P0 ✅ · P1 ✅ · P2 ✅ · P3 ✅ · P4 → next.** Epic **[#213](https://github.com/FS-GG/.github/issues/213)** on the Coordination board; phases P0–P5 = children [#214–#219](https://github.com/FS-GG/.github/issues/213). P0–P3 done. **P3 (#217 closed — board Done):** the provider-less **"dev-repo" provenance shape** is defined, merged, and **released** — SDD **feature 085** ([FS.GG.SDD#158](https://github.com/FS-GG/FS.GG.SDD/pull/158)) has `fsgg-sdd init` write a provider-less `.fsgg/scaffold-provenance.json` (`outcome: devRepoInit`, no template pin; schema stays v1), so `doctor`/`upgrade` engage on a hand-`init`'d repo instead of "nothing to reconcile". Shipped in **`fsgg-sdd` 0.8.0** (org feed + nuget.org). The dogfood itself landed as [FS.GG.Game#2](https://github.com/FS-GG/FS.GG.Game/pull/2): FS.GG.Game `init`'d with 0.8.0 (`doctor` → coherent, provider-less), then a first feature — `Rng.nextBool` — driven **charter→ship** (Game.Core.Tests 75/75). **Next: P4 — adapter + content** (#218).
...
**P4 is the live front (#218): adapter + content.** `FS.GG.Game.Render` (the Scene adapter), move the starter fragments + game skills into `FS.GG.Game.Template`, migrate `skills.yml` ownership, and freeze Rendering's `--profile game` — then P5 executes the staged Canvas+Scene majors and the publish-and-flip.
