...
- **Progress (2026-07-06):** **P0 ✅ · P1 ✅ · P2 ✅ · P3 → next.** Epic **[#213](https://github.com/FS-GG/.github/issues/213)** on the Coordination board; phases P0–P5 = children [#214–#219](https://github.com/FS-GG/.github/issues/213). P0/P1/P2 done (P2 #216 closed — board Done, PR [#222](https://github.com/FS-GG/.github/pull/222) merged). **[FS-GG/FS.GG.Game](https://github.com/FS-GG/FS.GG.Game) is live (public):** `FS.GG.Game.Core` builds/tests headless (71/71 Expecto, zero Skia/Scene), surface baseline committed (11 types), CI green (gate + coordination-coherence), coordination kit + labels distributed, board `game` scope added, registry `game-extraction` coherence row merged (`coherent: false`). **Next: P3 — `fsgg-sdd` dogfood** (#217).
...
| **P3 — Dogfood** ⏳ next | `fsgg-sdd init` the repo; run first feature (`charter→ship`) through `.fsgg/`; surface & close the no-template provenance shape in SDD. | Game, SDD | One feature shipped via SDD; provenance gap has a defined dev-repo shape. |
...
**P0 ✅, P1 ✅, and P2 ✅ are done** (2026-07-06): the `Scene.Geometry` cut line is settled (Option D — touches *two* packages, Canvas + Scene; no `FS.GG.Math` leaf), ADR-0022 is merged, the Coordination epic (#213) is open with all phases sequenced, and **[FS-GG/FS.GG.Game](https://github.com/FS-GG/FS.GG.Game) is stood up and live** — `FS.GG.Game.Core` builds/tests headless with the ~640 LOC clean-move (71/71 Expecto, zero Skia/Scene), CI is green, and the registry `game-extraction` coherence row is merged (#216 closed, PR #222).

**P3 is the live front (#217): the `fsgg-sdd` dogfood.** `fsgg-sdd init` the new repo, run one feature `charter→ship` through `.fsgg/`, and define the provider-less **"dev-repo" provenance shape** — a hand-`init`'d repo has no `fs-gg-ui-template` pin for `doctor`/`scaffold-provenance` to anchor to (Edge #2, §5). The primitive move (P2, done) and this SDD dogfood (P3) are the two milestones that prove the design; the consumer provider (P6) is deliberately a separate future epic.
