...
**Superseded as the default on 2026-09-08:** routine delivery is now the process default for all unified
roadmap work. This ADR's disposable-worker/full-lifecycle route applies only when a human records an
explicit heavyweight-process instruction for named scope. Absence or ambiguity selects routine delivery;
strict labels, GS2 registration, protected paths, policy changes, modeled work, protected operations and
inherited strict state do not invoke this ADR. Existing evidence remains historically authoritative, not a
route selector.

Routine delivery uses one accountable owner, one branch and one PR, focused and native checks, same-PR
repairs, native merge/readback and asynchronous telemetry. It requires no issue/claim, SDD artifact family,
phase lifecycle, mandatory critic, feedback/receipt cycle, metadata-`Done`, or projection PR. Substantive
technical checks, canonical model authority, permissions and release/deploy/credential/destructive/cutover/
external-acceptance safeguards remain independent and fail-closed; an invalid authorization blocks its
affected effect without selecting heavyweight ceremony.
