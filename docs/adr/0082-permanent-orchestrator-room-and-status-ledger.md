...
2. Orchestrator presence is an append-only status ledger with exactly four states: `active`, `waiting`,
   ...
   (or `none`), exact head (or `none`), and one bounded note token. The latest valid record per worker
   wins; records are not edited.
...
unpublished work. Machine-enforced status parsing/CAS may later replace the procedural check without
changing the vocabulary or room identity.
...
and `scripts/check-skill-quality` verifies mirrors, links, triggers, and budgets.
