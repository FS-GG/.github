...
- **Supersedes:** [ADR-0036](0036-the-build-config-drift-check-pins-its-source.md) — for **build config**. ADR-0036's receiver-side `sync-build-config.sh --check` + `.config/fsgg-build-config.sha` pin model is **retired** (#1262 step 3): build config now ships as the FS.GG.Kit package, and each receiver's `build-config-drift` job asserts its committed `.props` match the pinned package (the package materialize's content-addressed verify, ADR-0014, is the integrity guarantee). ADR-0036's de-racing rationale — "behind" is green-with-notice, not a merge-freeze — stands as the history of why the byte-copy check was shaped that way.
- **Interacts with:** [ADR-0058](0058-adopt-one-governing-principle-derive-dont-restate.md) (the governing principle — a byte-copy is a hand-maintained restatement of the hub's bytes in N repos); [ADR-0019](0019-org-repo-roster-registry-and-coordination-kit.md) (the coordination kit + roster this changes the *delivery* of); [ADR-0034](0034-typed-coordination-engine.md) (the `fsgg-coord` client already ships as a `dotnet tool` shim — the precedent this generalizes to the rest of the kit).
