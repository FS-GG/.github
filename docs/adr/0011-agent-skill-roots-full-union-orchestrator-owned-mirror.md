...
- **Amended by:** [ADR-0070](0070-restore-is-a-precondition-no-committed-skill-content-in-a-receiver.md) §1 and §5 — **2026-07-28, and NOT executed: that record decides a contract and retires nothing** ([#1837](https://github.com/FS-GG/.github/issues/1837)). In a `coordination-kit` **receiver**, Decision 1's roots and Decision 2's materialized copies stop being **committed** paths: both are generated from the restored `FS.GG.Kit` package at each runtime's expected path. Decision 1's *union rule* is untouched and, **for kit-derived content**, becomes structural rather than asserted — it all descends from one restored package, so there is no second copy for a first to diverge from; content the **receiver itself** put in a runtime root is outside that and is ADR-0070 §1.1's stated obstacle, not something this amendment authorises deleting. §Context's symlink rejection is untouched and re-affirmed **as written** — it is about a **committed** symlink degrading under `core.symlinks=false`, and a tree that commits nothing has no such link; it is not widened to uncommitted links, since the view root is already a symlink today under [ADR-0065](0065-one-agent-skill-root-contract.md)'s view disposition. **This record otherwise stays IN FORCE**, and ADR-0070 reaches only the receiver lane — the scaffolded-product-workspace half of Decisions 1–4, including `fsgg-sdd` as sole mirror authority for scaffolded trees, is untouched.
...
1. **Full union, every root.** `.claude/skills/`, `.codex/skills/`, and `.agents/skills/`
   ...
   > runtime already reads. **The union rule above is untouched and, for KIT-DERIVED content, becomes
   > structural rather than asserted:** it all descends from one restored package, so there is no
   > second copy for a first to diverge from, and *"each root holds the byte-identical union"* stops
   > being a fact a gate compares copies to establish. **Content the RECEIVER itself put in a runtime
   > root is outside that** — `FS.GG.SDD`'s `skill-manifest.json` and `FS.GG.Audio`'s 16 own skills are
   > the measured instances — and ADR-0070 §1.1 records it as an unresolved obstacle rather than
   > licence to delete it. The enumeration is unchanged; a **scaffolded product workspace** is out of
   > that record's scope entirely and this clause governs there as written.
