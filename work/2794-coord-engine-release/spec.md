...
- SB-004: Release only the four engine commits merged after `coord-engine/v0.67.0`, including `.github#2772` commit `7cef7301`; do not represent any unmerged behavior in notes or evidence.

## Non-Goals
- SB-005: Do not redesign the release saga, change engine behavior beyond the merged-main inventory, or incorporate unmerged source.
...
- AC-001 [US-001] [FR-001]: Given the current merged main after 0.67.0, when freshness and semantic impact are measured, then the next version is justified from the actual four-commit behavior, including merged #2772 and excluding unmerged work.
