...
- CQ-004 [AMB:AMB-003] decision: Store immutable, expiring board-orchestrator lease generations and generation-bound request receipts on the checked-in authority issue `FS-GG/.github#2801`. Production routes reject alternative caller refs before lease IO. A live lease forces routing; absence/expiry permits only the next generation, elected by lowest comment id.
...
- DEC-005 [CQ-004] [AMB:AMB-003]: The complete lease/request census on `FS-GG/.github#2801` is the sole board-orchestrator authority. External requests are idempotent and bound to its live generation; stale generation races and alternative authority refs refuse. Same-generation contenders use GitHub comment order and losers delete only their own marker.
