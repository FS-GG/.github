...
- PD-003 [AC-003] [FR-003] complete: Derive a stable cycle digest and install each generation-bound issue-body hint before writing that participant's durable freeze receipt, so production `widen`/`set-paths` refuse before or at the first durable-freeze boundary. A failed or response-lost receipt write leaves an explicit retryable orphan hint rather than a silent bypass; retrying the idempotent hint/receipt sequence recovers it, while a hinted generation without its matching receipt refuses closed. Request room create/reuse plus both back-references after both freezes. Production writers retain the pre-existing clean-path REST budget, refuse removal of a frozen shared token before precedence, narrow only through the dedicated precedence writer, and leave the loser frozen for guarded resume.
