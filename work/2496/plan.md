...
- The new paragraph:
  ...
  5. States the cost rationale in place: bounded to **exactly one call per item**, made right before merge — by that point in the flow `Cache.Scheduling`'s 90-second TTL has almost always lapsed since `take`, so the bound is the call count itself (comparable to the one scan-cost `take`/`done` each already pay), not a warm-cache reuse the design does not depend on. A repeat call (if one ever happened) would still be cheap — `rebindAuthorization` makes a call against an already-current marker a zero-cost no-op PATCH-skip — but that is a robustness property, not the reason this step is affordable; "once per item" is (DEC-002). What shipped in the SKILL.md paragraph itself makes only the "once per item, no-op-on-repeat" claim, not a warm-cache claim — this line previously overstated it.
