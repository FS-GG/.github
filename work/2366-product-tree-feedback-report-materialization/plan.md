...
- VO-001 [PD-002] [PC-001] semanticTest: A constructed, non-committed fixture (temp registry + temp skill tree under `/tmp`, matching `tests/skill-registry/run.sh`'s existing `write_registry`/`write_manifests` heredoc convention) proves the new check fails on an unimplied cross-reference and on an unregistered-and-satisfiable cross-reference, and passes on an implied one and on the unsatisfiable-referencer case (`lane-steward`-shaped). Recorded under `Verification:` in the PR per the gate-inversion requirement, not committed to `tests/` (outside `Paths:`).
