...
## Scope
- SB-001: typed pure candidate inspect/render/verify plus a live authority-observing seal consuming #3208 lifecycle/review ledgers and rerunning #3209 qualification, staged-intake transaction #3105, bounded patches, acceptance receipt and evidence index, roadmap-close handoff, tests, package, and work-roadmap adoption.
...
- AC-003 [US-001] [FR-003]: Given caller-authored #3208 lifecycle/review receipts and #3209 qualification evidence, when acceptance is inspected, then the result is only an internally coherent candidate; acceptance requires immutable roadmap/catalog recompilation, exact unedited GitHub-ledger readback bound to the live winning claim and implementation commit, a production qualification rerun, and an independent pinned SDD rerun at the immutable subject.
...
- AC-005 [US-001] [FR-005]: Given implementation and acceptance revisions, when identities are checked, then implementation candidate/merge, acceptance candidate/merge, and protected-main are distinct where required and each squash merge preserves the corresponding candidate tree exactly.
- AC-006 [US-001] [FR-006]: Given a qualified candidate, when the live sealing boundary independently observes intake/board state, lifecycle, review, SDD execution artifacts, PR merges, remote trees, protected main, and merge time, then one canonical acceptance receipt and evidence index are sealed atomically; no pure or deserialization route can construct them.
...
- FR-003: Recompile the plan from the immutable `.github` roadmap and `FS.GG.Coordination` catalog; reuse #3208 canonical lifecycle/review validation against exact unedited GitHub comment exports bound to the live claim and implementation commit; rerun #3209 through its production qualification executor; and independently run pinned `fsgg-sdd` 1.5.0 in a fresh remote checkout, binding each artifact to the immutable candidate. (Stories: US-001; Acceptance: AC-003)
...
- FR-005: Validate distinct implementation candidate/merge, acceptance candidate/merge, and protected-main identities plus exact remote candidate/merge tree equality without collapsing phase-specific authority. (Stories: US-001; Acceptance: AC-005)
- FR-006: Keep caller input as an opaque internally coherent candidate and permit only the live BoardOps adapter to mint the opaque observed capability that seals the acceptance receipt and evidence index. (Stories: US-001; Acceptance: AC-006)
