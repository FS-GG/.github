...
- A-003 [Q-003]: Caller-authored receipts establish only internal coherence. Final acceptance recompiles the plan from the immutable `.github` roadmap and `FS.GG.Coordination` catalog, re-exports the unedited #3208 lifecycle ledger bound to the live winning claim and implementation commit, validates the live structured review chain and exact evidence envelope, reruns #3209 qualification, independently runs pinned `fsgg-sdd` 1.5.0 in a fresh remote checkout, and binds the resulting artifacts plus PR/tree/main facts to immutable GitHub authority.
- A-004 [Q-004]: Implementation candidate, implementation merge, acceptance candidate, acceptance merge, and protected-main are separate named identities. Each squash merge must preserve its candidate tree exactly; protected-main equals the observed accepted merge only at final handoff.
