...
- Preparation and acceptance-candidate inspection are pure; final acceptance is a read-only live observation boundary, while GitHub mutation remains behind the existing staged-intake transaction.
...
- Add pure CLI `inspect|render|verify` candidate entry points, a live read-only acceptance seal, and reuse the established board-ops staged-intake transaction for every GitHub write.
