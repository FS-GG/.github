...
- PD-006 [AC-001] [FR-001] complete: SCOPE WIDENING. `tests/FS.GG.Coord.Cli.BoardOps.Tests/ApplicationServiceTests.fs:2019` (`Assert.Equal(6, world.RestCalls)`, with the adjacent comment describing the overlap verdict's courtesy notice) pins the corrected REST cost for an OVERLAP `widen`. The fixture moved with BoardOps ownership; this citation follows that tracked location while preserving the historical decision.
