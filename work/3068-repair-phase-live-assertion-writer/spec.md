...
- AC-003 [US-002] [FR-003]: Given a stale head, wrong review URL, wrong PR, implementer grantor, current-critic grantor, duplicate assertion, malformed assertion, or caller-requested purpose that differs from the purpose derived from live state, when the live reader or writer evaluates it, then it refuses before appending authority or wait state.
...
- FR-003: The writer and reader MUST reject stale, wrong-subject, self-granted, critic-granted, duplicate, malformed, and wrong-purpose assertions without selecting a latest-wins fallback; refusal MUST occur before any append-only assertion or wait mutation. (Stories: US-002; Acceptance: AC-003)
- FR-004: The CLI contract, live oracle, and live writer MUST derive the exact authorized purpose and next command from the same live state, including all required bindings, by resolving a unique exhausted predecessor from typed item cross-references and its review ledger rather than requiring hand-authored authority or item/PR number equality. Caller syntax MAY assert the expected purpose but MUST NOT select a different immutable semantic route. (Stories: US-002; Acceptance: AC-004)
