...
- CQ-002 [AMB:AMB-002] answer: Projects-v2 supplies no server-side compare-and-set, so every authoritative `Blocked by` writer must acquire the same server-ordered REST issue-comment mutation lease before it observes, derives, or writes the field and retain that lease through the GraphQL mutation. This includes add/remove, explicit replacement/clear, `set-field --batch`, `release --blocked-by`, intake, reconcile lifecycle repairs, and replay of a previously deferred write by `flush`. A contended or uncertain replay remains queued without duplication, while a replay whose mutation is known to have landed is removed even if ticket cleanup is uncertain. Add/remove still pass `{ Value; Revision }` to the guarded boundary and immediately re-read it, but that check is only a secondary stale detector inside the lease, not the atomicity mechanism. Tests must inject a changed second observation and prove mutation count zero, and must prove direct and deferred routes cannot bypass an active derived writer.
...
- DEC-002 [CQ-002] [AMB:AMB-002] [FR-003] [AC-003]: Put every authoritative `Blocked by` mutation route behind `Writes.withBlockedByMutationLease`, acquired before observation and held through mutation. The shared lease covers add/remove, explicit replacement/clear, `set-field --batch`, `release --blocked-by`, intake, reconcile, and `Board.flush` replay. Flush retains an entry when contention or transport uncertainty prevents a known mutation, drops it only when replay is known to have landed, and never re-appends it. Keep `Board.boardWriteGuarded` as a secondary detector: it accepts `BlockedByObservation`, re-reads immediately before mutation, and emits no mutation on mismatch. Fixtures control observations, the issue-comment lease, and the deferred queue independently, assert a stale diagnostic plus zero update/clear documents on mismatch, and assert that a lower-ID contender fences batch/release/flush without any board mutation.
...
- DEC-004 [FR-006] [AC-006]: Mutation controls independently replace union with requested-only, subtraction with empty, bypass the shared lease from an authoritative direct route, bypass it from deferred `flush` replay, and replace stale comparison with unconditional success; lint controls suppress the body comparison. Each control must red a named focused test, then be restored before the full gates.
