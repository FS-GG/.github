...
- VO-001 [PD-001] [PD-002] [PD-003] [PD-004] [PD-005] [PD-006] [PC-001] semanticTest: Run focused parser, BoardOps, GitHub Board, lint application, and production write-harness tests; demonstrate red/green inversions for union preservation, subtraction preservation, direct-writer lease bypass, deferred-flush lease bypass, stale refusal, lint comparison, and intake/reconcile lease-wire ordering; prove batch/release/flush use the shared lease, a losing contender emits zero mutations while flush retains one queue entry, intake brackets its atomic dependency projection, and reconcile releases its lease before writing the durable lifecycle receipt; then run all affected projects, the solution, formatting, signature/projection generation checks, and the SDD evidence/analyze/verify/ship fixed point.
