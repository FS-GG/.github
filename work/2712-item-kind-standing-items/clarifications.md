...
- DEC-003 [AMB:AMB-003] A body that could not be read yields `Kind = None`, and `None` means `Work` at the reducer — which is exactly today's behaviour for that row and therefore introduces no new failure. This is deliberately NOT closed by letting the board `Kind` column govern: ADR-0066 and `Schedulability.fs:126` both refuse to let a lagging projection decide, and `Class` collapses "no declaration" with "body unread" for the same reason. The residual is stated rather than hidden: a standing row whose body could not be read is not *known* to be standing, and the engine treats it as it does today. **Its size is `Done`, not `Backlog`.** An earlier draft of this line recorded the bound as `TouchSet.Unreadable` → `Deferred` → `Backlog`; the first arrow holds, the rest does not. `Deferred` → `Backlog` is only `projectWithIntent`'s LAST-RESORT arm, and four observation-driven arms sit **above** the intent dispatch — so a closed row carrying a done receipt projects `Project(Done, …)` whatever the intent, with or without a watermark. That is precisely the state this row was filed to stop (`.github#2106` sitting `Done` while its issue must stay `OPEN`), which is why the bound is recorded at its true size and pinned by `LifecycleProjectionTests`' *"the recorded residual bound is the one the reducer actually has"* rather than by this prose alone. *Verification:* executed against the built Release assembly — `reduce Work (Deferred …) closedDoneObservation` → `Project (Done, 10L)`; the same row **declaring** `Kind: register` → `Exempt Register`. The residual is nevertheless accepted rather than closed, and remains genuinely bounded: it needs the issue CLOSED **and** carrying a done receipt **and** its body unreadable on that pass, and the path is instrumented for operators (`Program.fs:255-257` counts `bodyUnreadable`). Closing it needs a fact the scan can carry that a failed body read cannot destroy, which is a separate cause and not this row's.
