...
- FR-002: Every heading adjudicated as authored judgement contract is restored to .claude/skills/pnext-item/references/independent-review.md and its .agents mirror, byte-identical between the two mirrors, and to a CITED home — each restored section is named by at least one live fragment citation from a non-exempt tracked document, so that deleting it reds the gate FR-004 builds. A restored section that nothing cites is not restored to the home AC-1 requires: the gate is blind to its deletion, which is the exact failure this work item exists to close. Citations from docs/adr/ do not satisfy this, because that prefix is exempt from the gate's subject selection. (Stories: US-001; Acceptance: AC-001)
