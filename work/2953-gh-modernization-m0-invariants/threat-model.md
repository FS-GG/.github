...
| A maintainer edits, copies, retracts, misattributes, or self-references review comments to fabricate authority. | `githubMutableState`; false review acceptance or completion. | Each role first posts a distinct narrative comment, then an exact current-head attestation with exactly one canonical final `Evidence` URL. Discovery resolves the earlier comment from the complete repair-PR ledger and requires both records to be unedited, from the same authorized GitHub author with an allowed association, ordered by ID and creation time, and on the same PR; missing, self, later, edited, wrong-author, outsider, wrong-PR, or trailing content fails closed. | GitHub administrators can still delete hosted objects; complete rereads fail closed and exported digests aid reconstruction. |
