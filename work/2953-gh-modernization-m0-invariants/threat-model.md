...
| A maintainer edits, copies, retracts, misattributes, or self-references review comments to fabricate authority; viewer-dependent `author_association` hides a private organization membership; or a malformed login exploits divergent identity parsing. | `githubMutableState`; false review acceptance, false rejection, or completion. | Each role first posts a distinct narrative comment, then an exact current-head attestation with exactly one canonical final `Evidence` URL. Discovery resolves the earlier comment from the complete repair-PR ledger and requires both records to be unedited, from the same authorized GitHub `User`, ordered by ID and creation time, and on the same PR. Before either association or allowlist authorization, every login must be 1–39 ASCII characters, use alphanumeric endpoints, and contain only alphanumerics or isolated internal hyphens. Authorization is then either an allowed live association or an exact fingerprint-bound allowlist match. Bots, malformed/missing users, non-allowlisted `CONTRIBUTOR`/`NONE`, missing, self, later, edited, wrong-author, wrong-PR, or trailing content fail closed. | GitHub administrators can still delete hosted objects or an allowlisted account can be compromised; complete rereads fail closed, every allowlist change invalidates the fingerprint, and exported digests aid reconstruction. |
...
unknown-writer, corpus, deletion-unit, threat-source, reviewer-fingerprint, attestation-grammar, and
narrative-reference mutations, and cite its earlier distinct unedited narrative record from the exact
attestation. A material mismatch blocks ADR acceptance.
