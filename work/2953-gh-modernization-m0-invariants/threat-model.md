...
| A maintainer edits, copies, retracts, misattributes, or self-references review comments to fabricate authority; or viewer-dependent `author_association` hides a private organization membership from the workflow token. | `githubMutableState`; false review acceptance, false rejection, or completion. | Each role first posts a distinct narrative comment, then an exact current-head attestation with exactly one canonical final `Evidence` URL. Discovery resolves the earlier comment from the complete repair-PR ledger and requires both records to be unedited, from the same authorized GitHub `User`, ordered by ID and creation time, and on the same PR. Authorization is either an allowed live association or an exact login in the fingerprint-bound unique `reviewAuthorAllowlist`; Bots, missing users, non-allowlisted `CONTRIBUTOR`/`NONE`, missing, self, later, edited, wrong-author, wrong-PR, or trailing content fail closed. | GitHub administrators can still delete hosted objects or an allowlisted account can be compromised; complete rereads fail closed, every allowlist change invalidates the fingerprint, and exported digests aid reconstruction. |
