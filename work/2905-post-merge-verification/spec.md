...
## Scope
- SB-001: Add a typed post-merge verification fact and receipt, exact-SHA/default-branch live evidence collection, completion admission, durable receipt binding, and focused inversion-backed tests.
...
- AC-002 [US-001] [FR-001] [FR-003]: Given the merged PR's exact merge SHA has a completed successful run whose event and branch identify the repository default branch, when delivery completion is retried, then the merge becomes `Verified` and may advance to terminal projection once every existing obligation also passes.
- AC-003 [US-001] [US-002] [FR-002] [FR-004]: Given exact-SHA post-merge evidence is absent, pending, red, cancelled, malformed, branch-mismatched, SHA-mismatched, or unreadable, when completion is inspected, then no completion receipt, issue closure, Done projection, or claim release occurs and the diagnostic distinguishes the observed state.
...
- AC-005 [US-001] [US-002] [FR-006]: Given the qualifying-evidence predicate or completion admission is inverted, when focused tests run, then a PR-green/no-post-merge-run world fails, an exact matching green world passes, and restoration returns all focused and affected suites to green.
...
- FR-002: Completion admission fails closed unless verification is a successful, completed execution for the exact merge SHA on the repository default branch; PR-head success, reachability, an empty run set, mismatched executions, and read failures never imply verification. (Stories: US-001, US-002; Acceptance: AC-001, AC-003)
- FR-003: The live delivery adapter reads post-merge execution evidence only after GitHub reports the PR merged, preserves pending/red/unreadable distinctions, and feeds the same typed fact to both the lifecycle reducer and completion writer. (Stories: US-001, US-002; Acceptance: AC-002, AC-003, AC-004)
...
- FR-006: Focused unit and live-adapter tests prove the inversion: PR-green plus merge/reachability but no qualifying branch run refuses all terminal writes, while adding the exact matching green run enables precisely the existing completion saga. (Stories: US-001, US-002; Acceptance: AC-005)
