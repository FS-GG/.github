...
def main() -> int:
    ...
    try:
        ...
        roadmap_path = (len(parts) >= 5 and parts[:2] == ("logs", "roadmap")
                        and parts[-2] == args.run and parts[-1] == f"{args.unit}.jsonl")
        item_path = (len(parts) == 5 and parts[:2] == ("logs", "items")
                     and re.fullmatch(r"[A-Za-z0-9_.-]+\.[A-Za-z0-9_.-]+", parts[2]) is not None
                     and parts[3].isdigit() and int(parts[3]) > 0 and parts[4] == "lifecycle.jsonl")
        if not roadmap_path and not item_path:
            fail("log path must be a canonical logs/items or logs/roadmap lifecycle path")
        if not path.is_file():
