...
FIELDS = [
    "timestamp", "task", "session_id", "thread_id", "turn_id", "provider", "model", "effort",
    "input", "cached_input", "cache_write_input", "output", "reasoning", "total",
    "thread_input", "thread_cached_input", "thread_cache_write_input", "thread_output",
...
def codex_rows(lines: list[str], task: str, wanted_turn: str | None, source: str) -> list[dict[str, object]]:
    ...
    for line_number, line in enumerate(lines, 1):
        ...
        if record.get("type") == "turn_context" and isinstance(payload, dict):
            ...
            if isinstance(turn, str):
                final[turn] = {"timestamp": record.get("timestamp"), "payload": payload}
    if not final:
        raise InvalidUsage("no token_usage_record rows found")
    selected = [wanted_turn] if wanted_turn else [next(reversed(final))]
    rows: list[dict[str, object]] = []
    for turn in selected:
        if turn not in final:
            raise InvalidUsage(f"turn_id not found: {turn}")
        entry = final[turn]
        ...
        row: dict[str, object] = {
            ...
            "provider": "OpenAI", "model": model, "effort": effort, "source": source,
        }
        row.update(turn_counts)
...
def claude_row(snapshot: dict[str, object], task: str, source: str) -> dict[str, object]:
    ...
    return {
        ...
        "turn_id": required_text(snapshot.get("prompt_id"), "prompt_id"), "provider": "Anthropic",
...
def self_test() -> None:
    ...
    lines = [json.dumps(context), json.dumps(record)]
    ...
    try:
        ...
        if args.runtime == "codex":
            ...
            rows = codex_rows(args.session_file.read_text(encoding="utf-8").splitlines(), task,
                              args.turn_id, str(args.session_file))
