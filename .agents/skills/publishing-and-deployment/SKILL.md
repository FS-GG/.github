...
Registry release inventory: 14 package-bearing contracts across 9 producers; 13 coherent release sets.

| owner | contract | source version | published version | coherent set | surface |
|---|---|---|---|---|---|
| `fsquint` | `fsquint-replay` | `0.1.0-preview.2` | `0.1.0-preview.2` | `fsquint:0.1.0-preview.2` | FsQuint and FsQuint.Tooling coherent package set; raw ITF, schema-v1 replay and optional bounded Quint process API (ADR-0085) |
...
| `github` | `coord-engine` | `0.89.0` | `0.89.0` | `github:0.89.0` | the `fsgg-coord-engine` CLI verb surface (claim/take/batch/who/widen/set-paths/say/landable/done/release/flush/…) + its exit-code contract, emitted from src/FS.GG.Coord.Core/Protocol.fs; shipped as the FS.GG.Coord.Cli dotnet tool |
