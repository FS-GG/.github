...
- **2026-07-05** — Coordination-kit `cross-repo-coordination` + `fsgg-coord` sha256 bumped (@.github; ADR-0019): added an **earned done-stamp** — a `fsgg-coord done <issue> [--pr N] [--flip]` subcommand that verifies an item is finished (closing PR merged **and** board `Status: Done`) in one thrifty query and prints a greppable green `FSGG-DONE` / red `FSGG-NOT-DONE` two-line stamp, plus a *Signal an item is finished* skill section wiring it into the release dance's "Land + record" step. Re-digested skill `30946d20…` → `ad7ae7e5…` and client `3ac2eb42…` → `66c7431f…`. Propagates to `coordination-kit` receivers.
